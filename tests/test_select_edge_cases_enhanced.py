...
class TestServiceIntegrationTimeoutRecovery:
    ...
    @pytest.mark.asyncio
    async def test_service_call_timeout_during_option_change(
        self, service_dependent_select
    ):
        """Test service call timeout during option selection."""
        # Mock service timeout
        with patch.object(
            service_dependent_select, "_async_set_select_option"
        ) as mock_set:
            mock_set.side_effect = asyncio.TimeoutError("Service timeout")

            # Should handle timeout gracefully
            with pytest.raises(
                HomeAssistantError, match="Failed to set notification_priority"
            ):
    ...
    async def test_coordinator_refresh_timeout_recovery(self, mock_coordinator):
        ...
        mock_coordinator.async_refresh_dog.side_effect = asyncio.TimeoutError(
            "Coordinator timeout"
        )
    ...
    @pytest.mark.asyncio
    async def test_partial_service_availability_during_setup(
        ...
        with patch(
            ...
            mock_has_service.side_effect = (
                lambda domain, service: service != "unavailable_service"
            )
    ...
    @pytest.mark.asyncio
    async def test_network_partition_during_option_change(
        self, service_dependent_select
    ):
...
class TestEntityRegistryCollisionResolution:
    ...
    @pytest.mark.asyncio
    async def test_entity_registry_stress_with_many_selects(
...
class TestPerformanceOptimizationUnderLoad:
    ...
    @pytest.mark.asyncio
    async def test_batch_processing_performance_with_large_datasets(
...
class TestConfigurationMigrationBackwardCompatibility:
    ...
    @pytest.mark.asyncio
    async def test_legacy_option_format_migration(
        self, hass: HomeAssistant, mock_coordinator
    ):
