...
class TestCoordinatorPerformanceBenchmark:
    ...
    @pytest.mark.asyncio
    async def test_single_dog_update_performance(self, hass: HomeAssistant, small_config):
        ...
        assert results["mean"] < 0.1, f"Mean update time {results['mean']:.3f}s exceeds 100ms threshold"
        assert results["max"] < 0.2, f"Max update time {results['max']:.3f}s exceeds 200ms threshold"
        
        print(f"Single dog performance: {results['mean']:.3f}s ± {results['stdev']:.3f}s")

    @pytest.mark.asyncio
    async def test_medium_load_update_performance(self, hass: HomeAssistant, medium_config):
        ...
        assert results["mean"] < 0.2, f"Mean update time {results['mean']:.3f}s exceeds 200ms threshold"
        assert results["max"] < 0.4, f"Max update time {results['max']:.3f}s exceeds 400ms threshold"
        
        print(f"Medium load (5 dogs) performance: {results['mean']:.3f}s ± {results['stdev']:.3f}s")

    @pytest.mark.asyncio
    async def test_large_load_update_performance(self, hass: HomeAssistant, large_config):
        ...
        assert results["mean"] < 0.5, f"Mean update time {results['mean']:.3f}s exceeds 500ms threshold"
        assert results["max"] < 1.0, f"Max update time {results['max']:.3f}s exceeds 1s threshold"
        
        print(f"Large load (20 dogs) performance: {results['mean']:.3f}s ± {results['stdev']:.3f}s")
    ...
    async def test_parallelization_efficiency(self, hass: HomeAssistant, medium_config):
        ...
        coordinator = PawControlCoordinator(hass, medium_config)
        managers = self.create_slow_mock_managers()  # Use slower managers to see parallelization benefit
        coordinator.set_managers(**managers)
        ...
        assert results["mean"] < 1.0, f"Parallel execution not efficient: {results['mean']:.3f}s"
        
        # Should be closer to single-dog time than 5x single-dog time
        single_dog_equivalent = 0.4  # 4 managers * 100ms each
        assert results["mean"] < single_dog_equivalent * 2, "Parallelization not providing benefit"
        
        print(f"Parallelization test: {results['mean']:.3f}s (should be ~400ms, not 2000ms)")

    @pytest.mark.asyncio
    async def test_cache_manager_performance_impact(self, hass: HomeAssistant, medium_config):
        ...
        print(f"Cache impact: cold={cold_time:.3f}s, warm={warm_time:.3f}s, overhead={cache_overhead:.2f}x")

    @pytest.mark.asyncio
    async def test_performance_monitoring_overhead(self, hass: HomeAssistant, medium_config):
        ...
        assert results["mean"] < 0.3, "Performance monitoring causing excessive overhead"
        
        print(f"Monitoring overhead test: {results['mean']:.3f}s with full monitoring")

    @pytest.mark.asyncio
    async def test_batch_manager_performance_impact(self, hass: HomeAssistant, medium_config):
        ...
        assert selective_time < 0.1, f"Selective refresh too slow: {selective_time:.3f}s"
        ...
        print(f"Batch manager: selective_refresh={selective_time:.3f}s, batch_processing={batch_time:.3f}s")

    @pytest.mark.asyncio
    async def test_memory_efficiency_under_load(self, hass: HomeAssistant, large_config):
    ...
    @pytest.mark.asyncio
    async def test_error_handling_performance_impact(self, hass: HomeAssistant, medium_config):
        ...
        assert results["mean"] < 0.5, f"Error handling causing excessive overhead: {results['mean']:.3f}s"
        
        # Check that some errors were handled
        perf_stats = coordinator._performance_monitor.get_stats()
        assert perf_stats["total_errors"] > 0, "No errors were recorded"
        
        print(f"Error handling test: {results['mean']:.3f}s with {perf_stats['total_errors']} errors")
    ...
    @pytest.mark.asyncio 
    async def test_coordinator_initialization_performance(self, hass: HomeAssistant, large_config):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_access_performance(self, hass: HomeAssistant, medium_config):
        ...
        sensor_tasks = [asyncio.create_task(simulate_sensor_access()) for _ in range(10)]
        ...
        assert concurrent_time < 1.0, f"Concurrent access too slow: {concurrent_time:.3f}s"
