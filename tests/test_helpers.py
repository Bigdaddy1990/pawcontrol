...
class TestPawControlDataStorage:
    ...
    def test_initialization(self, hass: HomeAssistant, mock_config_entry, mock_storage):
        ...
        assert len(storage_manager._stores) == 5  # walks, feedings, health, routes, stats
    ...
    def test_initialize_stores(self, hass: HomeAssistant, mock_config_entry, mock_storage):
        ...
        for call_args in calls:
            ...
            assert args[2].startswith(f"{DOMAIN}_{mock_config_entry.entry_id}")  # Key format
    ...
    async def test_load_all_data_success(self, data_storage, mock_storage):
        ...
        assert len(data) == 5
        assert all(store_key in data for store_key in ["walks", "feedings", "health", "routes", "statistics"])
...
class TestPawControlData:
    ...
    async def test_load_data_success(self, data_manager):
        ...
        with patch.object(data_manager.storage, 'async_load_all_data', return_value=test_data):
    ...
    async def test_load_data_failure(self, data_manager):
        ...
        with patch.object(data_manager.storage, 'async_load_all_data', side_effect=Exception("Storage error")):
            await data_manager.async_load_data()
            ...
            assert all(isinstance(data_manager._data[key], dict) for key in expected_keys)

    async def test_log_feeding_success(self, hass: HomeAssistant, data_manager, sample_feeding_data):
        ...
        with patch.object(data_manager.storage, 'async_save_data') as mock_save:
            ...
            mock_save.assert_called_once_with("feedings", data_manager._data["feedings"])
    ...
    async def test_log_feeding_fires_event(self, hass: HomeAssistant, data_manager, sample_feeding_data):
    ...
    async def test_start_walk_success(self, hass: HomeAssistant, data_manager, sample_walk_data):
    ...
    async def test_end_walk_success(self, hass: HomeAssistant, data_manager):
        ...
        with patch.object(data_manager.storage, 'async_save_data') as mock_save:
            ...
            assert data_manager._data["walks"]["test_dog"]["history"][0] == completed_walk
    ...
    async def test_log_health_success(self, hass: HomeAssistant, data_manager, sample_health_data):
...
class TestPawControlNotificationManager:
    ...
    async def test_send_notification_normal_priority(self, notification_manager):
        ...
        with patch.object(notification_manager, '_should_send_notification', return_value=True):
    ...
    async def test_send_notification_suppressed_quiet_hours(self, notification_manager):
        ...
        with patch.object(notification_manager, '_should_send_notification', return_value=False):
            await notification_manager.async_send_notification(
                "test_dog",
                "Test Title",
                "Test Message"
    ...
    async def test_send_notification_now_success(self, hass: HomeAssistant, notification_manager):
    ...
    async def test_send_notification_now_urgent_styling(self, hass: HomeAssistant, notification_manager):
    ...
    async def test_send_notification_now_high_styling(self, hass: HomeAssistant, notification_manager):
    ...
    def test_should_send_notification_quiet_hours_disabled(self, notification_manager):
        ...
        notification_manager.config_entry.options[CONF_NOTIFICATIONS][CONF_QUIET_HOURS] = False
    ...
    def test_should_send_notification_quiet_hours_active(self, notification_manager):
        ...
        with patch('homeassistant.util.dt.now') as mock_now:
            mock_time = Mock()
            mock_time.time.return_value = datetime.strptime("23:00:00", "%H:%M:%S").time()
    ...
    def test_should_send_notification_outside_quiet_hours(self, notification_manager):
        ...
        with patch('homeassistant.util.dt.now') as mock_now:
            mock_time = Mock()
            mock_time.time.return_value = datetime.strptime("12:00:00", "%H:%M:%S").time()
            mock_now.return_value = mock_time
            
            result = notification_manager._should_send_notification("normal")
            assert result is True

    def test_should_send_notification_quiet_hours_span_midnight(self, notification_manager):
        ...
        with patch('homeassistant.util.dt.now') as mock_now:
            mock_time = Mock()
            mock_time.time.return_value = datetime.strptime("01:00:00", "%H:%M:%S").time()
            mock_now.return_value = mock_time
    ...
    def test_should_send_notification_invalid_time_format(self, notification_manager):
        ...
        notification_manager.config_entry.options[CONF_NOTIFICATIONS].update({
            CONF_QUIET_START: "invalid_time",
            CONF_QUIET_END: "also_invalid"
        })
...
class TestHelpersIntegration:
    ...
    async def test_data_manager_with_storage_integration(self, hass: HomeAssistant, mock_config_entry):
        """Test data manager integration with storage."""
        # Create data manager and load initial data
        data_manager = PawControlData(hass, mock_config_entry)
        
        with patch.object(data_manager.storage, 'async_load_all_data', return_value={}), \
            ...
            await data_manager.async_start_walk("test_dog", {"start_time": dt_util.utcnow().isoformat()})
            await data_manager.async_end_walk("test_dog")
            
            # Should have made multiple save calls
            assert mock_save.call_count >= 3

    async def test_notification_manager_integration(self, hass: HomeAssistant, mock_config_entry):
        ...
        with patch("homeassistant.helpers.event.async_track_time_interval"):
            ...
            await manager.async_send_notification("test_dog", "Normal", "Normal message", "normal")
            await manager.async_send_notification("test_dog", "High", "High message", "high")
            await manager.async_send_notification("test_dog", "Urgent", "Urgent message", "urgent")
    ...
    async def test_full_workflow_integration(self, hass: HomeAssistant, mock_config_entry):
        ...
        with patch.object(data_manager.storage, 'async_load_all_data', return_value={}), \
            ...
            notification_manager = PawControlNotificationManager(hass, mock_config_entry)
            ...
            assert hass.services.async_call.call_count == 1  # High priority notification
...
class TestHelpersPerformance:
    ...
    async def test_data_storage_concurrent_operations(self, data_storage):
        ...
        with patch.object(data_storage, '_load_store_data', return_value={}) as mock_load:
...
class TestHelpersErrorHandling:
    ...
    async def test_notification_manager_service_failure(self, hass: HomeAssistant, notification_manager):
