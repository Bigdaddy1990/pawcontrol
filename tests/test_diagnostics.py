...
class DummyCoordinator:
    def __init__(self):
        self._dog_data = {"dog1": {"location":{"is_home": False,"current_location":"SECRET"},
...
async def test_diagnostics_redacts_sensitive(hass: HomeAssistant):
    entry = SimpleNamespace(entry_id="abc123", version=1, domain=DOMAIN, title="Paw Control")
    hass.data.setdefault(DOMAIN, {})[entry.entry_id] = {"coordinator": DummyCoordinator()}
