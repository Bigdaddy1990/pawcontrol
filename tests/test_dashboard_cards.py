...
class TestOverviewCardGenerator:
    ...
    @pytest.fixture
    def overview_generator(self, mock_hass_cards, mock_templates) -> OverviewCardGenerator:
    ...
    async def test_welcome_card_generation(
        ...
        welcome_card = await overview_generator.generate_welcome_card(dogs_config, options)
    ...
    async def test_quick_actions_generation(
        ...
        feed_button = next(
            card for card in quick_actions["cards"] 
            if card["name"] == "Feed All"
        )
...
class TestDogCardGenerator:
    ...
    async def test_gps_map_card_generation(
        self, dog_generator: DogCardGenerator
    ):
    ...
    async def test_gps_map_card_unavailable(
        self, dog_generator: DogCardGenerator
    ):
...
class TestHealthAwareFeedingCardGenerator:
    ...
    async def test_health_feeding_overview(
        self, health_feeding_generator: HealthAwareFeedingCardGenerator, sample_dog_config
    ):
        ...
        health_card = next(
            ...
            if "Health Feeding" in card.get("title", "")
        )
        assert health_card["type"] == "entities"
        assert len(health_card["entities"]) >= 4  # Health status, target, consumed, adjustment
    ...
    async def test_portion_calculator_card(
        ...
        with MockedEntity("sensor.max_health_aware_portions"):
            portion_card = await health_feeding_generator._generate_portion_calculator_card(
                "max", {}
            )
...
class TestModuleCardGenerator:
    ...
    async def test_feeding_cards_standard(
        ...
        schedule_card = next(
            card for card in cards 
            if card.get("title") == "Feeding Schedule"
        )
    ...
    async def test_health_cards_generation(
        ...
        metrics_card = next(
            card for card in cards
            if card.get("title") == "Health Metrics"
        )
    ...
    async def test_gps_cards_no_tracker(
        self, module_generator: ModuleCardGenerator
    ):
...
class TestStatisticsCardGenerator:
    ...
    @pytest.fixture
    def stats_generator(self, mock_hass_cards, mock_templates) -> StatisticsCardGenerator:
...
class TestCardEntityValidation:
    ...
    async def test_entity_validation_available(
        self, base_generator
    ):
    ...
    async def test_entity_validation_mixed(
        self, base_generator
    ):
    ...
    async def test_entity_exists_check(
        self, base_generator
    ):
