...
class TestAsyncSetupEntry:
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_with_gps_dogs(self, hass: HomeAssistant, mock_config_entry_with_gps, mock_coordinator):
        """Test setup entry with GPS-enabled dogs."""
        mock_add_entities = Mock()

        with patch("custom_components.pawcontrol.device_tracker._async_add_entities_in_batches", new_callable=AsyncMock) as mock_batch_add:
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_no_gps_dogs(self, hass: HomeAssistant, mock_config_entry_no_gps, mock_coordinator):
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_legacy_data_structure(self, hass: HomeAssistant, mock_coordinator):
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_empty_dogs_list(self, hass: HomeAssistant, mock_coordinator):
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_missing_coordinator_data(self, hass: HomeAssistant):
    ...
    @pytest.mark.asyncio
    async def test_async_setup_entry_missing_required_dog_keys(self, hass: HomeAssistant, mock_coordinator):
...
class TestPawControlDeviceTracker:
    ...
    def test_latitude_from_gps_data(self, device_tracker, mock_coordinator, mock_gps_data):
    ...
    def test_longitude_from_gps_data(self, device_tracker, mock_coordinator, mock_gps_data):
    ...
    def test_location_accuracy_from_gps_data(self, device_tracker, mock_coordinator, mock_gps_data):
    ...
    def test_location_accuracy_from_restored_data(self, device_tracker, mock_coordinator):
    ...
    def test_battery_level_from_gps_data(self, device_tracker, mock_coordinator, mock_gps_data):
    ...
    def test_location_name_home(self, device_tracker, mock_coordinator, mock_gps_data, hass: HomeAssistant):
    ...
    def test_location_name_not_home(self, device_tracker, mock_coordinator, mock_gps_data, hass: HomeAssistant):
    ...
    def test_determine_zone_from_coordinates_in_zone(self, device_tracker, hass: HomeAssistant):
        ...
        with patch.object(hass.states, 'async_all', return_value=[mock_zone_state]):
            zone_name = device_tracker._determine_zone_from_coordinates(37.7749, -122.4194)
            assert zone_name == "Dog Park"

    def test_determine_zone_from_coordinates_not_in_zone(self, device_tracker, hass: HomeAssistant):
    ...
    def test_determine_zone_from_coordinates_missing_attributes(self, device_tracker, hass: HomeAssistant):
    ...
    def test_extra_state_attributes_with_gps_data(self, device_tracker, mock_coordinator, mock_gps_data):
    ...
    def test_extra_state_attributes_without_gps_data(self, device_tracker, mock_coordinator):
    ...
    def test_is_currently_moving_with_location_change(self, device_tracker, mock_gps_data):
        ...
        with patch("custom_components.pawcontrol.device_tracker.distance", return_value=0.015):  # 15 meters
            assert device_tracker._is_currently_moving(mock_gps_data) is True

    def test_is_currently_moving_no_previous_location(self, device_tracker, mock_gps_data):
    ...
    def test_assess_gps_signal_quality_variations(self, device_tracker):
        ...
        # Test unknown signal
        assert device_tracker._assess_gps_signal_quality({"accuracy": None}) == "unknown"
    ...
    def test_assess_data_freshness_variations(self, device_tracker):
        ...
        assert device_tracker._assess_data_freshness({"last_seen": current_time}) == "current"
        
        # Test recent data
        recent_time = (now - timedelta(minutes=3)).isoformat()
        assert device_tracker._assess_data_freshness({"last_seen": recent_time}) == "recent"
        
        # Test stale data
        stale_time = (now - timedelta(minutes=10)).isoformat()
        assert device_tracker._assess_data_freshness({"last_seen": stale_time}) == "stale"
        ...
        assert device_tracker._assess_data_freshness({"last_seen": "invalid-date"}) == "unknown"

    def test_get_tracking_status_variations(self, device_tracker):
        """Test tracking status assessment."""
        # Test no location
        with patch.object(device_tracker, '_assess_gps_signal_quality', return_value="good"):
            with patch.object(device_tracker, '_assess_data_freshness', return_value="current"):
                with patch.object(device_tracker, '_get_battery_status', return_value="good"):
                    status = device_tracker._get_tracking_status({"latitude": None, "longitude": None})
                    assert status == "no_location"
        
        # Test active tracking
        with patch.object(device_tracker, '_assess_gps_signal_quality', return_value="excellent"):
            with patch.object(device_tracker, '_assess_data_freshness', return_value="current"):
                with patch.object(device_tracker, '_get_battery_status', return_value="good"):
                    status = device_tracker._get_tracking_status({"latitude": 37.7749, "longitude": -122.4194})
                    assert status == "tracking_active"
        
        # Test tracking with low battery
        with patch.object(device_tracker, '_assess_gps_signal_quality', return_value="good"):
            with patch.object(device_tracker, '_assess_data_freshness', return_value="current"):
                with patch.object(device_tracker, '_get_battery_status', return_value="low"):
                    status = device_tracker._get_tracking_status({"latitude": 37.7749, "longitude": -122.4194})
        ...
        with patch.object(device_tracker, '_assess_gps_signal_quality', return_value="good"):
            with patch.object(device_tracker, '_assess_data_freshness', return_value="current"):
                with patch.object(device_tracker, '_get_battery_status', return_value="critical"):
                    status = device_tracker._get_tracking_status({"latitude": 37.7749, "longitude": -122.4194})
                    assert status == "tracking_battery_critical"
        
        # Test degraded tracking
        with patch.object(device_tracker, '_assess_gps_signal_quality', return_value="poor"):
            with patch.object(device_tracker, '_assess_data_freshness', return_value="stale"):
                with patch.object(device_tracker, '_get_battery_status', return_value="good"):
                    status = device_tracker._get_tracking_status({"latitude": 37.7749, "longitude": -122.4194})
    ...
    def test_available_with_coordinates_fallback(self, device_tracker, mock_coordinator):
    ...
    @pytest.mark.asyncio
    async def test_async_added_to_hass_without_previous_state(self, hass: HomeAssistant, device_tracker):
    ...
    @pytest.mark.asyncio
    async def test_async_added_to_hass_with_previous_state(self, hass: HomeAssistant, device_tracker):
        ...
        with patch.object(device_tracker, 'async_get_last_state', return_value=mock_state):
            await device_tracker.async_added_to_hass()
    ...
    def test_handle_coordinator_update_with_location_change(self, device_tracker, mock_coordinator, hass: HomeAssistant):
        ...
        with patch("custom_components.pawcontrol.device_tracker.distance", return_value=0.015):  # 15 meters
            with patch.object(device_tracker, '_update_location_history') as mock_update_history:
    ...
    def test_handle_coordinator_update_insignificant_change(self, device_tracker, mock_coordinator, hass: HomeAssistant):
        ...
        with patch("custom_components.pawcontrol.device_tracker.distance", return_value=0.005):  # 5 meters
            with patch.object(device_tracker, '_update_location_history') as mock_update_history:
                device_tracker._handle_coordinator_update()
    ...
    def test_handle_coordinator_update_first_location(self, device_tracker, mock_coordinator, hass: HomeAssistant):
    ...
    def test_handle_coordinator_update_zone_change(self, device_tracker, mock_coordinator, hass: HomeAssistant):
        ...
        with patch.object(device_tracker, 'location_name', STATE_NOT_HOME):
            with patch.object(device_tracker, '_handle_zone_change') as mock_zone_change:
                device_tracker._handle_coordinator_update()
    ...
    def test_get_dog_data_coordinator_unavailable(self, device_tracker, mock_coordinator):
    ...
    @pytest.mark.asyncio
    async def test_async_update_location_valid_coordinates(self, device_tracker, mock_coordinator):
    ...
    def test_calculate_distance_traveled_with_history(self, device_tracker):
        ...
        with patch("custom_components.pawcontrol.device_tracker.distance", return_value=0.1):  # 0.1 km
    ...
    def test_extra_state_attributes_with_walk_data(self, device_tracker, mock_coordinator):
    ...
    def test_extra_state_attributes_with_dog_info(self, device_tracker, mock_coordinator):
...
class TestDeviceTrackerIntegrationScenarios:
    ...
    def test_tracker_isolation_between_dogs(self, mock_coordinator):
        ...
        def mock_get_module_data(dog_id, module):
            if dog_id == "dog1" and module == "gps":
                return {"latitude": 37.7749, "longitude": -122.4194, "battery_level": 85}
    ...
    def test_gps_data_edge_cases(self, mock_coordinator):
        ...
        edge_cases = [
            ...
            ({"latitude": "invalid", "longitude": "invalid"}, None, None),  # Invalid types
            ({"latitude": 91.0, "longitude": -122.4194}, 91.0, -122.4194),  # Out of range lat
            ({"latitude": 37.7749, "longitude": 181.0}, 37.7749, 181.0),  # Out of range lon
        ]
    ...
    def test_battery_monitoring_comprehensive(self, mock_coordinator):
        ...
        for battery_level, expected_status in battery_scenarios:
            mock_coordinator.get_module_data.return_value = {"battery_level": battery_level}
    ...
    @pytest.mark.asyncio
    async def test_zone_change_event_comprehensive(self, mock_coordinator, hass: HomeAssistant):
        ...
        for old_zone, new_zone, expected_event in zone_transitions:
            ...
            if expected_event in ["pawcontrol_dog_arrived_home", "pawcontrol_dog_left_home"]:
    ...
    async def test_state_restoration_comprehensive(self, hass: HomeAssistant, mock_coordinator):
        ...
        for case_attrs in restoration_cases:
            ...
            if ATTR_LATITUDE in case_attrs and ATTR_LONGITUDE in case_attrs:
                assert tracker._last_known_location == (case_attrs[ATTR_LATITUDE], case_attrs[ATTR_LONGITUDE])
