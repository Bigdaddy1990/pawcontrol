...
from custom_components.pawcontrol.data_manager import (  # noqa: E402
...
async def test_number_set_value_updates_gps_config(  # noqa: D103
    ...
    assert coordinator.refreshed == [dog_id]
    assert (
        coordinator.data[dog_id]["config"][DOG_GPS_CONFIG_FIELD][
            CONF_GPS_UPDATE_INTERVAL
        ]
        == 120
    )
...
async def test_number_update_flows_through_runtime_managers(  # noqa: D103
    ...
    assert (
        data_manager._dogs_config["test_dog"][DOG_FEEDING_CONFIG_FIELD][
            CONF_MEALS_PER_DAY
        ]
        == 4
    )
