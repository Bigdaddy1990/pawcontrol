...
class TestEntityPerformanceScaling:
    ...
    @pytest.mark.parametrize("profile", ["basic", "standard", "advanced", "gps_focus", "health_focus"])
    def test_single_dog_performance(
        self, entity_factory: EntityFactory, profile: str, standard_modules: dict[str, bool]
    ) -> None:
        ...
        for _ in range(10):  # Simulate multiple entity checks
            entity_factory.should_create_entity(profile, "sensor", "feeding", priority=5)
    ...
    def test_multi_dog_scaling_standard_profile(
        self, entity_factory: EntityFactory, dog_count: int, standard_modules: dict[str, bool]
    ) -> None:
        ...
        expected_entities_per_dog = entity_factory.estimate_entity_count("standard", standard_modules)
    ...
    def test_profile_performance_comparison(
        ...
        for profile in ENTITY_PROFILES.keys():
            ...
            for entity_type, module, priority in checks:
                entity_factory.should_create_entity(profile, entity_type, module, priority)
    ...
    def test_memory_efficiency_large_scale(self, entity_factory: EntityFactory) -> None:
        ...
        for config in dog_configs:
            count = entity_factory.estimate_entity_count(config["profile"], config["modules"])
            metrics = entity_factory.get_performance_metrics(config["profile"], config["modules"])
    ...
    def test_edge_case_performance(self, entity_factory: EntityFactory) -> None:
        ...
        edge_cases = [
            ...
            ({module: False for module in ["feeding", "walk", "health", "gps"]}, "advanced"),
            # Maximum modules
            ({
                "feeding": True, "walk": True, "health": True, "gps": True,
                "notifications": True, "dashboard": True, "visitor": True,
                "medication": True, "training": True, "grooming": True,
            }, "advanced"),
        ]
    ...
    def test_platform_priority_performance(
        ...
        platforms = [
            ...
            Platform.DEVICE_TRACKER, Platform.DATE, Platform.DATETIME, Platform.TEXT,
        ]
...
class TestEntityPerformanceBenchmarks:
    ...
    def test_real_world_scenario_small_household(self, entity_factory: EntityFactory) -> None:
        """Benchmark: Small household with 1-2 dogs."""
        dogs = [
            {"id": "buddy", "profile": "standard", "modules": {
                "feeding": True, "walk": True, "health": True, "notifications": True
            }},
            {"id": "bella", "profile": "basic", "modules": {
                "feeding": True, "walk": True, "health": False, "notifications": True
            }},
        ]
        ...
        for dog in dogs:
            ...
            metrics = entity_factory.get_performance_metrics(dog["profile"], dog["modules"])
        ...
        print(f"Small household benchmark: {execution_time:.3f}s, {total_entities} entities")

    def test_real_world_scenario_dog_daycare(self, entity_factory: EntityFactory) -> None:
        ...
        for i in range(12):  # Medium-sized daycare
            ...
            dogs.append({"id": f"daycare_dog_{i:02d}", "profile": profile, "modules": modules})
        ...
        # Check utilization
        avg_utilization = sum(m["utilization_percentage"] for m in performance_metrics) / len(performance_metrics)
        assert 20 <= avg_utilization <= 80  # Reasonable utilization
        
        print(f"Dog daycare benchmark: {execution_time:.3f}s, {total_entities} entities, {avg_utilization:.1f}% avg utilization")

    def test_real_world_scenario_professional_breeder(self, entity_factory: EntityFactory) -> None:
        ...
        for i in range(25):  # Large breeding operation
            ...
            if i < 5:  # Breeding stock - comprehensive monitoring
                ...
                modules = {
                    "feeding": True, "walk": True, "health": True, "gps": True,
                    "medication": True, "grooming": True, "notifications": True,
                }
            elif i < 15:  # Regular adults - standard monitoring
                profile = "standard" 
                modules = {
                    "feeding": True, "walk": True, "health": True,
                    "notifications": True, "grooming": i % 2 == 0,
                }
            else:  # Puppies/young dogs - basic monitoring
                profile = "basic"
                modules = {"feeding": True, "walk": False, "health": True, "notifications": True}
            
            dogs.append({"id": f"breeder_dog_{i:02d}", "profile": profile, "modules": modules})
        ...
        for dog in dogs:
            ...
            profile_breakdown[dog["profile"]] = profile_breakdown.get(dog["profile"], 0) + count
        ...
        print(f"Professional breeder benchmark: {execution_time:.3f}s, {total_entities} entities")
        print(f"Profile breakdown: {profile_breakdown}")

    def test_performance_regression_detection(self, entity_factory: EntityFactory) -> None:
        """Detect performance regressions with standardized test."""
        # Standardized test case
        modules = {
            "feeding": True, "walk": True, "health": True, "gps": True,
            "notifications": True, "dashboard": True,
        }
        ...
        for _ in range(10):
            ...
            # Standard operations
            count = entity_factory.estimate_entity_count("standard", modules)
            metrics = entity_factory.get_performance_metrics("standard", modules)
            
            # Entity decision making
            for priority in [3, 5, 7, 9]:
                entity_factory.should_create_entity("standard", "sensor", "feeding", priority)
        ...
        print(f"Regression test: avg={avg_time*1000:.2f}ms, max={max_time*1000:.2f}ms, min={min_time*1000:.2f}ms")
