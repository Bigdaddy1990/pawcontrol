...
@pytest.mark.parametrize("payload", [{"key": "value"}, None, "invalid"])
...
@pytest.mark.parametrize(
    ("remaining", "expected"),
    [(None, None), (4, 4), (4.9, 4), ("7", 7), ("bad", None), (object(), None)],
)
...
@pytest.mark.parametrize(
    ("remaining", "expected"),
    [(None, False), (3, False), (0, True), (-1, True), ("5", False), ("0", True)],
)
...
def test_create_module_entities_uses_standard_fallback_profile(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _Factory:
        def get_budget(self, _dog_id: str, _profile: str) -> object:
            return object()
    ...
    monkeypatch.setattr(
        ...
        {"walk": {"standard": [("activity", lambda *_: "entity", 1)]}},
    )
...
def test_create_module_entities_stops_when_budget_is_exhausted(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    class _Budget:
        remaining = 0
    ...
    class _Factory:
        ...
        def get_budget(self, _dog_id: str, _profile: str) -> _Budget:
            return _Budget()
    ...
    monkeypatch.setattr(
        sensor,
        "_MODULE_ENTITY_RULES",
        {"health": {"standard": [("metric", lambda *_: "entity", 1)]}},
    )
...
async def test_async_setup_entry_adds_entities_and_awaits_callback(
    ...
    runtime_data = SimpleNamespace(
        coordinator=object(),
        dogs=[{"dog_id": "dog-1", "dog_name": "Luna", "modules": {"walk": True}}],
        entity_factory=_Factory(),
        entity_profile="standard",
    )
    monkeypatch.setattr(sensor, "get_runtime_data", lambda _hass, _entry: runtime_data)
    monkeypatch.setattr(
        sensor, "coerce_dog_modules_config", lambda _dog: {"walk": True}
    )
...
@pytest.mark.asyncio
async def test_async_setup_entry_finalizes_budget_after_module_exception(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    begin_calls: list[tuple[str, str, int]] = []
    finalize_calls: list[tuple[str, str]] = []
    ...
    def _raise(*_args: object) -> list[str]:
        raise RuntimeError("boom")

    monkeypatch.setattr(sensor, "_create_module_entities", _raise)
