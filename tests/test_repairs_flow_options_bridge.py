...
async def test_invalid_geofence_fix_starts_options_flow(hass, monkeypatch):
    ...
    entry = ConfigEntry(version=1, domain=comp.DOMAIN, title="Paw", data={}, source="user", entry_id="r1", options={})
    ...
    monkeypatch.setattr(hass.config_entries.options, "async_init", fake_init, raising=True)
