...
class TestPawControlDataManager:
    ...
    async def test_async_initialize_success(self, data_manager, mock_store):
        ...
        with patch.object(data_manager, '_load_initial_data') as mock_load, \
    ...
    async def test_async_initialize_failure(self, data_manager):
        ...
        with patch.object(data_manager, '_load_initial_data', side_effect=Exception("Load failed")):
    ...
    async def test_async_get_dog_data_found(self, data_manager):
        ...
        with patch.object(data_manager, '_get_cache', return_value={"test_dog": dog_data}):
    ...
    async def test_async_update_dog_data(self, data_manager):
        ...
        with patch.object(data_manager, 'async_get_dog_data', return_value=existing_data), \
    ...
    async def test_async_delete_dog_data(self, data_manager, mock_store):
        ...
        with patch.object(data_manager, '_get_cache', return_value={"test_dog": {}, "other_dog": {}}), \
             patch.object(data_manager, '_set_cache') as mock_set_cache, \
             patch.object(data_manager, '_delete_dog_from_all_modules') as mock_delete_modules:
    ...
    async def test_async_log_feeding(self, data_manager):
        ...
        with patch.object(data_manager, '_append_to_module_data') as mock_append, \
    ...
    async def test_async_log_feeding_error(self, data_manager):
        ...
        with patch.object(data_manager, '_append_to_module_data', side_effect=Exception("Storage error")):
    ...
    async def test_async_get_feeding_history(self, data_manager):
        ...
        with patch.object(data_manager, 'async_get_module_data', return_value=feeding_data) as mock_get:
            result = await data_manager.async_get_feeding_history("test_dog", 7)
            
            assert result == feeding_data
            mock_get.assert_called_once_with("feeding", "test_dog", start_date=pytest.approx(datetime.now() - timedelta(days=7), abs=timedelta(seconds=1)))
    ...
    async def test_async_get_current_walk(self, data_manager):
        ...
        with patch.object(data_manager, 'async_get_dog_data', return_value=dog_data), \
             patch.object(data_manager, 'async_get_module_data', return_value=walks_data):
    ...
    async def test_async_log_gps(self, data_manager):
        ...
        with patch.object(data_manager, '_append_to_module_data') as mock_append, \
             patch.object(data_manager, 'async_update_dog_data') as mock_update, \
             patch.object(data_manager, 'async_get_current_walk', return_value=current_walk), \
    ...
    async def test_async_log_gps_no_current_walk(self, data_manager):
        ...
        with patch.object(data_manager, '_append_to_module_data'), \
    ...
    async def test_async_start_grooming(self, data_manager):
        ...
        with patch.object(data_manager, '_append_to_module_data') as mock_append, \
            ...
            grooming_id = await data_manager.async_start_grooming("test_dog", grooming_data)
    ...
    async def test_async_get_module_data(self, data_manager):
        ...
        with patch.object(data_manager, '_get_cache', return_value=module_data), \
             patch.object(data_manager, '_ensure_cache_fresh'):
            
            # Test with limit
            result = await data_manager.async_get_module_data("feeding", "test_dog", limit=2)
            assert len(result) == 2
            assert result[0]["data"] == "entry1"  # Most recent first
            
            # Test with date filter
            start_date = datetime(2025, 1, 14, 0, 0, 0)
            result = await data_manager.async_get_module_data("feeding", "test_dog", start_date=start_date)
            assert len(result) == 2  # Should exclude entry3
            
            # Test with end date
            end_date = datetime(2025, 1, 14, 12, 0, 0)
            result = await data_manager.async_get_module_data("feeding", "test_dog", end_date=end_date)
    ...
    async def test_async_get_module_data_cache_miss(self, data_manager, mock_store):
        ...
        with patch.object(data_manager, '_get_cache', side_effect=[[], store_data["test_dog"]]), \
             patch.object(data_manager, '_set_cache') as mock_set_cache, \
             patch.object(data_manager, '_ensure_cache_fresh'):
    ...
    async def test_async_reset_all_dogs_stats(self, data_manager):
        ...
        with patch.object(data_manager, 'async_get_all_dogs', return_value=dogs_data), \
    ...
    async def test_async_cleanup_old_data(self, data_manager):
        ...
        with patch.object(data_manager, '_cleanup_module_data', return_value=5) as mock_cleanup:
    ...
    async def test_cleanup_module_data(self, data_manager, mock_store):
        ...
        with patch.object(data_manager, '_set_cache') as mock_set_cache:
            deleted_count = await data_manager._cleanup_module_data("feeding", cutoff_date)
    ...
    async def test_update_module_entry(self, data_manager, mock_store):
        ...
        await data_manager._update_module_entry("feeding", "test_dog", "feed_124", updates)
    ...
    async def test_periodic_cleanup_task(self, data_manager):
        ...
        with patch.object(data_manager, 'async_cleanup_old_data', side_effect=mock_cleanup), \
    ...
    async def test_error_handling_and_metrics(self, data_manager):
        ...
        with patch.object(data_manager, '_append_to_module_data', side_effect=Exception("Test error")):
            with pytest.raises(Exception):
                await data_manager.async_log_feeding("test_dog", {"meal_type": "breakfast"})
    ...
    async def test_concurrent_operations(self, data_manager):
        ...
        for i in range(10):
            task = data_manager.async_log_feeding(f"dog_{i}", {"meal_type": "breakfast"})
            tasks.append(task)
    ...
    async def test_memory_efficient_operations(self, data_manager):
        ...
        with patch.object(data_manager, '_get_cache', return_value=large_data):
            ...
            result = await data_manager.async_get_module_data("feeding", "test_dog", limit=10)
