...
class TestPawControlDashboardGenerator:
    ...
    async def test_create_dog_dashboard_invalid_config(self, dashboard_generator):
        ...
        with pytest.raises(ValueError, match="Dog ID is required"):
            await dashboard_generator.async_create_dog_dashboard({CONF_DOG_NAME: "Test"})
    ...
    async def test_update_nonexistent_dashboard(self, dashboard_generator, sample_dogs_config):
    ...
    async def test_generate_overview_cards(self, dashboard_generator, sample_dogs_config):
        """Test overview cards generation."""
        options = {"title": "Test Dashboard", "show_activity_summary": True}
        
        cards = await dashboard_generator._generate_overview_cards(sample_dogs_config, options)
    ...
    async def test_generate_dog_cards(self, dashboard_generator):
        ...
        cards = await dashboard_generator._generate_dog_cards(dog_config, theme, options)
    ...
    async def test_generate_statistics_cards(self, dashboard_generator, sample_dogs_config):
    ...
    async def test_generate_settings_cards(self, dashboard_generator, sample_dogs_config):
...
class TestDashboardGeneratorErrorHandling:
    ...
    async def test_storage_errors(self, hass: HomeAssistant, mock_config_entry):
        ...
        with patch.object(generator, "_store") as mock_store:
            ...
            with pytest.raises(HomeAssistantError, match="Dashboard storage save failed"):
