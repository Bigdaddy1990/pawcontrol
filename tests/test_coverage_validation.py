...
class TestCoverageValidator:
    ...
    def _analyze_integration_files(self) -> None:
        ...
        print(f"  ✅ Integration Analysis Complete: {total_files} files, {total_items} items")
    ...
    def _analyze_test_files(self) -> None:
        ...
        for test_file in test_files:
            ...
            try:
                ...
                print(f"    Test methods: {len(test_methods)}, Test classes: {len(test_classes)}")
        ...
        print(f"  ✅ Test Analysis Complete: {total_test_files} files, {total_tests} tests")
    ...
    def _calculate_coverage(self) -> None:
        ...
        for integration_file, integration_data in self.integration_files.items():
            ...
            if coverage_data["has_test_file"]:
                ...
                covered_items_set = coverage_data["covered_functions"] | coverage_data["covered_classes"]
                coverage_data["missing_coverage"] = all_items - covered_items_set
            
            self.coverage_report[integration_file] = coverage_data
            
            status = "✅" if coverage_data["coverage_percentage"] >= 95 else "⚠️" if coverage_data["coverage_percentage"] >= 85 else "❌"
            print(f"  {status} {integration_file}: {coverage_data['coverage_percentage']:.1f}% coverage")
    
    def _match_coverage(self, integration_data: Dict, test_data: Dict, integration_file: str) -> int:
    ...
    def _generate_coverage_report(self) -> None:
        ...
        print(f"Overall Coverage: {overall_coverage:.1f}% ({total_covered}/{total_items} items)")
        ...
        if high_coverage:
            ...
            for file_name, percentage in sorted(high_coverage, key=lambda x: x[1], reverse=True):
                print(f"  {file_name}: {percentage:.1f}%")
        
        if medium_coverage:
            print("\n⚠️  MEDIUM COVERAGE (85-94%):")
            for file_name, percentage in sorted(medium_coverage, key=lambda x: x[1], reverse=True):
                print(f"  {file_name}: {percentage:.1f}%")
        
        if low_coverage:
            print("\n❌ LOW COVERAGE (<85%):")
            for file_name, percentage in sorted(low_coverage, key=lambda x: x[1], reverse=True):
                print(f"  {file_name}: {percentage:.1f}%")
    ...
    def _validate_gold_standard(self) -> bool:
        ...
        print(f"Gold Standard Target: 95.0%")
