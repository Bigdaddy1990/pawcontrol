...
async def test_device_trigger_gps_location_posted(hass):
    ...
    from custom_components.pawcontrol.device_trigger import async_get_triggers, async_attach_trigger
    from homeassistant.helpers import device_registry as dr

    # Create a device with identifiers (DOMAIN, dog_id)
    dev_reg = dr.async_get(hass)
    device = dev_reg.async_get_or_create(config_entry_id="e1", identifiers={(comp.DOMAIN, "dog-1")})
    device_id = device.id

    # Build trigger config
    trigger = {"domain": comp.DOMAIN, "type": "gps_location_posted", "device_id": device_id}
    ...
    try:
        ...
        hass.bus.async_fire(f"{comp.DOMAIN}_gps_location_posted", {"device_id": device_id})
...
async def test_device_trigger_geofence_alert(hass):
    ...
    dev_reg = dr.async_get(hass)
    device = dev_reg.async_get_or_create(config_entry_id="e1", identifiers={(comp.DOMAIN, "dog-2")})
    device_id = device.id
    ...
    unsub = await async_attach_trigger(hass, trigger, action, {"platform": "device"})
    try:
        hass.bus.async_fire(f"{comp.DOMAIN}_geofence_alert", {"device_id": device_id, "action": "entered", "zone": "home"})
        await hass.async_block_till_done()
        assert called["ok"]
    finally:
        unsub()
