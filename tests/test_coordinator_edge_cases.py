...
class TestCoordinatorMassiveEntityLoadScenarios:
    ...
    @pytest.mark.asyncio
    async def test_500_entity_load_performance(self, hass, massive_config_entry, mock_managers):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_entity_access_stress(self, hass, massive_config_entry, mock_managers):
        ...
        success_rate = sum(1 for _, _, success in access_results if success) / len(access_results)
        assert success_rate > 0.95  # 95%+ success rate
        
        await coordinator.async_shutdown()

    @pytest.mark.asyncio
    async def test_selective_refresh_optimization_massive_load(self, hass, massive_config_entry, mock_managers):
    ...
    @pytest.mark.asyncio
    async def test_memory_usage_massive_dataset(self, hass, massive_config_entry, mock_managers):
...
class TestCoordinatorManagerDelegationFailures:
    ...
    def coordinator_with_failing_managers(self, hass):
        ...
        failing_feeding.async_get_feeding_data = AsyncMock(side_effect=Exception("Feeding service down"))
        
        timeout_walk = AsyncMock()
        timeout_walk.async_get_walk_data = AsyncMock(side_effect=asyncio.TimeoutError("Walk timeout"))
        timeout_walk.async_get_gps_data = AsyncMock(side_effect=asyncio.TimeoutError("GPS timeout"))
    ...
    @pytest.mark.asyncio
    async def test_feeding_manager_failure_recovery(self, coordinator_with_failing_managers):
    ...
    @pytest.mark.asyncio
    async def test_walk_manager_timeout_handling(self, coordinator_with_failing_managers):
    ...
    async def test_all_managers_failing_scenario(self, hass):
        ...
        for manager_name in ["feeding_manager", "walk_manager", "dog_data_manager"]:
            manager = AsyncMock()
            for method_name in ["async_get_feeding_data", "async_get_walk_data", "async_get_gps_data", "async_get_dog_data"]:
                if hasattr(manager, method_name):
                    getattr(manager, method_name).side_effect = Exception(f"{manager_name} failed")
...
class TestCoordinatorPerformanceStressScenarios:
    ...
    def realistic_managers(self):
        ...
        async def variable_feeding_data(dog_id):
            ...
            return {"last_fed": "2024-09-07 08:00:00", "food_remaining": 1000 + hash(dog_id) % 2000}
    ...
    @pytest.mark.asyncio
    async def test_rapid_update_cycles_stress(self, hass, stress_config_entry, realistic_managers):
    ...
    @pytest.mark.asyncio
    async def test_concurrent_selective_refresh_stress(self, hass, stress_config_entry, realistic_managers):
        ...
        async def concurrent_selective_refresh(refresh_id):
            ...
            try:
                for i in range(5):
                    ...
                    refresh_results.append((refresh_id, len(dog_ids), priority, duration))
        ...
        refreshers = [asyncio.create_task(concurrent_selective_refresh(i)) for i in range(8)]
    ...
    @pytest.mark.asyncio
    async def test_cache_invalidation_stress(self, hass, stress_config_entry, realistic_managers):
...
class TestCoordinatorBackgroundTaskManagement:
    ...
    def task_test_coordinator(self, hass):
        ...
        feeding_manager = AsyncMock()
        feeding_manager.async_get_feeding_data = AsyncMock(return_value={"status": "fed"})
    ...
    @pytest.mark.asyncio
    async def test_graceful_shutdown_with_long_running_tasks(self, task_test_coordinator):
        ...
        async def concurrent_modifier(modifier_id):
            ...
            try:
                for i in range(20):
                    ...
                    if i % 3 == 0:
                        await coordinator.async_request_selective_refresh(["integrity_dog"], priority=5)
                    elif i % 3 == 1:
                        data = coordinator.get_dog_data("integrity_dog")
                        modification_results.append((modifier_id, "read", data is not None))
                    else:
    ...
    for i in range(8):
        if i % 3 == 0:
            ...
            task = asyncio.create_task(
                coordinator.async_request_selective_refresh([f"integration_dog_{i:02d}"], priority=5)
            )
        else:
            task = asyncio.create_task(coordinator.invalidate_dog_cache(f"integration_dog_{i:02d}"))
