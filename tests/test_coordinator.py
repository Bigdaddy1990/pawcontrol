...
class TestRefactoredPawControlCoordinator:
    ...
    @pytest.mark.asyncio
    async def test_fetch_dog_data_delegated_all_modules(self, coordinator, mock_managers):
        ...
        mock_managers["feeding_manager"].async_get_feeding_data.assert_called_once_with("test_dog")
        mock_managers["walk_manager"].async_get_walk_data.assert_called_once_with("test_dog")
        mock_managers["walk_manager"].async_get_gps_data.assert_called_once_with("test_dog")
        mock_managers["dog_data_manager"].async_get_dog_data.assert_called_once_with("test_dog")
    ...
    async def test_async_update_data_with_managers(self, coordinator, mock_managers):
        ...
        mock_managers["feeding_manager"].async_get_feeding_data.assert_called_with("test_dog")
    ...
    async def test_manager_timeout_handling(self, coordinator, mock_managers):
        ...
        mock_managers["walk_manager"].async_get_gps_data.return_value = {"available": False}
        mock_managers["dog_data_manager"].async_get_dog_data.return_value = {"health": {}}
    ...
    async def test_error_handling_in_managers(self, coordinator, mock_managers):
        ...
        mock_managers["feeding_manager"].async_get_feeding_data.side_effect = Exception("Manager error")
    ...
    async def test_multiple_dogs_with_managers(self, coordinator, mock_managers):
        ...
        mock_managers["feeding_manager"].async_get_feeding_data.side_effect = mock_feeding_response
        mock_managers["walk_manager"].async_get_walk_data.side_effect = mock_walk_response
        mock_managers["walk_manager"].async_get_gps_data.return_value = {"available": False}
        mock_managers["dog_data_manager"].async_get_dog_data.return_value = {"health": {}}
    ...
    @pytest.mark.asyncio
    async def test_update_interval_calculation_with_complexity(self, hass: HomeAssistant):
        ...
        entry_complex.data = {
            CONF_DOGS: [
                {CONF_DOG_ID: f"gps_dog_{i}", "modules": {MODULE_GPS: True, MODULE_WALK: True, MODULE_HEALTH: True}}
                for i in range(5)
            ]
        }
        ...
        assert coordinator_complex.update_interval.total_seconds() >= UPDATE_INTERVALS["frequent"]

        # Test with low complexity (feeding only)
        entry_simple = Mock(spec=ConfigEntry)
        entry_simple.data = {
            CONF_DOGS: [
                {CONF_DOG_ID: "simple_dog", "modules": {MODULE_FEEDING: True}}
            ]
        }
        ...
        assert coordinator_simple.update_interval.total_seconds() == UPDATE_INTERVALS["frequent"]
