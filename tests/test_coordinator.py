...
class TestPawControlCoordinator:
    ...
    @pytest.mark.asyncio
    async def test_coordinator_initialization_default_interval(self, hass: HomeAssistant):
        ...
        assert coordinator.update_interval.total_seconds() == DEFAULT_GPS_UPDATE_INTERVAL
    ...
    async def test_is_module_enabled_false(self, coordinator):
        ...
        assert coordinator.is_module_enabled("dog_no_gps", MODULE_HEALTH) is False  # Not in modules
    ...
    async def test_fetch_dog_data_all_modules(self, coordinator):
        ...
        with patch.object(coordinator, '_fetch_feeding_data', return_value={"last_meal": "breakfast"}) as mock_feeding, \
             patch.object(coordinator, '_fetch_walk_data', return_value={"in_progress": False}) as mock_walk, \
             patch.object(coordinator, '_fetch_health_data', return_value={"weight": 25.0}) as mock_health, \
             patch.object(coordinator, '_fetch_gps_data', return_value={"lat": 52.5, "lon": 13.4}) as mock_gps:
    ...
    async def test_fetch_dog_data_selective_modules(self, coordinator):
        ...
        with patch.object(coordinator, '_fetch_feeding_data', return_value={"meals": 3}) as mock_feeding, \
    ...
    async def test_fetch_gps_data_error(self, coordinator):
        ...
        mock_data_manager = AsyncMock()
        mock_data_manager.async_get_current_gps_data.side_effect = GPSUnavailableError("test_dog", "Device offline")
    ...
    async def test_update_interval_calculation(self, hass: HomeAssistant):
        ...
        assert coordinator_single.update_interval.total_seconds() == DEFAULT_GPS_UPDATE_INTERVAL
        ...
        assert coordinator_multi.update_interval.total_seconds() >= DEFAULT_GPS_UPDATE_INTERVAL

    @pytest.mark.asyncio
    async def test_data_caching(self, coordinator):
        """Test data caching functionality."""
        # First call should fetch data
        with patch.object(coordinator, '_fetch_dog_data', return_value={"cached": True}) as mock_fetch:
        ...
        coordinator._cache = {"test_dog": {"cached": True, "timestamp": dt_util.utcnow()}}
        coordinator._cache_ttl = timedelta(minutes=5)
        
        with patch.object(coordinator, '_fetch_dog_data') as mock_fetch_cached:
    ...
    async def test_error_recovery(self, coordinator):
        ...
        with patch.object(coordinator, '_fetch_dog_data', side_effect=mock_fetch_with_errors):
    ...
    def test_get_update_interval_from_config(self, hass: HomeAssistant):
        ...
        coordinator = PawControlCoordinator(hass, entry)
        assert coordinator.update_interval.total_seconds() == UPDATE_INTERVALS["frequent"]
    ...
    async def test_module_data_aggregation(self, coordinator):
        ...
        with patch.object(coordinator, '_fetch_feeding_data', return_value={"meals": 3}), \
             patch.object(coordinator, '_fetch_walk_data', return_value={"walks": 2}), \
             patch.object(coordinator, '_fetch_health_data', return_value={"weight": 25.0}), \
