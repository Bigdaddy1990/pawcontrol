...
class TestRefactoredCoordinatorIntegration:
    ...
    def coordinator_with_managers(self, hass: HomeAssistant, mock_entry):
        ...
        mock_feeding_manager.async_get_feeding_data.return_value = {
            ...
            "feedings_today": {"breakfast": 1, "lunch": 0, "dinner": 1, "snack": 0},
    ...
    @pytest.mark.asyncio
    async def test_coordinator_data_structure_compatibility(
        self, coordinator_with_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_sensor_integration_with_refactored_coordinator(
        self, hass, coordinator_with_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_sensor_attributes_with_refactored_coordinator(
        self, hass, coordinator_with_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_manager_timeout_handling_in_integration(
        self, hass, coordinator_with_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_cache_manager_integration_with_sensors(
        self, hass, coordinator_with_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_performance_monitoring_with_sensor_updates(
        self, hass, coordinator_with_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_batch_manager_integration_with_selective_refresh(
        self, hass, coordinator_with_managers
    ):
    ...
    @pytest.mark.asyncio
    async def test_complete_sensor_platform_compatibility(
        self, hass, coordinator_with_managers
    ):
    ...
    async def test_error_recovery_in_integration(self, hass, coordinator_with_managers):
        ...
        coordinator.feeding_manager.async_get_feeding_data.side_effect = Exception(
            "Manager error"
        )
    ...
    def test_coordinator_public_interface_unchanged(
        self, hass, coordinator_with_managers
    ):
        ...
        is_enabled = coordinator.is_module_enabled(
            "integration_test_dog", MODULE_FEEDING
        )
    ...
    @pytest.mark.asyncio
    async def test_memory_efficiency_with_managers(
        self, hass, coordinator_with_managers
    ):
