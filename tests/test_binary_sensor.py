...
class TestAsyncSetupEntry:
    ...
    async def test_async_setup_entry_with_runtime_data(self, hass: HomeAssistant, mock_config_entry, mock_coordinator):
        ...
        with patch("custom_components.pawcontrol.binary_sensor._async_add_entities_in_batches", new_callable=AsyncMock) as mock_batch_add:
...
class TestPawControlBinarySensorBase:
    ...
    def test_base_sensor_get_dog_data_coordinator_unavailable(self, base_sensor, mock_coordinator):
    ...
    def test_base_sensor_available_coordinator_available_with_dog_data(self, base_sensor, mock_coordinator):
    ...
    def test_base_sensor_available_coordinator_unavailable(self, base_sensor, mock_coordinator):
...
class TestPawControlAttentionNeededBinarySensor:
    ...
    def test_is_on_critically_hungry(self, attention_sensor):
        ...
        with patch.object(attention_sensor, '_get_dog_data', return_value=mock_dog_data):
    ...
    def test_is_on_health_alert(self, attention_sensor):
        ...
        mock_dog_data = {
            ...
            "walk": {},
            "health": {
                "health_alerts": ["high_temperature", "low_activity"]
            },
            "gps": {}
        }
    ...
    def test_calculate_urgency_level_medium(self, attention_sensor):
        ...
        attention_sensor._attention_reasons = ["hungry", "urgent_walk_needed", "outside_safe_zone"]
    ...
    def test_get_recommended_actions(self, attention_sensor):
        ...
        attention_sensor._attention_reasons = ["critically_hungry", "urgent_walk_needed", "health_alert"]
...
class TestPawControlIsHungryBinarySensor:
    ...
    def test_is_on_hungry(self, hungry_sensor):
        ...
        with patch.object(hungry_sensor, '_get_module_data', return_value=mock_feeding_data):
...
class TestPawControlFeedingDueBinarySensor:
    ...
    def test_is_on_feeding_due(self, feeding_due_sensor):
        ...
        with patch.object(feeding_due_sensor, '_get_module_data', return_value=mock_feeding_data):
...
class TestBinarySensorIntegrationScenarios:
    ...
    def test_multiple_dogs_unique_entities(self, mock_coordinator):
        ...
        for dog_id, dog_name in dogs:
            sensors.append(PawControlOnlineBinarySensor(mock_coordinator, dog_id, dog_name))
            sensors.append(PawControlIsHungryBinarySensor(mock_coordinator, dog_id, dog_name))
    ...
    def test_sensor_state_consistency_across_modules(self, mock_coordinator):
        ...
        walk_progress_sensor = PawControlWalkInProgressBinarySensor(mock_coordinator, "dog1", "Buddy")
        needs_walk_sensor = PawControlNeedsWalkBinarySensor(mock_coordinator, "dog1", "Buddy") 
    ...
    def test_error_handling_malformed_data(self, mock_coordinator):
        ...
        try:
            ...
            pytest.fail(f"Sensor should handle malformed data gracefully, but raised: {e}")
    ...
    async def test_performance_with_many_sensors(self, mock_coordinator):
        ...
        for dog_num in range(10):
            ...
            sensors.extend([
                ...
                PawControlAttentionNeededBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlVisitorModeBinarySensor(mock_coordinator, dog_id, dog_name),
                
                # Feeding sensors (4)
                PawControlIsHungryBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlFeedingDueBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlFeedingScheduleOnTrackBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlDailyFeedingGoalMetBinarySensor(mock_coordinator, dog_id, dog_name),
                
                # Walk sensors (4) 
                PawControlWalkInProgressBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlNeedsWalkBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlWalkGoalMetBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlLongWalkOverdueBinarySensor(mock_coordinator, dog_id, dog_name),
                
                # GPS sensors (6)
                PawControlIsHomeBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlInSafeZoneBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlGPSAccuratelyTrackedBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlMovingBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlGeofenceAlertBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlGPSBatteryLowBinarySensor(mock_coordinator, dog_id, dog_name),
                
                # Health sensors (6)
                PawControlHealthAlertBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlWeightAlertBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlMedicationDueBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlVetCheckupDueBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlGroomingDueBinarySensor(mock_coordinator, dog_id, dog_name),
                PawControlActivityLevelConcernBinarySensor(mock_coordinator, dog_id, dog_name),
            ])
