...
class TestValidationFunctions:
    ...
    def test_validate_dog_id_invalid(self):
        ...
        invalid_cases = [
            ...
            ("dog with spaces", "Dog ID can only contain letters, numbers, and underscores"),
            ("dog-with-dashes", "Dog ID can only contain letters, numbers, and underscores"),
...
class TestFormattingFunctions:
    ...
    def test_format_distance_adaptive_metric(self):
        ...
        for meters, expected in test_cases:
            ...
            assert result == expected, f"Expected {expected}, got {result} for {meters}m"
...
class TestUtilityFunctions:
    ...
    def test_safe_convert_bool(self):
        ...
        assert safe_convert("true", bool, False) == True
        assert safe_convert("false", bool, True) == False
        assert safe_convert("yes", bool, False) == True
        assert safe_convert("1", bool, False) == True
        assert safe_convert("invalid", bool, True) == True
    ...
    def test_is_within_time_range_enhanced(self):
        ...
        is_within, error = is_within_time_range_enhanced(current_time, "invalid", "12:00")
        assert not is_within
        assert error is not None
    ...
    async def test_async_batch_validate(self):
        ...
        def simple_validator(value):
            return (isinstance(value, int), "Must be integer" if not isinstance(value, int) else None)
        ...
        assert results[0][0] == True  # Valid
        assert results[1][0] == True  # Valid
        assert results[2][0] == False  # Invalid
        assert results[3][0] == True  # Valid
