...
async def test_safe_service_call_success():
    ...
    ok = await safe_service_call(hass, "notify", "persistent_notification", {"message": "hi"}, blocking=True)
    assert ok is True
...
async def test_safe_service_call_failure_returns_false():
    ...
    ok = await safe_service_call(hass, "light", "turn_on", {"entity_id": "light.test"}, blocking=False)
    assert ok is False
