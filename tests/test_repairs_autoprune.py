...
async def _make_device(hass, entry, dog_id):
    ...
    return dev_reg.async_get_or_create(config_entry_id=entry.entry_id, identifiers={(DOMAIN, dog_id)})
...
async def test_stale_devices_issue_when_auto_false(hass, monkeypatch):
    ...
    entry = ConfigEntry(version=1, domain=comp.DOMAIN, title="Paw", data={}, source="user", entry_id="e1", options={"auto_prune_devices": False})
    await comp.async_setup_entry(hass, entry)

    # Create a stale device (dog-x not in coordinator data)
    dev = await _make_device(hass, entry, "dog-x")
    ...
    issues = [i for i in registry.issues.values() if i.domain == comp.DOMAIN and i.issue_id == "stale_devices"]
...
async def test_stale_devices_auto_prunes(hass, monkeypatch):
    ...
    entry = ConfigEntry(version=1, domain=comp.DOMAIN, title="Paw", data={}, source="user", entry_id="e2", options={"auto_prune_devices": True})
    await comp.async_setup_entry(hass, entry)
...
async def test_prune_service_removes(hass):
    ...
    entry = ConfigEntry(version=1, domain=comp.DOMAIN, title="Paw", data={}, source="user", entry_id="e3", options={})
    await comp.async_setup_entry(hass, entry)

    dev_reg = dr.async_get(hass)
    dev = dev_reg.async_get_or_create(config_entry_id=entry.entry_id, identifiers={(comp.DOMAIN, "dog-z")})

    # call service
    await hass.services.async_call(comp.DOMAIN, "prune_stale_devices", {"auto": True}, blocking=True)
