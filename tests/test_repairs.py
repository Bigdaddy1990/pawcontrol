...
def test_notification_delivery_errors_clears_issues_when_clean(
    ...
    assert any(str(name).endswith("notification_auth_error")
               for name in deleted)
    assert any(str(name).endswith("notification_device_unreachable")
               for name in deleted)
    assert any(str(name).endswith("notification_missing_service")
               for name in deleted)
    ...
    assert any(str(name).endswith("notification_delivery_repeated")
               for name in deleted)
...
def test_async_publish_feeding_compliance_issue_creates_alert(
    ...
    assert data["issue_summary"] in (
        ["issue_item"], ["2024-05-04: Missed breakfast"])
...
def test_async_publish_feeding_compliance_issue_handles_no_data(
    ...
    summary = data["localized_summary"]
    assert summary["title"] in {
        "no_data_title", "🍽️ Feeding telemetry missing"}
