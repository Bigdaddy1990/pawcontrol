...
class TestComplexMultiDietScenarios:
    ...
    async def test_senior_diabetic_dog_with_multiple_conditions(self) -> None:
        ...
        dogs = [{
            ...
            "feeding_config": {
                ...
                "special_diet": [
                    "senior_formula", "diabetic", "kidney_support", 
                    "low_fat", "joint_support", "prescription"
                ],
        ...
        assert calculated_portion < base_portion * 0.8, "Should be reduced for multiple conditions"
        assert calculated_portion > 50, "Should not be too restrictive for nutritional needs"
        
        # Should flag safety concerns
        safety = result["safety_validation"]
        assert len(safety["warnings"]) > 0 or not safety["safe"], "Should have safety warnings"
    ...
    async def test_young_puppy_conflicting_diets_growth_needs(self) -> None:
        ...
        dogs = [{
            ...
            "feeding_config": {
                ...
                "special_diet": ["puppy_formula", "weight_control", "sensitive_stomach"],
                "health_aware_portions": True,
                "dog_weight": 12.0,
        ...
        assert calculated_portion >= base_portion * 0.85, "Should support growth despite weight goal"
    ...
    async def test_raw_diet_with_medical_conditions_complex_warnings(self) -> None:
        ...
        dogs = [{
            ...
            "feeding_config": {
                ...
                "special_diet": [
                    "raw_diet", "prescription", "diabetic", 
                    "kidney_support", "hypoallergenic", "organic"
                ],
                "health_aware_portions": True,
                "dog_weight": 28.0,
        ...
        assert calculated_portion < base_portion * 0.9, "Medical warnings should reduce portions"
        ...
        safety = result["safety_validation"]
        recommendations = " ".join(safety["recommendations"])
        assert "monitoring" in recommendations.lower() or "veterinary" in recommendations.lower()
    ...
    async def test_giant_breed_puppy_multiple_support_diets(self) -> None:
        ...
        assert calculated_portion >= base_portion * 0.95, "Should support giant breed growth"
    ...
    async def test_senior_with_all_age_related_conditions(self) -> None:
        ...
        dogs = [{
            ...
            "feeding_config": {
                ...
                "special_diet": [
                    "senior_formula", "prescription", "kidney_support",
                    "low_fat", "joint_support", "dental_care", "sensitive_stomach"
                ],
                "health_aware_portions": True,
                "dog_weight": 8.0,
                ...
                "health_conditions": [
                    "kidney_disease", "arthritis", "dental_problems", 
                    "digestive_issues", "heart_disease"
                ],
    ...
    async def test_extreme_diet_complexity_boundary_testing(self) -> None:
        ...
        dogs = [{
            ...
            "feeding_config": {
                ...
                "special_diet": [
                    "puppy_formula", "senior_formula",  # Conflicting
                    "prescription", "diabetic", "kidney_support",  # Multiple prescriptions
                    "raw_diet", "hypoallergenic", "organic",  # Conflicting approaches
                    "weight_control", "joint_support"  # Additional complexity
                ],
                "health_aware_portions": True,
                "dog_weight": 25.0,
                "ideal_weight": 22.0,
    ...
    async def test_diet_validation_adjustment_factor_calculations(self) -> None:
        ...
        for scenario in test_scenarios:
            ...
            calculated_adjustment = HealthCalculator.calculate_diet_validation_adjustment(
                scenario["diet_validation"] or {}, 
                health_metrics.special_diet
            )
            
            # Verify within reasonable bounds and close to expected
            assert 0.8 <= calculated_adjustment <= 1.1, f"Adjustment out of bounds for {scenario['name']}"
            
            if scenario["expected_adjustment"] != 1.0:
                # Allow some variance for complex calculations
                assert abs(calculated_adjustment - scenario["expected_adjustment"]) < 0.1, \
    ...
    async def test_portion_safety_validation_complex_scenarios(self) -> None:
        ...
        for scenario in safety_scenarios:
            ...
            assert abs(safety_result["portion_per_kg"] - expected_portion_per_kg) < 0.1, \
    ...
    async def test_health_aware_feeding_integration_end_to_end(self) -> None:
        ...
        assert validation_result["portion"] == breakfast_portion, "Portions should match"
        ...
        assert feeding_event.amount == breakfast_portion, "Feeding amount should match calculated portion"
        assert feeding_event.portion_size == breakfast_portion, "Portion size should be calculated"
        
        # 4. Get feeding analysis
        feeding_analysis = await manager.async_analyze_feeding_health(
            "integration_test", 7
        )
        assert feeding_analysis["status"] != "insufficient_data", "Should have feeding data"
        ...
        update_success = await manager.async_update_health_data(
            "integration_test", {"weight": 24.0}  # Weight loss progress
        )
    ...
    async def test_diet_compatibility_matrix_validation(self) -> None:
        ...
        for test_case in compatibility_tests:
            validation_result = validation_logic._validate_diet_combinations(test_case["diets"])
            ...
            if "should_recommend_vet" in test_case:
                assert validation_result["recommended_vet_consultation"] == test_case["should_recommend_vet"], \
