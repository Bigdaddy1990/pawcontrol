...
class TestDogDataManagerDataAccess:
    ...
    async def test_async_update_dog_data_existing_dog(self, initialized_manager):
        ...
        result = await initialized_manager.async_update_dog_data(
            "dog1", "feeding", update_data
        )
    ...
    async def test_async_update_dog_data_nonexistent_dog(self, initialized_manager):
        ...
        result = await initialized_manager.async_update_dog_data(
            "nonexistent", "feeding", update_data
        )
...
class TestDogDataManagerValidation:
    ...
    async def test_validate_dog_data_valid(self, initialized_manager, valid_dog_data):
        ...
        result = await initialized_manager.async_validate_dog_data(
            "dog1", valid_dog_data
        )
    ...
    async def test_validate_dog_data_missing_required_fields(self, initialized_manager):
        ...
        assert any(
            "Missing required field: dog_info" in error for error in result["errors"]
        )
    ...
    async def test_validate_dog_data_invalid_dog_info(self, initialized_manager):
        ...
        assert result["valid"] is False
        assert any(
            "dog_info must be a dictionary" in error for error in result["errors"]
        )
    ...
    async def test_validate_dog_data_missing_dog_info_fields(self, initialized_manager):
        ...
        assert result["valid"] is False
        assert any(
            f"Missing required dog_info field: {CONF_DOG_NAME}" in error
    ...
    async def test_validate_dog_data_empty_dog_info_fields(self, initialized_manager):
        ...
        assert result["valid"] is False
        assert any(
            f"Empty required dog_info field: {CONF_DOG_ID}" in error
            for error in result["errors"]
        )

    async def test_validate_dog_data_nonexistent_dog(
        self, initialized_manager, valid_dog_data
    ):
        """Test validation for non-existent dog."""
        result = await initialized_manager.async_validate_dog_data(
            "nonexistent", valid_dog_data
        )

        assert result["valid"] is False
        assert any("Dog configuration not found" in error for error in result["errors"])

    async def test_validate_dog_data_unexpected_fields(
        self, initialized_manager, valid_dog_data
    ):
        ...
        assert result["valid"] is True
        assert any(
            "Unexpected data field: unexpected_field" in warning
            for warning in result["warnings"]
        )

    async def test_validate_dog_data_invalid_timestamps(
        self, initialized_manager, valid_dog_data
    ):
        ...
        assert result["valid"] is False
        assert any(
            "Invalid timestamp format in created_at" in error
            for error in result["errors"]
        )
    ...
    async def test_validate_dog_data_caching(self, initialized_manager, valid_dog_data):
        ...
        # First validation
        result1 = await initialized_manager.async_validate_dog_data(
            "dog1", valid_dog_data
        )

        # Second validation with same data should use cache
        result2 = await initialized_manager.async_validate_dog_data(
            "dog1", valid_dog_data
        )
    ...
    async def test_validate_module_data_feeding(self, initialized_manager):
        ...
        errors = await initialized_manager._validate_module_data(
    ...
    async def test_validate_module_data_health(self, initialized_manager):
        ...
        assert len(errors) >= 1
        assert any(
            "current_weight must be between 0 and 150 kg" in error for error in errors
        )
    ...
    async def test_validate_module_data_gps(self, initialized_manager):
        ...
        assert any(
            "longitude must be between -180 and 180" in error for error in errors
        )
    ...
    def test_is_valid_timestamp(self, initialized_manager):
        ...
        assert (
            initialized_manager._is_valid_timestamp("2024-01-01T12:00:00+02:00") is True
        )
...
class TestDogDataManagerStatisticsAndCleanup:
    ...
    async def test_async_get_data_statistics_with_updates(self, initialized_manager):
        ...
        assert (
            stats["module_data_counts"]["feeding"] >= 2
        )  # Should count the data points
    ...
    async def test_async_get_data_statistics_recent_updates(self, initialized_manager):
        ...
        await initialized_manager.async_update_dog_data(
            "dog1", "feeding", {"test": "data"}
        )
    ...
    async def test_async_get_data_statistics_last_updated_times(
...
class TestDogDataManagerEdgeCases:
    ...
    async def test_operations_on_uninitialized_manager(self, manager):
        ...
        result = await manager.async_update_dog_data(
            "dog1", "feeding", {"test": "data"}
        )
        assert result is False
    ...
    async def test_empty_module_updates(self, manager):
        ...
        config = [
            {CONF_DOG_ID: "dog1", CONF_DOG_NAME: "Test", "modules": {"feeding": True}}
        ]
