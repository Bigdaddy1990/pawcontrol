...
class TestHighLoadScenarios:
    ...
    def test_percentile_calculation_performance(self):
        ...
        assert percentiles[-3] <= percentiles[-2] <= percentiles[-1]  # p50 <= p95 <= p99
...
class TestAlertingUnderStress:
    ...
    def test_alert_rate_limiting_timing(self):
        ...
        with patch('custom_components.pawcontrol.performance_manager._LOGGER') as mock_logger:
            with patch('custom_components.pawcontrol.performance_manager.dt_util') as mock_dt:
                # Control time progression
                base_time = datetime(2025, 1, 1, 12, 0, 0)
                time_progression = [
                    base_time,
                    base_time + timedelta(minutes=5),   # 5 min later
                    base_time + timedelta(minutes=11),  # 11 min later (should allow new alert)
                ]
...
class TestHealthScoringStress:
    ...
    def test_health_recommendations_accuracy(self):
        ...
        test_scenarios = [
            ...
            {"updates": [(0.1, 0)] * 25 + [(10.0, 0)] * 25, "should_recommend": "variance"},
        ]
    ...
    def test_concurrent_record_update(self):
        ...
        for thread_id in range(thread_count):
            thread = threading.Thread(
                ...
                args=(thread_id, updates_per_thread)
            )
...
class TestRealisticsScenarios:
    ...
    def test_realistic_50_dog_production_load(self):
        ...
        for cycle in range(simulation_cycles):
            ...
            total_time = (base_update_time + entity_processing_time + gc_overhead + network_delay) * system_load_factor
        ...
        assert stats["min_update_time"] >= expected_base_time * 0.5  # Within reasonable bounds
