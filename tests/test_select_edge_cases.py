...
class TestSelectBaseEdgeCases:
    ...
    async def test_invalid_option_selection(self, base_select):
        ...
        with pytest.raises(
            HomeAssistantError, match="Invalid option 'invalid_option' for test_select"
        ):
    ...
    async def test_empty_string_option_selection(self, base_select):
        ...
        with pytest.raises(
            HomeAssistantError, match="Invalid option '' for test_select"
        ):
    ...
    async def test_none_option_selection(self, base_select):
        ...
        with pytest.raises(
            HomeAssistantError, match="Invalid option 'None' for test_select"
        ):
    ...
    async def test_case_sensitive_option_validation(self, base_select):
        ...
        with pytest.raises(
            HomeAssistantError, match="Invalid option 'OPTION1' for test_select"
        ):
    ...
    async def test_option_selection_with_service_failure(self, base_select):
        ...
        with (
            ...
            pytest.raises(HomeAssistantError, match="Failed to set test_select"),
        ):
...
class TestHealthSelectDataIntegration:
    ...
    def test_health_status_select_missing_status_in_data(self, mock_coordinator):
        ...
        mock_coordinator.get_module_data.return_value = {"other_data": "value"}
