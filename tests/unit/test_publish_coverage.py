...
def test_publish_coverage_degrades_without_network(tmp_path, monkeypatch) -> None:
    ...
    (html_root /
     "style.css").write_text("body {color: #333;}", encoding="utf-8")
...
def test_github_pages_publisher_prunes_expired_runs(monkeypatch) -> None:
    ...
    old_summary = {
        ...
        "content": base64.b64encode(
            json.dumps({"generated_at": old_timestamp.isoformat()}
                       ).encode("utf-8")
        ).decode("ascii"),
    }
    recent_summary = {
        "encoding": "base64",
        "content": base64.b64encode(
            json.dumps({"generated_at": recent_timestamp.isoformat()}
                       ).encode("utf-8")
        ).decode("ascii"),
    }
    ...
    removed = publisher.prune_expired_runs(
        "coverage", dt.timedelta(days=30), now=now)

    assert removed == ["coverage/run-old"]
    tree_payload = captured_payloads["https://api.github.com/repos/owner/repo/git/trees"]
...
def test_publish_uses_custom_prune_max_age(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        publish_coverage, "GitHubPagesPublisher", _DummyPublisher)
