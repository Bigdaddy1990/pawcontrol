    ...
    def __init__(
        ...
        *,
        ...
        if context is None:
            ...
            self._context_override = cast(
                PersonNotificationContext, dict(context))
...
class TestQuietHours:
    ...
    async def test_quiet_hours_cache_ttl_and_recompute(
        ...
        with pytest.MonkeyPatch.context() as mp:
            ...
            is_cached, cached_value = manager._cache.is_quiet_time_cached(
                quiet_key)
...
class TestChannelDelivery:
    ...
    async def test_send_to_persistent_channel(self, mock_notification_manager, mock_hass):
        ...
        assert any(mock_call[0][0] ==
                   "persistent_notification" for mock_call in calls)
...
class TestBatchProcessing:
    ...
    async def test_background_batch_flushes_pending_notifications(
        ...
        process_task = asyncio.create_task(
            manager._process_batch_notifications())
...
class TestPersonTargeting:
    ...
    async def test_person_targeting_falls_back_to_static_services(
        ...
        assert stub_person_manager.requests == [
            (False, "person_targets_doggo_False")]
