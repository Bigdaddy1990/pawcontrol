...
class TestRateLimiting:
    ...
    async def test_rate_limit_blocks_excessive_notifications(self, mock_hass):
        ...
        id1 = await manager.async_send_notification(
            notification_type=NotificationType.SYSTEM_INFO,
            title="First",
            message="First",
            dog_id="test_dog",
        ...
        notification2 = manager._notifications.get(id2)
...
class TestNotificationAcknowledgment:
    ...
    async def test_acknowledge_nonexistent_notification(self, mock_notification_manager):
    ...
    async def test_acknowledged_notifications_excluded_from_batch(
        ...
        id2 = await mock_notification_manager.async_send_notification(
            notification_type=NotificationType.FEEDING_REMINDER,
            title="Test 2",
            message="Test 2",
            dog_id="test_dog",
...
class TestNotificationCleanup:
    ...
    async def test_old_acknowledged_notifications_cleaned(self, mock_notification_manager):
...
class TestPerformanceStatistics:
    ...
    async def test_statistics_track_failed_notifications(self, mock_notification_manager, mock_hass):
...
class TestEdgeCases:
    ...
    async def test_send_notification_with_very_long_message(self, mock_notification_manager):
    ...
    async def test_shutdown_cancels_background_tasks(self, mock_notification_manager):
        ...
        assert mock_notification_manager._retry_task is None or \
               mock_notification_manager._retry_task.done()
