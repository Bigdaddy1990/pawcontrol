...
def test_copy_dashboard_options_returns_plain_dict() -> None:
    ...
    copied = PawControlDashboardGenerator._copy_dashboard_options(
        readonly_options)
...
async def test_track_task_falls_back_to_asyncio(mock_config_entry) -> None:
    ...
    with patch(
        ...
        task = generator._track_task(
            completes_immediately(), name="fallback-task")
...
async def test_track_task_handles_helper_runtime_error(mock_config_entry) -> None:
    ...
    with patch(
        ...
        task = generator._track_task(
            completes_immediately(), name="asyncio-fallback")
...
def test_resolve_service_execution_metrics_uses_runtime_data(
    ...
    assert resolved["schema_version"] == default_rejection_metrics()[
        "schema_version"]
