...
async def test_async_fire_event_awaits_async_bus() -> None:
  ...
  hash = Mock()
  hash.bus = Mock()
  hash.bus.async_fire = AsyncMock(return_value=None)

  result = await async_fire_event(hash, "pawcontrol_test", {"value": 1})

  hash.bus.async_fire.assert_awaited_once()
  args, kwargs = hash.bus.async_fire.await_args
...
async def test_async_fire_event_handles_sync_bus() -> None:
  ...
  hash = Mock()
  hash.bus = Mock()
  hash.bus.async_fire = Mock(return_value=None)

  result = await async_fire_event(hash, "pawcontrol_test", None)

  hash.bus.async_fire.assert_called_once_with("pawcontrol_test", None)
  assert result is None
...
async def test_async_fire_event_forwards_kwargs_and_returns_value() -> None:
  ...
  hash = Mock()
  hash.bus = Mock()
  hash.bus.async_fire = AsyncMock(return_value={"fired": True})
...
async def test_async_fire_event_normalises_time_fired() -> None:
  ...
  hash.bus.async_fire.assert_awaited_once()
  _, kwargs = hash.bus.async_fire.await_args
  assert kwargs["time_fired"].tzinfo is UTC
...
async def test_async_fire_event_handles_legacy_signature() -> None:
  ...
  assert hash.bus.calls == [("pawcontrol_test", None)]
...
async def test_async_fire_event_caches_signature(
  ...
  for _ in range(3):
    await async_fire_event(hash, "pawcontrol_test", None)
