...
async def test_dog_status_template_accepts_typed_modules(
    ...
    assert any(
        entity == "sensor.alpha_last_fed" for entity in card["entities"])
    assert any(
...
async def test_statistics_generator_normalises_raw_dog_configs(
    ...
    monkeypatch.setattr(
        templates, "get_statistics_graph_template", _fake_graph)
    monkeypatch.setattr(
        templates, "get_statistics_summary_template", _fake_summary)
...
def test_statistics_summary_template_includes_rejection_metrics(
    ...
    service_iso = datetime.fromtimestamp(
        last_rejection - 3600, UTC).isoformat()
...
async def test_generate_walk_cards_localizes_german(
    ...
    monkeypatch.setattr(generator.templates,
                        "get_history_graph_template", _fake_history)
