...
def _parse_localization_table(
    ...
    if start_index is None or end_index is None or end_index <= start_index:
        raise AssertionError(
            "Localization table markers missing from docs/diagnostics.md")
    ...
    if header is None:
        raise AssertionError(
            "Localization table missing from docs/diagnostics.md")
    ...
    for cell in header[1:]:
        ...
        if match is None:
            raise AssertionError(
                f"Missing language code in header cell '{cell}'")
...
def test_documentation_lists_all_setup_flag_translations() -> None:
    ...
    assert languages == translation_languages, (
