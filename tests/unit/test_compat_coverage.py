...
@pytest.mark.unit
def test_ensure_config_entry_symbols_no_raise() -> None:
    ensure_homeassistant_config_entry_symbols()  # must not raise


@pytest.mark.unit
def test_ensure_exception_symbols_no_raise() -> None:
    ensure_homeassistant_exception_symbols()


@pytest.mark.unit
def test_ensure_symbols_idempotent() -> None:
    ensure_homeassistant_config_entry_symbols()
    ensure_homeassistant_config_entry_symbols()  # safe to call twice


@pytest.mark.unit
def test_ensure_exception_symbols_idempotent() -> None:
    ensure_homeassistant_exception_symbols()
    ensure_homeassistant_exception_symbols()
