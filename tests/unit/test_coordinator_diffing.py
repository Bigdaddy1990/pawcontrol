    ...
    def test_coordinator_data_diff_has_changes(self) -> None:
        ...
        diff = CoordinatorDataDiff(
            dog_diffs={
                "buddy": DogDataDiff(
                    "buddy",
                    module_diffs={"gps": DataDiff(
                        modified_keys=frozenset({"lat"}))},
                )
            }
        )
        assert diff.has_changes is True
...
class TestShouldNotifyEntities:
    ...
    def test_should_notify_entities_specific_module(self) -> None:
        ...
        assert should_notify_entities(
            diff, dog_id="buddy", module="gps") is True
        assert should_notify_entities(
            diff, dog_id="buddy", module="walk") is False
...
class TestEdgeCases:
    ...
    def test_should_notify_entities_added_dog_with_module_filter(self) -> None:
        ...
        assert should_notify_entities(
            diff, dog_id="new_dog", module="gps") is True
