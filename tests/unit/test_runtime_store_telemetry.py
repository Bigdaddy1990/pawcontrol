...
def test_update_runtime_store_health_records_counts(monkeypatch: MonkeyPatch) -> None:
    ...
    assert timeline_summary["last_level_duration_seconds"] == pytest.approx(
        0.0)
    assert timeline_summary["level_duration_peaks"]["ok"] == pytest.approx(0.0)
    assert timeline_summary["level_duration_peaks"]["watch"] == pytest.approx(
        0.0)
    assert timeline_summary["level_duration_peaks"]["action_required"] == pytest.approx(
        0.0
    )
    assert timeline_summary["level_duration_latest"]["ok"] == pytest.approx(
        0.0)
    ...
    assert timeline_summary["level_duration_totals"]["ok"] == pytest.approx(
        0.0)
    assert timeline_summary["level_duration_totals"]["watch"] == pytest.approx(
        0.0)
    assert timeline_summary["level_duration_totals"]["action_required"] == pytest.approx(
        0.0
    )
    assert timeline_summary["level_duration_minimums"]["ok"] == pytest.approx(
        0.0)
    assert timeline_summary["level_duration_minimums"]["watch"] is None
    assert timeline_summary["level_duration_minimums"]["action_required"] is None
    ...
    assert timeline_summary["level_duration_averages"]["ok"] == pytest.approx(
        0.0)
    ...
    assert timeline_summary["level_duration_medians"]["ok"] == pytest.approx(
        0.0)
    ...
    assert timeline_summary["level_duration_standard_deviations"]["ok"] == pytest.approx(
        0.0
    )
    assert timeline_summary["level_duration_standard_deviations"]["watch"] is None
    assert (
    ...
    assert timeline_summary["level_duration_alert_thresholds"]["ok"] == pytest.approx(
        0.0)
...
def test_update_runtime_store_health_does_not_increment_when_suppressed(
    ...
    assert timeline_summary["timeline_window_seconds"] == pytest.approx(
        86400.0)
    ...
    assert timeline_summary["last_level_duration_seconds"] == pytest.approx(
        0.0)
    assert timeline_summary["level_duration_latest"]["watch"] == pytest.approx(
        0.0)
...
def test_update_runtime_store_health_records_first_event_when_suppressed(
    ...
    assert timeline_summary["level_duration_minimums"][
        "action_required"
    ] == pytest.approx(0.0)
    ...
    assert timeline_summary["level_duration_samples"]["watch"] == 0
    assert timeline_summary["level_duration_averages"][
        "action_required"
    ] == pytest.approx(0.0)
    ...
    assert timeline_summary["level_duration_medians"]["watch"] is None
    assert timeline_summary["level_duration_standard_deviations"][
        "action_required"
    ] == pytest.approx(0.0)
    assert timeline_summary["level_duration_standard_deviations"]["ok"] is None
    assert timeline_summary["level_duration_standard_deviations"]["watch"] is None
    ...
    assert timeline_summary["level_duration_percentiles"]["watch"] == {}
    assert timeline_summary["level_duration_alert_thresholds"][
        "action_required"
    ] == pytest.approx(0.0)
...
def test_update_runtime_store_health_escalates_on_persistent_divergence(
    ...
    assert assessment["current_level_duration_seconds"] == pytest.approx(
        86400.0)
    events = assessment["events"]
    assert isinstance(events, list)
    assert len(events) >= 2
...
def test_runtime_store_assessment_tracks_trends(monkeypatch: MonkeyPatch) -> None:
    ...
    first_assessment = cast(RuntimeStoreHealthAssessment,
                            first_history["assessment"])
    ...
    assert first_assessment["current_level_duration_seconds"] == pytest.approx(
        0.0)
    ...
    second_assessment = cast(RuntimeStoreHealthAssessment,
                             second_history["assessment"])
    ...
    assert second_assessment["current_level_duration_seconds"] == pytest.approx(
        0.0)
    ...
    third_assessment = cast(RuntimeStoreHealthAssessment,
                            third_history["assessment"])
    ...
    assert third_assessment["current_level_duration_seconds"] == pytest.approx(
        0.0)
    ...
    fourth_assessment = cast(RuntimeStoreHealthAssessment,
                             fourth_history["assessment"])
    ...
    assert fourth_assessment["current_level_duration_seconds"] == pytest.approx(
        0.0)
...
def test_runtime_store_assessment_event_log_capped(monkeypatch: MonkeyPatch) -> None:
    ...
    assert events[-1]["timestamp"] == (base +
                                       timedelta(days=limit + 4)).isoformat()
    segments = history.get("assessment_timeline_segments")
    assert isinstance(segments, list)
...
def test_summarise_runtime_store_events_backfills_legacy_durations() -> None:
    ...
    assert summary["level_duration_standard_deviations"]["ok"] == pytest.approx(
        0.0)
    ...
    assert summary["level_duration_percentiles"]["ok"]["p75"] == pytest.approx(
        21600.0)
    assert summary["level_duration_percentiles"]["ok"]["p90"] == pytest.approx(
        21600.0)
    assert summary["level_duration_percentiles"]["ok"]["p95"] == pytest.approx(
        21600.0)
    assert summary["level_duration_percentiles"]["watch"] == {}
    assert summary["level_duration_alert_thresholds"]["ok"] == pytest.approx(
        21600.0)
...
def test_summarise_runtime_store_events_includes_percentiles() -> None:
    ...
    assert summary["level_duration_alert_thresholds"]["ok"] == pytest.approx(
        39.0)
    assert summary["level_duration_alert_thresholds"]["watch"] == pytest.approx(
        20.0)
