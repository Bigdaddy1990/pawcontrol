...
from custom_components.pawcontrol.types import DoorSensorOverrideScalar
from custom_components.pawcontrol.types import DoorSensorSettingsInput
...
async def test_update_settings_without_entity_change(
    ...
    monkeypatch.setattr(manager, "_validate_sensor_entity",
...
async def test_update_without_changes_does_not_restart(
    ...
    manager._stop_sensor_monitoring.assert_not_called()
    # type: ignore[attr-defined]
    manager._start_sensor_monitoring.assert_not_called()
...
async def test_initialize_persists_trimmed_payload(
    ...
    persisted_settings = cast(
        DoorSensorSettingsPayload, persisted_settings_raw)
