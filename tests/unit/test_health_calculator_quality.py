...
class TestLifeStageCalculation:
    ...
    def test_life_stage_thresholds(self) -> None:
        ...
        assert HealthCalculator.calculate_life_stage(
            10, "medium") == LifeStage.PUPPY
        assert HealthCalculator.calculate_life_stage(
            20, "medium") == LifeStage.YOUNG_ADULT
        assert HealthCalculator.calculate_life_stage(
            70, "large") == LifeStage.ADULT
        assert HealthCalculator.calculate_life_stage(
            90, "medium") == LifeStage.SENIOR
        assert HealthCalculator.calculate_life_stage(
            130, "medium") == LifeStage.GERIATRIC
...
class TestFeedingHistoryAnalysis:
    ...
    def test_analyze_feeding_history_balanced_plan(self) -> None:
        ...
        assert any(
            "well balanced" in recommendation for recommendation in result["recommendations"]
        )
