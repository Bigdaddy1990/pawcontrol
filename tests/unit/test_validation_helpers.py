...
def test_validate_coordinate_bounds() -> None:
    ...
    with pytest.raises(ValidationError) as err:
        validate_coordinate(181, field="longitude",
...
def test_validate_interval_clamps() -> None:
    assert validate_interval(
        2, field="interval", minimum=5, maximum=10, clamp=True) == 5
    assert (
        validate_interval(12, field="interval", minimum=5,
                          maximum=10, clamp=True) == 10
    )
