...
async def test_manual_event_choices_support_disable_and_translations(
    ...
    with patch(
        ...
        guard_options = flow._manual_event_choices(
            "manual_guard_event", current_system)
        check_options = flow._manual_event_choices(
            "manual_check_event", current_system)
...
async def test_configure_door_sensor_persistence_failure_records_telemetry(
    ...
    data_manager = SimpleNamespace(
        async_update_dog_data=AsyncMock(
...
def test_export_payload_normalises_legacy_options(
    ...
    notifications = cast(NotificationOptions,
                         payload["options"][CONF_NOTIFICATIONS])
    assert notifications[CONF_QUIET_HOURS] is False
