...
def test_build_update_statistics_defaults_rejection_metrics(monkeypatch) -> None:
    ...
    monkeypatch.setattr(tasks, "get_runtime_data", lambda *_: runtime_data)
    monkeypatch.setattr(
        tasks, "collect_resilience_diagnostics", lambda *_: None)
    _patch_runtime_store(monkeypatch)
...
def test_derive_rejection_metrics_handles_missing_summary() -> None:
    ...
    assert tasks.derive_rejection_metrics(
        None) == tasks.default_rejection_metrics()
    assert tasks.derive_rejection_metrics(
        {}) == tasks.default_rejection_metrics()
...
def test_collect_resilience_diagnostics_persists_summary(monkeypatch) -> None:
    ...
    assert diagnostics_payload["summary"]["rejection_rate"] == pytest.approx(
        0.25)
    assert diagnostics_payload["summary"]["rejection_breakers"] == [
        "automation"]
...
def test_collect_resilience_diagnostics_prefers_breaker_metadata(monkeypatch) -> None:
    ...
    diagnostics_payload = runtime_data.performance_stats["resilience_diagnostics"]
    assert diagnostics_payload["summary"]["rejection_breaker_ids"] == [
        "api-primary"]
    assert diagnostics_payload["summary"]["rejection_rate"] == pytest.approx(
        1 / (2 + 1))
...
def test_collect_resilience_diagnostics_converts_temporal_values(monkeypatch) -> None:
    ...
    try:
        ...
        start_of_day = datetime.combine(
            fallback_date, datetime.min.time(), tzinfo=UTC)
...
def test_build_runtime_statistics_threads_rejection_metrics(monkeypatch) -> None:
    ...
    assert pytest.approx(rejection_metrics["rejection_rate"], rel=1e-6) == pytest.approx(
        2 / 12, rel=1e-6
    )
