...
async def test_process_feeding_batch_limits_history_and_emits_events(
    ...
    monkeypatch.setattr(
        "custom_components.pawcontrol.helpers.MAX_HISTORY_ITEMS", 2)
...
async def test_process_health_batch_serializes_structured_events(
    ...
    monkeypatch.setattr(
        "custom_components.pawcontrol.helpers.MAX_HISTORY_ITEMS", 3)
    ...
    assert all(isinstance(entry.get("timestamp"), str)
...
async def test_process_walk_batch_sorts_history_descending(
    ...
    serialized_history = cast(
        list[JSONMutableMapping], serialized_entry["history"])
...
async def test_async_process_notifications_prioritizes_high_priority(
    ...
    task = asyncio.create_task(
        notification_manager._async_process_notifications())
