...
class _DummyLoader:
    ...
    def get_data(self) -> str:
        if not self._has_data:
            raise RuntimeError(
                "No data available - check_data() returned False")
...
def test_safe_load_accepts_legacy_loader_keyword() -> None:
    data = vendored_yaml.safe_load(
        "answer: 42", Loader=vendored_yaml.SafeLoader)
    assert data == {"answer": 42}
