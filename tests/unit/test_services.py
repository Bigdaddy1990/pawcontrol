...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("dog_name", "unknown_constraint", None, "dog_name is invalid"),
    ],
)
...
def test_format_expires_in_hours_error_boundary_cases() -> None:
    ...
    required = services.ValidationError(
        ...
        value=None,
        constraint="expires_in_hours_required",
    )
    min_only = services.ValidationError(
        field="expires_in_hours",
        value=0,
        constraint="expires_in_hours_out_of_range",
        min_value=1.0,
    )
    max_only = services.ValidationError(
        field="expires_in_hours",
        value=900,
        constraint="expires_in_hours_out_of_range",
        max_value=48.0,
    )
    generic = services.ValidationError(
        field="expires_in_hours",
        value="bad",
        constraint="unknown",
    )
    ...
    assert (
        services._format_expires_in_hours_error(min_only)
        == "expires_in_hours must be greater than 1"
    )
    assert (
        services._format_expires_in_hours_error(max_only)
        == "expires_in_hours must be less than 48"
    )
    assert (
...
def test_normalise_service_details_handles_sequence_and_scalar_payloads() -> None:
    ...
    assert services._normalise_service_details([1, "two"]) == {"items": [1, "two"]}
    assert services._normalise_service_details(5) == {"value": 5}
...
def test_classify_error_reason_detects_notification_failures() -> None:
    ...
    assert (
        classify_error_reason("exception", error="Unauthorized device") == "auth_error"
    )
    assert (
...
class _DataManagerStub:
    ...
    async def async_start_grooming_session(
        ...
        self.groom_calls.append({
            "dog_id": dog_id,
            "grooming_data": dict(grooming_data),
        })
...
async def test_gps_start_walk_service_coerces_common_boolean_inputs(
    ...
    await handler(
        SimpleNamespace(data={"dog_id": "fido", "track_route": service_value})
    )
...
async def test_setup_automatic_gps_service_rejects_invalid_safe_zone_radius(
    ...
    with pytest.raises(
        ...
        await handler(
            SimpleNamespace(data={"dog_id": "fido", "safe_zone_radius": "wide"})
        )
