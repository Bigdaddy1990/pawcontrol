...
def test_classify_error_reason_detects_notification_failures() -> None:
    ...
    assert classify_error_reason(
        "exception", error="Unauthorized device") == "auth_error"
    assert (
...
class _DataManagerStub:
    ...
    async def async_log_poop_data(
        ...
        self.poop_calls.append(
            {"dog_id": dog_id, "poop_data": dict(poop_data)})
...
class _NotificationManagerStub:
    ...
    async def async_send_notification(self, **kwargs: object) -> str:
        ...
        if force_channels is not None:
            ...
            assert all(isinstance(channel, NotificationChannel)
...
async def _setup_service_environment(
    ...
    monkeypatch.setattr(services, "_coordinator_resolver",
                        lambda hass_instance: resolver)
    monkeypatch.setattr(
...
async def test_perform_daily_reset_records_cache_diagnostics(
    ...
    assert snapshots["coordinator_modules"].to_mapping(
    ...
    assert hasattr(cache_metrics["snapshots"]
                   ["coordinator_modules"], "to_mapping")
    assert (
...
async def test_perform_daily_reset_normalises_complex_reconfigure_metadata(
    ...
    monkeypatch.setattr(
        services, "update_runtime_reconfigure_summary", fake_update)
    monkeypatch.setattr(services, "get_runtime_data",
                        lambda hass, entry: runtime_data)
...
async def test_send_notification_service_accepts_enum_inputs(
    ...
    force_channels = sent_payload["force_channels"]
    assert force_channels == [
        NotificationChannel.MOBILE, NotificationChannel.DISCORD]
...
async def test_gps_start_walk_service_rejects_invalid_boolean_toggle(
    ...
    assert result["status"] == "error"
    assert "track_route must be a boolean (got str)" in result.get(
        "message", "")
...
async def test_setup_automatic_gps_service_rejects_invalid_boolean_toggle(
    ...
    with pytest.raises(
        ...
        await handler(
            SimpleNamespace(
                data={"dog_id": "fido", "auto_start_walk": "perhaps"})
        )

    result = runtime_data.performance_stats["last_service_result"]
    assert result["service"] == services.SERVICE_SETUP_AUTOMATIC_GPS
    assert result["status"] == "error"
    assert "auto_start_walk must be a boolean (got str)" in result.get(
        "message", "")
...
async def test_add_health_snack_records_success(
    ...
    coordinator = _CoordinatorStub(
        SimpleNamespace(), feeding_manager=feeding_manager)
...
async def test_check_feeding_compliance_notifies_on_issues(
    ...
    context = Context(user_id="user-1", parent_id="parent-1",
                      context_id="ctx-1")
    ...
    compliance_result = cast(FeedingComplianceCompleted,
...
def test_merge_service_context_metadata_respects_include_none() -> None:
    ...
    services._merge_service_context_metadata(
        target, metadata, include_none=True)
...
async def test_check_feeding_compliance_records_errors(
    ...
    feeding_manager = _FeedingManagerStub()
    feeding_manager.compliance_error = services.HomeAssistantError(
        "compliance failed")
    notification_manager = _NotificationManagerStub()
    coordinator = _CoordinatorStub(
        SimpleNamespace(),
