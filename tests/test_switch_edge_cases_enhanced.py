...
class TestNetworkTimeoutAndFailureRecovery:
    ...
    @pytest.mark.asyncio
    async def test_coordinator_timeout_during_state_change(self, network_sensitive_switch):
        """Test switch behavior when coordinator times out."""
        # Mock coordinator timeout
        network_sensitive_switch.coordinator.async_request_selective_refresh = AsyncMock(
    ...
    @pytest.mark.asyncio
    async def test_coordinator_unavailable_during_setup(self, hass: HomeAssistant, mock_entry, mock_coordinator):
    ...
    async def test_partial_service_failure_handling(self, mock_coordinator):
        ...
        with patch.object(switch, 'hass') as mock_hass:
            ...
            mock_hass.services.async_call = AsyncMock(side_effect=service_call_side_effect)
...
class TestEntityRegistryCorruptionRecovery:
    ...
    @pytest.mark.asyncio
    async def test_duplicate_unique_id_handling(self, hass: HomeAssistant, mock_coordinator):
...
class TestIntegrationReloadEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_reload_during_entity_creation(self, hass: HomeAssistant, mock_entry, mock_coordinator):
    ...
    async def test_config_entry_update_during_operation(self, mock_coordinator):
        ...
        original_data = {"dogs": [{"dog_id": "update_dog", "modules": {MODULE_FEEDING: False}}]}
        updated_data = {"dogs": [{"dog_id": "update_dog", "modules": {MODULE_FEEDING: True}}]}
...
class TestDependenciesMissingEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_missing_coordinator_dependency(self, hass: HomeAssistant, mock_entry):
...
class TestConcurrentModificationProtection:
    ...
    async def test_concurrent_entity_creation(self, mock_coordinator):
        ...
        assert len(unique_ids) == len(set(unique_ids))  # All unique IDs should be unique
...
class TestConfigurationMigrationEdgeCases:
    ...
    @pytest.mark.asyncio
    async def test_legacy_config_format_support(self, hass: HomeAssistant, mock_coordinator):
    ...
    @pytest.mark.asyncio
    async def test_config_entry_corruption_recovery(self, hass: HomeAssistant, mock_coordinator):
