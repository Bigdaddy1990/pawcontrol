    ...
    def check_manifest(self) -> None:
        ...
        try:
            ...
            required_fields = [
                ...
                "issue_tracker", "codeowners", "config_flow", 
                "iot_class", "integration_type"
            ]
            ...
            for field in recommended_fields:
                if field not in manifest:
                    self.warnings.append(f"manifest.json missing recommended field: {field}")
            ...
            self.info.append(f"✅ manifest.json valid - Version: {manifest.get('version', 'unknown')}")
        ...
        for py_file in py_files:
            try:
                ...
                if not has_type_hints:
                    self.warnings.append(f"{py_file.name} lacks comprehensive type hints")
    ...
    def check_services_yaml(self) -> None:
        ...
        try:
            ...
            if services:
                ...
                self.info.append(f"✅ services.yaml valid - {service_count} services defined")
                
                # Check if all services have schemas
                for service_name, service_def in services.items():
                    if not service_def.get('fields'):
                        self.warnings.append(f"Service '{service_name}' has no fields defined")
        ...
        try:
            ...
            for section in required_sections:
                if section not in strings:
                    self.errors.append(f"strings.json missing required section: {section}")
    ...
    def check_imports(self) -> None:
        ...
        try:
            ...
            has_schema_import = any(
                imp.module == '.schemas' for imp in imports
            )
