...
- Zusätzlich liefert das Panel `setup_flags_panel` lokalisierte Überschriften, Quellenangaben und aktiv/inaktiv-Zähler, sodass Support-Dashboards den Onboarding-Status ohne Nachbearbeitung übernehmen können.【F:custom_components/pawcontrol/diagnostics.py†L120-L210】【F:custom_components/pawcontrol/strings.json†L1396-L1405】【F:tests/components/pawcontrol/test_diagnostics.py†L288-L405】
...
- Der Diagnostik-Export ergänzt nun einen eigenständigen `resilience`-Block, der die zuletzt gespeicherten Circuit-Breaker-Snapshots samt Recovery-Latenz, Ablehnungsrate und Breaker-Identifikatoren aus dem Runtime-Store zieht, sodass Support-Teams auch bei pausiertem Koordinator vollständige Resilience-Daten abrufen können.【F:custom_components/pawcontrol/diagnostics.py†L600-L676】【F:custom_components/pawcontrol/telemetry.py†L400-L470】【F:tests/components/pawcontrol/test_diagnostics.py†L430-L520】
- Der Runtime-Store führt eine Versionsprüfung durch: Legacy-Snapshots werden vor der Ausgabe auf das kompatible Mindest-Schema angehoben, während zukünftige Versionen sofort verworfen und mit einem leeren Cache ersetzt werden. Diagnostics und System-Health laden dadurch niemals inkompatible Payloads und weisen Anwender direkt auf einen notwendigen Reload hin.【F:custom_components/pawcontrol/runtime_data.py†L1-L312】【F:tests/test_runtime_data.py†L1-L640】
 - Koordinator-Statistiken aktualisieren den Runtime-Store-Snapshot bei jedem Diagnostiklauf, zählen kompatible und divergierende Zustände, stempeln die letzte Prüfung und führen jetzt kumulierte Laufzeiten pro Level sowie die aktuelle Verweildauer mit. Diagnostics und System-Health geben den gespeicherten Verlauf samt Zeitanteilen parallel zum aktuellen Snapshot aus, damit Support-Teams Trendanalysen ohne zusätzliche Tools fahren können. Eine begrenzte Timeline hält zusätzlich die jüngsten Bewertungen inklusive Levelwechsel, Divergenzraten und empfohlenen Aktionen bereit, sodass Eskalationen auch nach Wartungsfenstern nachvollziehbar bleiben. Ergänzend fasst eine `runtime_store_timeline_summary` Totalereignisse, Levelwechsel-Rate, Status-/Level-Histogramme, Beobachtungsfenster, Event-Dichte, häufigste Gründe/Status, Divergenz-Durchschnitt/Peak sowie Spitzen- und Letztwerte der Level-Dauern zusammen, damit Reviews die Timeline ohne manuelles Parsen beurteilen können. Neu hinzugekommene `level_duration_guard_alerts` markieren zusätzlich, wenn die `p95`-Dauern für `watch`- oder `action_required`-Phasen definierte Guard-Limits überschreiten und liefern die zugehörigen Empfehlungen direkt an Diagnostics, System Health und Repairs weiter.【F:custom_components/pawcontrol/telemetry.py†L300-L520】【F:custom_components/pawcontrol/diagnostics.py†L600-L690】【F:custom_components/pawcontrol/system_health.py†L70-L520】【F:tests/unit/test_runtime_store_telemetry.py†L120-L360】【F:tests/components/pawcontrol/test_diagnostics.py†L500-L612】【F:tests/components/pawcontrol/test_system_health.py†L18-L200】
- Zusätzlich liefern Diagnostics und System-Health einen `runtime_store`-Abschnitt, der Entry- und Store-Metadaten (Version, Erstellversion, Mindestkompatibilität) mit Migrationstatus, Divergenzerkennung und Future-Version-Flags kombiniert, sodass Support:innen fehlerhafte Caches ohne Debug-Konsole identifizieren können.【F:custom_components/pawcontrol/runtime_data.py†L1-L390】【F:custom_components/pawcontrol/diagnostics.py†L610-L684】【F:custom_components/pawcontrol/system_health.py†L420-L520】【F:tests/test_runtime_data.py†L520-L640】【F:tests/components/pawcontrol/test_diagnostics.py†L430-L520】【F:tests/components/pawcontrol/test_system_health.py†L20-L940】
- Die `runtime_store_assessment` fasst Divergenzraten, Migrationserfordernisse und Entry-/Store-Status zu den Stufen `ok`, `watch` oder `action_required` zusammen, damit Support:innen ohne zusätzliche Berechnungen entscheiden können, wann der `runtime_store_compatibility`-Repair oder ein Reload nötig ist. Diagnostics, System-Health und Koordinatorstatistiken blenden die Bewertung parallel zu Snapshot und Historie ein. Zusätzlich speichert die Integration das vorherige Level, die Level-Streak, den Zeitpunkt der letzten Änderung, Eskalations- und Deeskalationszähler sowie die akkumulierten Sekunden je Level und die aktuelle Verweildauer, sodass Bereitschaften sofort erkennen, ob sich die Laufzeit-Caches stabilisieren oder erneute Maßnahmen erforderlich sind.【F:custom_components/pawcontrol/telemetry.py†L347-L497】【F:custom_components/pawcontrol/coordinator_tasks.py†L108-L143】【F:custom_components/pawcontrol/diagnostics.py†L606-L690】【F:custom_components/pawcontrol/system_health.py†L430-L540】【F:tests/unit/test_runtime_store_telemetry.py†L17-L260】【F:tests/components/pawcontrol/test_diagnostics.py†L500-L580】【F:tests/components/pawcontrol/test_system_health.py†L1-L160】【F:tests/unit/test_coordinator_tasks.py†L200-L226】
- Die Reparatursuite hängt sich an denselben Snapshot: `_check_runtime_store_health` legt bei Divergenzen, Legacy-Schemata oder zukünftigen Versionen das Repair-Issue `runtime_store_compatibility` mit Warn-/Fehler-Schwere an und entfernt es automatisch, sobald `describe_runtime_store_status` wieder `current` meldet.【F:custom_components/pawcontrol/repairs.py†L64-L190】【F:custom_components/pawcontrol/repairs.py†L360-L520】【F:custom_components/pawcontrol/repairs.py†L732-L815】【F:tests/integration/test_runtime_store_ui.py†L180-L310】
...
- Das neue Feld `service_execution.entity_factory_guard` spiegelt die adaptive Entity-Factory-Laufzeitüberwachung inklusive aktueller Guard-Bodenschwelle, Delta zum Baseline-Floor, beobachtetem Höchst-/Tiefst-Floor, letzter Bodenzeitänderung (absolut und relativ), Durchschnitts-/Minimal-/Maximallaufzeit der Samples, Stabilitäts- und Volatilitätsquoten sowie Laufzeit-Jitter über gesamte Historie und die letzten fünf Samples wider. Rekalibrierungen werden direkt im Runtime-Store gesichert, Telemetrie reichert sie mit Zeitstempeln, Guard-Event-Historie, Recency-Samples und Streak-Zählern an, und Diagnostics sowie System-Health rendern den JSON-Snapshot inklusive Stabilitätstrend, der Recency-Stabilität gegen den Lifetime-Durchschnitt stellt, neben den Guard- und Breaker-Indikatoren.【F:custom_components/pawcontrol/entity_factory.py†L1017-L1136】【F:custom_components/pawcontrol/telemetry.py†L101-L244】【F:custom_components/pawcontrol/diagnostics.py†L1387-L1477】【F:custom_components/pawcontrol/system_health.py†L394-L612】【F:tests/components/pawcontrol/test_diagnostics.py†L540-L612】【F:tests/components/pawcontrol/test_system_health.py†L18-L663】
- `PawControlCoordinator.get_performance_snapshot()` spiegelt den selben
  `service_execution`-Block und teilt sich das `rejection_metrics`-Objekt mit den
  Laufzeitstatistiken, sodass API-Clients und Support-Tools ohne zusätzliche
  Normalisierung auf Guard-Zähler, Skip-Gründe und die synchronisierten
  Breaker-Metriken zugreifen können.【F:custom_components/pawcontrol/coordinator.py†L474-L525】【F:tests/unit/test_coordinator.py†L117-L165】
