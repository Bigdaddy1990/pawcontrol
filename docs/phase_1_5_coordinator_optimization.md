...
class PawControlCoordinator(DataUpdateCoordinator):
    ...
    def __init__(self, hash, entry, session):
        super().__init__(...)
...
async def test_selective_entity_updates(hash, coordinator):
    """Test that only changed entities update."""
    # Setup entities
    gps_entity = setup_gps_entity(hash, coordinator, "buddy")
    walk_entity = setup_walk_entity(hash, coordinator, "buddy")
...
async def test_data_access_enforcement(hash, coordinator):
    """Test coordinator access enforcement."""
    entity = setup_entity_with_enforcement(hash, coordinator)
