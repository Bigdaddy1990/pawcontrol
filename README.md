...
The system health panel reuses the same serialised history so operators reviewing the guard summary can also trace the manual intervention trail without opening diagnostics exports.【F:custom_components/pawcontrol/system_health.py†L103-L176】【F:tests/components/pawcontrol/test_system_health.py†L130-L210】
Unit coverage exercises system health guard/breaker summaries, runtime store status, and coercion helpers to keep the system health payload stable under mixed telemetry payloads.【F:tests/unit/test_system_health.py†L1-L161】
