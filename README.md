...
[![Super-Linter](https://github.com/Bigdaddy1990/pawcontrol/actions/workflows/superlinter.yml/badge.svg)](https://github.com/marketplace/actions/super-linter)
...
1. **Prerequisites Check**:
   ...
   # Settings → System → General → Version (tested with 2025.9.x builds)
   ```
...
- The dashboard configuration wizard localizes its module summary and feature
  ...
  enabled.【F:custom_components/pawcontrol/config_flow_modules.py†L90-L659】【F:custom_components/pawcontrol/config_flow_dashboard_extension.py†L20-L236】【F:tests/unit/test_config_flow_modules.py†L360-L402】
...
- Grooming module toggles and fine-grained switches resolve translated labels such as "Pflege-Tracking", "Pflegeplan", and "Pflege-Erinnerungen" through the shared helper so localized installations see consistent names across the UI.【F:custom_components/pawcontrol/switch.py†L626-L688】
...
Regression coverage exercises both rejected and reset circuits to guarantee the helper persists the zeroed defaults, attaches the metrics to service diagnostics, and retains the resilience details on the last service result exported to support teams.【F:tests/unit/test_services.py†L94-L203】
...
- The auto-generated Lovelace statistics view now ships with a **Resilience metrics**
  ...
  bespoke templates.【F:custom_components/pawcontrol/dashboard_templates.py†L1723-L1966】【F:tests/unit/test_dashboard_templates.py†L560-L620】
...
- **Growing Test Coverage**: Core flows (config flows, service handling, and telemetry helpers) are covered by unit and harness tests, with additional scenarios under active development.【F:tests/unit/test_config_flow_modules.py†L1-L430】【F:tests/unit/test_services.py†L1-L3100】【F:tests/unit/test_runtime_store_telemetry.py†L1-L360】
- **Workflow Validation**: Targeted harness tests cover critical coordinator and service guard paths for runtime statistics and resilience snapshots.【F:tests/unit/test_coordinator.py†L1-L260】【F:tests/unit/test_coordinator_tasks.py†L1000-L1305】
- **Edge Case Testing**: Input validation and diagnostics redaction paths include regression tests for boundary conditions and sensitive data handling.【F:tests/test_validation_inputs.py†L1-L240】【F:tests/test_diagnostics.py†L1-L200】
...
- Unit and harness tests exercise config flow, coordinator updates, diagnostics helpers, and repairs using the Home Assistant stubs in `tests/helpers` for repeatable coverage.【F:tests/unit/test_config_flow_dogs.py†L1-L260】【F:tests/unit/test_coordinator.py†L1-L260】【F:tests/test_diagnostics.py†L1-L200】【F:tests/test_repairs.py†L1-L220】【F:tests/helpers/homeassistant_test_stubs.py†L1-L120】
