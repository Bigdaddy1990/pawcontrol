...
2. Deliver the remediation targets above, then re-run `mypy` to confirm the error count drops materially before tightening repository gates.【7bf48b†L1-L87】
3. Continue chipping away at the typing backlog—next targets are the remaining config flow mixins and helper factories (module toggles, feeding schedulers) so the mypy error count falls ahead of reenabling strict gates.【db3742†L1-L126】【F:custom_components/pawcontrol/config_flow_dogs.py†L200-L760】【F:custom_components/pawcontrol/helper_manager.py†L200-L560】
