...
class Coverage:
    ...
    def __init__(self, *, source: Iterable[str] | None = None) -> None:  # noqa: E111
        ...
        if monitoring is not None and all(
            hasattr(monitoring, attr) for attr in required
        ):
    ...
    def report(self, *, file: Any | None = None, skip_empty: bool = True) -> float:  # noqa: E111
        ...
        files = []
        for path, executed in sorted(
            self._executed.items(), key=lambda item: str(item[0])
        ):
            ...
            percent = (
                100.0 if statements == 0 else (executed_count / statements) * 100.0
            )  # noqa: E111
