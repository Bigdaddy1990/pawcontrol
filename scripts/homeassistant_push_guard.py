    ...
    def parse(cls, value: str) -> Version:
        if not isinstance(value, str):
            raise InvalidVersion(
                f"Expected version string, got {type(value)!r}")
...
def load_rules(path: Path) -> RuleSet:
    try:
        ...
        raise ValueError(
            f"Invalid JSON in rules file '{path}': {err}") from err
    ...
    for field_name in ("min_covered_version", "max_covered_version", "rules"):
        if field_name not in payload:
            raise ValueError(
                f"Rules file is missing required field '{field_name}'.")
...
def fetch_latest_homeassistant_version() -> Version:
    ...
    try:
        # nosec B310 - request URL is restricted by _validated_https_url.
        with urlopen(request, timeout=REQUEST_TIMEOUT_SECONDS) as response:
            status = getattr(response, "status", 200)
            if status != 200:
                raise ValueError(
                    f"PyPI API returned unexpected status {status}")
        ...
        raise RuntimeError(
            "PyPI returned malformed Home Assistant version data.") from err
    ...
    try:
        ...
        if backup_created and backup_path.exists():
            try:
                ...
                _LOGGER.warning(
                    "Failed to remove backup file %s: %s", backup_path, err)
...
def run(
    ...
    for rule in rules:
        for path in collect_python_files(root, rule.file_globs):
            ...
            if count:
                findings.append(MatchResult(
                    rule_id=rule.id, path=path, count=count))
