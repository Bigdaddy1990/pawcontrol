...
def fetch_pypi_metadata() -> dict[str, Any]:
    ...
    try:
        ...
        raise MonitoringError(
            "Failed to fetch PyYAML metadata from PyPI") from exc
    data = response.json()
...
def query_osv(vendor_version: Version) -> list[VulnerabilityRecord]:
    ...
    try:
        ...
        raise MonitoringError(
            "Failed to query OSV for PyYAML vulnerabilities") from exc
    data = response.json()
    entries: list[dict[str, Any]] = data.get(
        "vulns") or data.get("vulnerabilities") or []
    ...
    *,
    ...
    for profile in wheel_profiles:
        ...
        wheel_matches.append(
            WheelMatch(profile=profile, release=release,
