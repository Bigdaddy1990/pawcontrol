...
class TypeHintMatch:
    ...
    def need_to_check_function(self, node: nodes.FunctionDef) -> bool:
        ...
        return (
            ...
            or (self.has_async_counterpart and node.name == f"async_{self.function_name}")
            or (
...
_MODULE_REGEX: re.Pattern[str] = re.compile(
    r"^homeassistant\.components\.\w+(\.\w+)?$")
...
def _is_valid_type(
    ...
    if (
        ...
        return (
            ...
            and isinstance(node.slice, nodes.Tuple)
            and all(
                _is_valid_type(match.group(n + 2),
                               node.slice.elts[n], in_return)
    ...
    if (
        ...
        isinstance(node, nodes.Name) and node.name.endswith(
            _KNOWN_GENERIC_TYPES_TUPLE)
    ):
        return True
...
def _is_valid_return_type(match: TypeHintMatch, node: nodes.NodeNG) -> bool:
    ...
    if isinstance(match.return_type, (str, list)) and isinstance(node, nodes.Name):
        if isinstance(match.return_type, str):
            ...
            valid_types = {
                el for el in match.return_type if isinstance(el, str)}
