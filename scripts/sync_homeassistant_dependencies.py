        ...
        *,
        ...
        for requirement_entry in requirements:
            ...
            if reference_requirement is None:
                ...
                continue
            new_requirement = compose_requirement(
...
def update_requirement_file(
    ...
    for line in lines:
        ...
        if reference_requirement is None:
            new_lines.append(line)
            continue
        updated_requirement = compose_requirement(
            requirement, reference_requirement)
        if comment:
...
def download_pyyaml_source(version: Version) -> Path:
    ...
    for entry in release_files:
        ...
        return archive_path
    raise RuntimeError(
        f"Unable to locate PyYAML {version} source distribution")
...
def extract_pyyaml_archive(archive: Path, version: Version) -> None:
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        if not candidates:
            raise RuntimeError(
                "PyYAML archive did not contain an expected root folder")
        ...
        if not yaml_source.exists():
            raise RuntimeError(
                "PyYAML archive did not contain the yaml package")
...
def main() -> int:
    ...
    loader = ReferenceLoader(
        root=args.home_assistant_root, base_url=args.reference_url)
