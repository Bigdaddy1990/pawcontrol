...
def build_cli() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--mode", choices={"pages", "artifact"}, default="artifact")
...
class GitHubPagesPublisher:
    ...
    def _request_json(
        ...
        *,
        ...
        data = json.dumps(payload).encode(
            "utf-8") if payload is not None else None
    ...
    def prune_expired_runs(
        ...
        *,
        ...
        tree_sha = tree_response.get("sha") if isinstance(
            tree_response, dict) else None
        if not tree_sha:
            return removed
        ...
        commit_sha = (
            commit_response.get("sha") if isinstance(
                commit_response, dict) else None
        )
        if not commit_sha:
            return removed
        ...
        if token and repository:
            try:
                publisher = GitHubPagesPublisher(
                    token, repository, args.pages_branch)
