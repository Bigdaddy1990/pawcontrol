...
def run_validation_suite() -> Dict[str, Any]:
    ...
    try:
        ...
        comprehensive_bonus = (
            len(comprehensive_tests_found) / len(comprehensive_tests) * 10
        )
        ...
        print(
            f"  Core File Coverage: {core_coverage:.1f}% ({len(core_files_with_tests)}/{len(core_files)})"
        )
        print(
            f"  Comprehensive Tests: {len(comprehensive_tests_found)}/{len(comprehensive_tests)}"
        )
        ...
        if estimated_coverage >= 95.0:
            ...
            validation_results["recommendations"].append(
                "Increase test coverage to reach 95% Gold Standard"
            )
        ...
        if manifest_path.exists():
            try:
                ...
                if missing_keys:
                    ...
                    structure_results["issues"].append(
    ...
    test_coverage_pass = validation_results["test_coverage"].get(
        "meets_gold_standard", False
    )
    structure_pass = validation_results["hassfest_hacs"].get(
    ...
    if overall_compliance >= 1.0:
        ...
        final_message = (
            "Integration meets all Gold Standard requirements (95%+ coverage)"
        )
    ...
    validation_results["summary"] = {
        ...
        "test_coverage_percentage": validation_results["test_coverage"].get(
            "estimated_coverage", 0
        ),
        "quality_score": validation_results["quality_assessment"]
        .get("quality_checks", {})
        .get("total_score", 0),
    }
    ...
    print("=" * 60)
    print(
        f"Overall Compliance: {overall_compliance * 100:.1f}% ({passed_checks}/{total_checks})"
    )
    print(
        f"Test Coverage: {validation_results['summary']['test_coverage_percentage']:.1f}%"
    )
