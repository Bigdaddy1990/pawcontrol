"""Minimal hassfest trigger validation used in tests."""
...
def _load_description(domain: str) -> Mapping[str, Mapping[str, object]]:
    path = Path(domain) / TRIGGER_DESCRIPTION_FILENAME
...
def validate(integrations, config) -> None:
    ...
    for domain, integration in integrations.items():
        ...
        try:
            ...
            integration.add_error(ROOT_KEY, "Invalid triggers.yaml")
            continue
        icons = _load_json(domain, TRIGGER_ICONS_FILENAME).get(ROOT_KEY, {})
        strings = _load_json(domain, TRIGGER_STRINGS_FILENAME).get(ROOT_KEY, {})
