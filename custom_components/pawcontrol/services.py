...
SERVICE_FEED_HEALTH_AWARE_SCHEMA: Final = vol.Schema({
    ...
    vol.Optional("override_portion"): vol.All(vol.Coerce(float), vol.Range(min=0, max=2000)),
...
SERVICE_UPDATE_HEALTH_DATA_SCHEMA: Final = vol.Schema({
    ...
    vol.Optional("ideal_weight"): vol.All(vol.Coerce(float), vol.Range(min=0.1, max=200)),
    vol.Optional("body_condition_score"): vol.All(vol.Coerce(int), vol.Range(min=1, max=9)),
    vol.Optional("age_months"): vol.All(vol.Coerce(int), vol.Range(min=1, max=300)),
    vol.Optional("activity_level"): vol.In(["very_low", "low", "moderate", "high", "very_high"]),
...
SERVICE_FEED_WITH_MEDICATION_SCHEMA: Final = vol.Schema({
    ...
    vol.Optional("medication_timing", default="optimal"): vol.In(["optimal", "immediate", "delayed"]),
    vol.Optional("notes", default=""): cv.string,
})
...
def service_handler(
    require_dog: bool = True,
    cache_priority: int = 5,
    timeout: float = 10.0
):
...
class PawControlServiceManager:
    ...
    async def _handle_log_medication_service(
        ...
        data_manager = runtime_data["data_manager"]
        await data_manager.async_log_health(dog_id, {
            "type": "medication",
            **medication_data,
        })
    ...
    async def _handle_recalculate_health_portions(
        ...
        try:
            ...
            _LOGGER.info(
                ...
                dog_id, old_portion, new_portion, trigger_reason
            )
    ...
    async def _handle_feed_health_aware(
        ...
        try:
            ...
            _LOGGER.info(
                ...
                dog_id, meal_type, portion_size, calculation_method
            )
    ...
    async def _handle_feed_with_medication(
        ...
        try:
            ...
            if auto_calculate:
                ...
                portion_size = config.calculate_portion_size() * 0.3  # 30% of normal portion
            else:
                portion_size = config.calculate_portion_size() * 0.5  # 50% of normal portion
