  ...
  *,
  ...
  if level != "action_required":
    if entry_status in _RUNTIME_STORE_ENTRY_ACTION_STATUSES or (
      ...
      level = "watch"
      reason = "Runtime store metadata is pending an upgrade to the current schema."

  if level == "ok":
    if divergence_detected:
      level = "watch"
      reason = "The latest compatibility check detected divergence between caches."
  ...
  if level_changed:
    ...
    if previous_level is not None:
      ...
      level_durations[previous_level] = base_previous_total + resolved_previous_duration
