...
class ReportGenerator:
        ...
        for dog in dogs:
            ...
            dog_report = {
                ...
                "walk": {
                    ...
                    "total_distance": dog_data.get("walk", {}).get("total_distance_today", 0),
                    "last_walk": dog_data.get("walk", {}).get("last_walk"),
                    "needs_walk": dog_data.get("walk", {}).get("needs_walk", False),
                },
                "feeding": {
                    "breakfast": dog_data.get("feeding", {}).get("feedings_today", {}).get("breakfast", 0),
                    "lunch": dog_data.get("feeding", {}).get("feedings_today", {}).get("lunch", 0),
                    "dinner": dog_data.get("feeding", {}).get("feedings_today", {}).get("dinner", 0),
                    "snacks": dog_data.get("feeding", {}).get("feedings_today", {}).get("snack", 0),
                    "total_portions": dog_data.get("feeding", {}).get("total_portions_today", 0),
                    "is_hungry": dog_data.get("feeding", {}).get("is_hungry", False),
                },
                "health": {
                    "weight": dog_data.get("health", {}).get("weight_kg", 0),
                    "medications_given": dog_data.get("health", {}).get("medications_today", 0),
                    "last_medication": dog_data.get("health", {}).get("last_medication"),
                },
                "grooming": {
                    "needs_grooming": dog_data.get("grooming", {}).get("needs_grooming", False),
                    "last_grooming": dog_data.get("grooming", {}).get("last_grooming"),
                },
                "activity": {
                    "play_time": dog_data.get("activity", {}).get("play_duration_today_min", 0),
                    "training_sessions": dog_data.get("training", {}).get("training_sessions_today", 0),
                    "activity_level": dog_data.get("activity", {}).get("activity_level", "medium"),
                    "calories_burned": dog_data.get("activity", {}).get("calories_burned_today", 0),
                },
                "statistics": {
                    "poop_count": dog_data.get("statistics", {}).get("poop_count_today", 0),
    ...
    async def _send_notification_report(self, report_data: Dict[str, Any], scope: str) -> None:
    ...
    async def _save_json_report(self, filepath: Path, report_data: Dict[str, Any]) -> None:
        """Save report as JSON."""
        with open(filepath, "w", encoding="utf-8") as f:
            json.dump(report_data, f, indent=2, ensure_ascii=False, default=str)

    async def _save_csv_report(self, filepath: Path, report_data: Dict[str, Any]) -> None:
        """Save report as CSV."""
    ...
    async def _save_text_report(self, filepath: Path, report_data: Dict[str, Any]) -> None:
        """Save report as text."""
        with open(filepath, "w", encoding="utf-8") as f:
    ...
    def _format_text_report(self, report_data: Dict[str, Any]) -> str:
        ...
        for dog_id, dog_data in report_data["dogs"].items():
            ...
            lines.append(f"🍽️ Meals: B:{meals['breakfast']} L:{meals['lunch']} D:{meals['dinner']} S:{meals['snacks']}")
        ...
        try:
            ...
            health_export = {
                ...
                "health_data": {
                    ...
                    "medications": dog_data.get("health", {}).get("medication_history", []),
                    "health_notes": dog_data.get("health", {}).get("health_notes", []),
                },
                "grooming_history": dog_data.get("grooming", {}).get("grooming_history", []),
                "training_history": dog_data.get("training", {}).get("training_history", []),
            }
            
            if format_type == "json":
                with open(filepath, "w", encoding="utf-8") as f:
                    json.dump(health_export, f, indent=2, ensure_ascii=False, default=str)
            elif format_type == "csv":
