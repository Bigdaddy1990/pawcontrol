...
class _Binding:
  unsub: Callable[[], None]  # noqa: E111
  task: asyncio.Task[None] | None  # noqa: E111
...
def _haversine_m(lat1: float, lon1: float, lat2: float, lon2: float) -> float:
  r = 6371000.0  # noqa: E111
  phi1 = math.radians(lat1)  # noqa: E111
  phi2 = math.radians(lat2)  # noqa: E111
  dphi = math.radians(lat2 - lat1)  # noqa: E111
  dl = math.radians(lon2 - lon1)  # noqa: E111
  a = math.sin(dphi / 2) ** 2 + math.cos(phi1) * math.cos(phi2) * math.sin(dl / 2) ** 2  # noqa: E111
  return 2 * r * math.asin(math.sqrt(a))  # noqa: E111


def _extract_coords(
  state_obj: Any,
) -> tuple[float | None, float | None, float | None, float | None]:
  attrs = getattr(state_obj, "attributes", None)  # noqa: E111
  if not isinstance(attrs, Mapping):  # noqa: E111
    return None, None, None, None

  lat = attrs.get("latitude")  # noqa: E111
  lon = attrs.get("longitude")  # noqa: E111
  acc = attrs.get("gps_accuracy") or attrs.get("accuracy")  # noqa: E111
  alt = attrs.get("altitude")  # noqa: E111

  if isinstance(lat, (int, float)) and isinstance(lon, (int, float)):  # noqa: E111
    return (
      float(lat),
      float(lon),
  ...
  return None, None, None, None  # noqa: E111


async def async_setup_external_bindings(
  hass: HomeAssistant, entry: ConfigEntry
) -> None:
  """Set up external entity listeners for this config entry."""  # noqa: E111
  ...
  bindings = store.setdefault(_STORE_KEY, {}).setdefault(entry.entry_id, {})  # noqa: E111
  if not isinstance(bindings, dict):  # noqa: E111
    store[_STORE_KEY][entry.entry_id] = {}
    bindings = store[_STORE_KEY][entry.entry_id]

  async def _process_change(dog_id: str, entity_id: str, event: Event) -> None:  # noqa: E111
    # Debounce
    ...
    try:
      current = await gps_manager.async_get_current_location(dog_id)  # noqa: E111
    except Exception:
      current = None  # noqa: E111
    if (
      current
    ...
    try:
      ok = await gps_manager.async_add_gps_point(  # noqa: E111
        dog_id=dog_id,
        latitude=lat,
        longitude=lon,
        altitude=alt,
  ...
  for dog in dogs:  # noqa: E111
    ...
    def _on_change(
      ...
      binding = cast(_Binding | None, bindings.get(dog_id))  # noqa: E111
      if binding is None:  # noqa: E111
        return
      if binding.task and not binding.task.done():  # noqa: E111
        binding.task.cancel()
      binding.task = hass.async_create_task(_process_change(dog_id, source, event))  # noqa: E111
  ...
  _LOGGER.debug("External GPS bindings ready for entry %s", entry.entry_id)  # noqa: E111


async def async_unload_external_bindings(
  hass: HomeAssistant, entry: ConfigEntry
) -> None:
  """Unload external entity listeners for this entry."""  # noqa: E111
  store = _domain_store(hass)  # noqa: E111
  entry_map = store.get(_STORE_KEY)  # noqa: E111
  if not isinstance(entry_map, dict):  # noqa: E111
    return
  bindings = entry_map.pop(entry.entry_id, None)  # noqa: E111
  if not isinstance(bindings, dict):  # noqa: E111
    return
  for binding in bindings.values():  # noqa: E111
