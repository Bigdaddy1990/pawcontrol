...
class FeedingManager:
    ...
    async def _setup_reminders(self, dog_id: str) -> None:
        ...
        self._reminders[dog_id] = asyncio.create_task(self._reminder_loop(dog_id))
        ...
        while True:
            try:
                ...
                for schedule in config.get_todays_schedules():
                    ...
                    if reminder_time and (
                        next_reminder is None or reminder_time < next_reminder
                    ):
    ...
    async def async_skip_feeding(
        self, dog_id: str, meal_type: str, reason: Optional[str] = None
    ) -> FeedingEvent:
    ...
    async def async_get_feeding_data(self, dog_id: str) -> dict[str, Any]:
        ...
        async with self._lock:
            ...
            last_hours = (
                ((now - last_time).total_seconds() / 3600) if last_time else None
            )
                    ...
                    if scheduled_datetime <= now:
                        ...
                        for event in actual_feedings:
                            if event.meal_type == schedule.meal_type:
                                ...
                                time_diff = abs(
                                    (event.time - scheduled_datetime).total_seconds()
                                )
            ...
            return {
                ...
                "last_feeding_type": last_event.meal_type.value
                if last_event and last_event.meal_type
                else None,
                ...
                "daily_amount_target": config.daily_food_amount if config else 500,
                "daily_amount_percentage": int(
                    (daily_amount / config.daily_food_amount * 100)
                )
                ...
                "next_feeding_type": self._get_next_feeding_type(config, next_feeding)
                if config and next_feeding
                else None,
    ...
    def _get_next_feeding_type(
        self, config: FeedingConfig, next_feeding_time: datetime
    ) -> Optional[str]:
        ...
        async with self._lock:
            ...
            return {
                ...
                "daily_target_met_percentage": int(
                    (avg_daily_amount / config.daily_food_amount * 100)
                )
                if config and config.daily_food_amount > 0
                else 0,
            }
