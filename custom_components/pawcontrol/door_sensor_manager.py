...
class WalkDetectionState:
    ...
    def calculate_confidence(self) -> float:
        ...
        rapid_changes = len(
            [
                event for event in recent_events if (
                now - event[0]
                ).total_seconds() < 120
            ],
        )
...
class DoorSensorManager:
    ...
    def register_cache_monitors(
        ...
        _LOGGER.debug(
            'Registering door sensor cache monitor with prefix %s', prefix,
        )
    ...
    async def _cleanup_expired_states(self) -> None:
        ...
        while True:
            try:
                ...
                if cleaned > 0:
                    _LOGGER.debug(
                        'Cleaned %d old state history entries', cleaned,
                    )
    ...
    async def async_update_dog_configuration(
        ...
        if removing:
            ...
            if not config:
                _LOGGER.warning(
                    'No door sensor configuration found for dog %s', dog_id,
                )
                return False
