...
class WeatherForecast:
    ...
    def get_next_optimal_window(self, activity_type: str = "walk") -> ActivityTimeSlot | None:
...
class WeatherHealthManager:
    ...
    async def async_update_weather_data(
        ...
        try:
            ...
            if not weather_state or weather_state.state in [STATE_UNAVAILABLE, STATE_UNKNOWN]:
            ...
            if temperature_c is not None:
                ...
                temp_unit = attributes.get("temperature_unit", UnitOfTemperature.CELSIUS)
    ...
    async def async_update_forecast_data(
        ...
        try:
            ...
            weather_state = self.hass.states.get(weather_entity_id)
            if not weather_state or weather_state.state in [STATE_UNAVAILABLE, STATE_UNKNOWN]:
                _LOGGER.warning("Weather entity %s is unavailable for forecast", weather_entity_id)
                return None
                
            # Extract forecast data from attributes
            forecast_data = weather_state.attributes.get(ATTR_FORECAST)
            if not forecast_data:
                _LOGGER.debug("No forecast data available in weather entity %s", weather_entity_id)
                return None
        ...
        for forecast_item in forecast_data:
            try:
                ...
                if temp_high is not None:
                    temp_unit = forecast_item.get("temperature_unit", UnitOfTemperature.CELSIUS)
                    if temp_unit == UnitOfTemperature.FAHRENHEIT:
                ...
                forecast_point = ForecastPoint(
                    ...
                    precipitation_probability=forecast_item.get(ATTR_FORECAST_PRECIPITATION_PROBABILITY),
    ...
    def _assess_forecast_quality(self, forecast_data: list[dict[str, Any]]) -> ForecastQuality:
        ...
        for item in forecast_data:
            ...
            optional_fields = [ATTR_FORECAST_HUMIDITY, ATTR_FORECAST_UV_INDEX, ATTR_FORECAST_CONDITION]
    ...
    def _calculate_forecast_point_derived_values(self, forecast_point: ForecastPoint) -> None:
        ...
        if (temp_c >= 20 and 
            forecast_point.humidity_percent is not None and 
            forecast_point.humidity_percent >= 40):
        ...
        if (temp_c <= 10 and 
            forecast_point.wind_speed_kmh is not None and 
            forecast_point.wind_speed_kmh > 5):
    ...
    async def _calculate_forecast_health_scores(self) -> None:
        ...
        for forecast_point in self._current_forecast.forecast_points:
            forecast_point.health_score = self._calculate_point_health_score(forecast_point)
    ...
    def _predict_point_alerts(self, forecast_point: ForecastPoint) -> list[WeatherHealthImpact]:
        ...
        # UV alerts
        if (forecast_point.uv_index is not None and 
            forecast_point.uv_index >= self.uv_thresholds[WeatherSeverity.MODERATE]):
            alerts.append(WeatherHealthImpact.UV_EXPOSURE)
            
        # Humidity alerts
        if (forecast_point.humidity_percent is not None and 
            forecast_point.humidity_percent >= self.humidity_thresholds[WeatherSeverity.MODERATE]):
            alerts.append(WeatherHealthImpact.RESPIRATORY_RISK)
            
        # Precipitation alerts
        if (forecast_point.precipitation_probability is not None and 
            forecast_point.precipitation_probability > 60):
            alerts.append(WeatherHealthImpact.PAW_PROTECTION)
            
        # Storm alerts
        if (forecast_point.condition and 
            any(keyword in forecast_point.condition.lower() 
    ...
    def _calculate_forecast_statistics(self) -> None:
        ...
        health_scores = [
            point.health_score for point in self._current_forecast.forecast_points 
        ...
        if health_scores:
            ...
            self._current_forecast.avg_health_score = int(sum(health_scores) / len(health_scores))
    ...
    def _find_activity_windows(
        ...
        for point in self._current_forecast.forecast_points:
            if point.health_score is not None and point.health_score >= min_score:
                ...
                if current_window_start is not None:
                    window_duration = (point.timestamp - current_window_start).total_seconds() / 3600
                    ...
                    if window_duration >= min_duration_hours:
                        ...
                        windows.append(ActivityTimeSlot(
        ...
        if current_window_start is not None and current_window_scores:
            last_point = self._current_forecast.forecast_points[-1]
            window_duration = (last_point.timestamp - current_window_start).total_seconds() / 3600
    ...
    def _calculate_derived_conditions(self) -> None:
        ...
        # Calculate heat index if hot and humid
        if (temp_c >= 20 and 
            self._current_conditions.humidity_percent is not None and 
            self._current_conditions.humidity_percent >= 40):
        ...
        # Calculate wind chill if cold and windy
        if (temp_c <= 10 and 
            self._current_conditions.wind_speed_kmh is not None and 
            self._current_conditions.wind_speed_kmh > 5):
    ...
    def _check_temperature_alerts(self) -> list[WeatherAlert]:
        ...
        effective_temp = self._current_conditions.heat_index or self._current_conditions.wind_chill or temp
        ...
        if temp >= self.temperature_thresholds["hot"][WeatherSeverity.EXTREME]:
            alerts.append(WeatherAlert(
                ...
                title=self._get_translation("weather.alerts.extreme_heat_warning.title"),
                ...
                recommendations=[
                    self._get_translation("weather.recommendations.avoid_peak_hours"),
                    self._get_translation("weather.recommendations.provide_water"),
                    self._get_translation("weather.recommendations.keep_indoors"),
                    self._get_translation("weather.recommendations.watch_heat_signs"),
                    self._get_translation("weather.recommendations.use_cooling_aids"),
                    self._get_translation("weather.recommendations.never_leave_in_car"),
                ],
                duration_hours=6,
                affected_breeds=["brachycephalic", "thick_coat", "elderly", "overweight"],
            ...
            alerts.append(WeatherAlert(
                ...
                title=self._get_translation("weather.alerts.high_heat_advisory.title"),
                message=self._get_translation(
                    "weather.alerts.high_heat_advisory.message",
                    temperature=temp
                ),
                recommendations=[
                    self._get_translation("weather.recommendations.limit_outdoor_time"),
                    self._get_translation("weather.recommendations.ensure_shade"),
                    self._get_translation("weather.recommendations.provide_shade_always"),
                    self._get_translation("weather.recommendations.monitor_overheating"),
                    self._get_translation("weather.recommendations.cooler_surfaces"),
                ],
                duration_hours=4,
            ...
            alerts.append(WeatherAlert(
                ...
                title=self._get_translation("weather.alerts.warm_weather_caution.title"),
                message=self._get_translation(
                ...
                recommendations=[
                    ...
                    self._get_translation("weather.recommendations.cooler_day_parts"),
                    self._get_translation("weather.recommendations.watch_heat_stress"),
                ],
                duration_hours=3,
            ))
        ...
        if temp <= self.temperature_thresholds["cold"][WeatherSeverity.EXTREME]:
            alerts.append(WeatherAlert(
                ...
                severity=WeatherSeverity.EXTREME,
                title=self._get_translation("weather.alerts.extreme_cold_warning.title"),
                message=self._get_translation(
                ...
                recommendations=[
                    ...
                    self._get_translation("weather.recommendations.protective_clothing"),
                    self._get_translation("weather.recommendations.protect_paws"),
                    self._get_translation("weather.recommendations.warm_shelter"),
                    self._get_translation("weather.recommendations.watch_hypothermia"),
                    self._get_translation("weather.recommendations.postpone_activities"),
                ],
                duration_hours=8,
            ...
            alerts.append(WeatherAlert(
                alert_type=WeatherHealthImpact.COLD_STRESS,
                severity=WeatherSeverity.HIGH,
                title=self._get_translation("weather.alerts.high_cold_advisory.title"),
                message=self._get_translation(
                    "weather.alerts.high_cold_advisory.message",
                    temperature=temp
                ),
                recommendations=[
                    self._get_translation("weather.recommendations.shorten_activities"),
                    self._get_translation("weather.recommendations.consider_clothing"),
                    self._get_translation("weather.recommendations.cold_surface_protection"),
                    self._get_translation("weather.recommendations.warm_shelter_available"),
                ],
                duration_hours=6,
    ...
    def _check_uv_alerts(self) -> list[WeatherAlert]:
        ...
        if (not self._current_conditions or 
            self._current_conditions.uv_index is None):
            return alerts
        ...
        if uv >= self.uv_thresholds[WeatherSeverity.EXTREME]:
            alerts.append(WeatherAlert(
                ...
                severity=WeatherSeverity.EXTREME,
                title=self._get_translation("weather.alerts.extreme_uv_warning.title"),
                message=self._get_translation(
                ...
                recommendations=[
                    ...
                    self._get_translation("weather.recommendations.provide_shade_always"),
                    self._get_translation("weather.recommendations.uv_protective_clothing"),
                    self._get_translation("weather.recommendations.protect_nose_ears"),
            ...
            alerts.append(WeatherAlert(
                alert_type=WeatherHealthImpact.UV_EXPOSURE,
                severity=WeatherSeverity.HIGH,
                title=self._get_translation("weather.alerts.high_uv_advisory.title"),
                message=self._get_translation(
                    "weather.alerts.high_uv_advisory.message",
                    uv_index=uv
                ),
                recommendations=[
                    self._get_translation("weather.recommendations.shade_during_activities"),
                    self._get_translation("weather.recommendations.limit_peak_exposure"),
                    self._get_translation("weather.recommendations.monitor_skin_irritation"),
                ],
                duration_hours=4,
            ))
    ...
    def _check_humidity_alerts(self) -> list[WeatherAlert]:
        ...
        if (not self._current_conditions or 
            self._current_conditions.humidity_percent is None):
            return alerts
        ...
        if humidity >= self.humidity_thresholds[WeatherSeverity.HIGH]:
            alerts.append(WeatherAlert(
                ...
                severity=WeatherSeverity.HIGH,
                title=self._get_translation("weather.alerts.high_humidity_alert.title"),
                message=self._get_translation(
                    "weather.alerts.high_humidity_alert.message",
                    humidity=humidity
                ),
                recommendations=[
                    self._get_translation("weather.recommendations.reduce_exercise_intensity"),
                    self._get_translation("weather.recommendations.good_air_circulation"),
                    self._get_translation("weather.recommendations.monitor_breathing"),
                    self._get_translation("weather.recommendations.cool_ventilated_areas"),
                ],
                duration_hours=4,
    ...
    def _check_condition_alerts(self) -> list[WeatherAlert]:
        ...
        if any(keyword in condition for keyword in ["rain", "drizzle", "shower"]):
            alerts.append(WeatherAlert(
                ...
                severity=WeatherSeverity.MODERATE,
                title=self._get_translation("weather.alerts.wet_weather_advisory.title"),
                message=self._get_translation("weather.alerts.wet_weather_advisory.message"),
                recommendations=[
                    self._get_translation("weather.recommendations.dry_paws_thoroughly"),
                    self._get_translation("weather.recommendations.check_toe_irritation"),
                    self._get_translation("weather.recommendations.use_paw_balm"),
                    self._get_translation("weather.recommendations.waterproof_protection"),
                ],
                duration_hours=2,
            ))
        ...
        if any(keyword in condition for keyword in ["storm", "thunder", "lightning"]):
            alerts.append(WeatherAlert(
                ...
                message=self._get_translation("weather.alerts.storm_warning.message"),
                recommendations=[
                    self._get_translation("weather.recommendations.keep_indoors_storm"),
                    self._get_translation("weather.recommendations.comfort_anxious"),
                    self._get_translation("weather.recommendations.secure_id_tags"),
                    self._get_translation("weather.recommendations.avoid_until_passes"),
                ],
                duration_hours=3,
        ...
        if any(keyword in condition for keyword in ["snow", "ice", "sleet"]):
            alerts.append(WeatherAlert(
                ...
                message=self._get_translation("weather.alerts.snow_ice_alert.message"),
                recommendations=[
                    self._get_translation("weather.recommendations.use_paw_protection"),
                    self._get_translation("weather.recommendations.watch_ice_buildup"),
                    self._get_translation("weather.recommendations.rinse_salt_chemicals"),
                    self._get_translation("weather.recommendations.provide_traction"),
                ],
                duration_hours=4,
            ))
    ...
    def get_recommendations_for_dog(
        ...
        if not active_alerts:
            recommendations.append("Weather conditions are suitable for normal activities")
            return recommendations
        ...
        for alert in active_alerts:
            ...
            if dog_breed:
                ...
                if (breed_lower in alert.affected_breeds or 
                    any(breed_type in breed_lower for breed_type in alert.affected_breeds)):
            ...
            if dog_age_months is not None:
                if dog_age_months < 12 and "puppies" in alert.age_considerations:
                    recommendations.append(
                        self._get_translation("weather.recommendations.puppy_extra_monitoring")
                    )
                elif dog_age_months > 84 and "senior_dogs" in alert.age_considerations:
                    recommendations.append(
                        self._get_translation("weather.recommendations.senior_extra_protection")
                    )
            ...
            if health_conditions:
                for condition in health_conditions:
                    if condition.lower() in ["respiratory", "breathing", "asthma"]:
                        if alert.alert_type in [WeatherHealthImpact.RESPIRATORY_RISK, WeatherHealthImpact.AIR_QUALITY]:
                            recommendations.append(
                                self._get_translation("weather.recommendations.respiratory_monitoring")
                            )
                    elif condition.lower() in ["heart", "cardiac"]:
                        if alert.alert_type in [WeatherHealthImpact.HEAT_STRESS, WeatherHealthImpact.EXERCISE_LIMITATION]:
                            recommendations.append(
                                self._get_translation("weather.recommendations.heart_avoid_strenuous")
                            )
