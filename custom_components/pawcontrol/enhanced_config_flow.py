...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_select_dog_edit(
        ...
        schema = vol.Schema({
            vol.Required("dog_to_edit"): vol.In(dog_options)
        })
    ...
    async def async_step_select_dog_remove(
        ...
        if user_input is not None:
            ...
            new_data[CONF_DOGS] = [
                dog for dog in current_dogs 
                if dog.get(CONF_DOG_ID) != dog_id_to_remove
            ]
        ...
        schema = vol.Schema({
            vol.Required("dog_to_remove"): vol.In(dog_options)
        })
    ...
    async def async_step_gps(
        ...
        schema = vol.Schema({
            ...
            vol.Optional(
                "gps_distance_filter",
                default=current_gps.get("distance_filter", GPS_POINT_FILTER_DISTANCE),
    ...
    async def async_step_geofence(
        ...
        schema = vol.Schema({
            ...
            ): vol.All(vol.Coerce(int), vol.Range(min=MIN_SAFE_ZONE_RADIUS, max=MAX_SAFE_ZONE_RADIUS)),
            vol.Optional(
    ...
    async def async_step_notifications(
        ...
        schema = vol.Schema({
            ...
            vol.Optional(
                "reminder_repeat_min",
                default=current_notifications.get("reminder_repeat_min", DEFAULT_REMINDER_REPEAT),
    ...
    def _update_dog_config(self, dog: dict[str, Any], user_input: dict[str, Any]) -> None:
    ...
    def _get_dog_config_schema(self, dog: dict[str, Any] | None = None) -> vol.Schema:
        ...
        return vol.Schema({
            ...
            vol.Required(CONF_DOG_NAME, default=defaults.get(CONF_DOG_NAME, "")): str,
            vol.Optional(CONF_DOG_BREED, default=defaults.get(CONF_DOG_BREED, "")): str,
            vol.Optional(CONF_DOG_AGE, default=defaults.get(CONF_DOG_AGE, 1)): vol.All(
                vol.Coerce(int), vol.Range(min=MIN_DOG_AGE_YEARS, max=MAX_DOG_AGE_YEARS)
            ),
            vol.Optional(CONF_DOG_WEIGHT, default=defaults.get(CONF_DOG_WEIGHT, 20.0)): vol.All(
                vol.Coerce(float), vol.Range(min=MIN_DOG_WEIGHT_KG, max=MAX_DOG_WEIGHT_KG)
            ),
            vol.Optional(CONF_DOG_SIZE, default=defaults.get(CONF_DOG_SIZE, SIZE_MEDIUM)): vol.In([
                SIZE_SMALL, SIZE_MEDIUM, SIZE_LARGE, SIZE_XLARGE
            ]),
            # Module toggles
            vol.Optional(f"module_{MODULE_WALK}", default=current_modules.get(MODULE_WALK, True)): bool,
            vol.Optional(f"module_{MODULE_FEEDING}", default=current_modules.get(MODULE_FEEDING, True)): bool,
            vol.Optional(f"module_{MODULE_HEALTH}", default=current_modules.get(MODULE_HEALTH, True)): bool,
            vol.Optional(f"module_{MODULE_GPS}", default=current_modules.get(MODULE_GPS, True)): bool,
            vol.Optional(f"module_{MODULE_NOTIFICATIONS}", default=current_modules.get(MODULE_NOTIFICATIONS, True)): bool,
            vol.Optional(f"module_{MODULE_DASHBOARD}", default=current_modules.get(MODULE_DASHBOARD, True)): bool,
            vol.Optional(f"module_{MODULE_GROOMING}", default=current_modules.get(MODULE_GROOMING, True)): bool,
            vol.Optional(f"module_{MODULE_MEDICATION}", default=current_modules.get(MODULE_MEDICATION, True)): bool,
            vol.Optional(f"module_{MODULE_TRAINING}", default=current_modules.get(MODULE_TRAINING, True)): bool,
        })
