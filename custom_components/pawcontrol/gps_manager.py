...
class GPSPoint:
    ...
    def is_accurate(self) -> bool:
        ...
        return self.accuracy_level in [GPSAccuracy.EXCELLENT, GPSAccuracy.GOOD, GPSAccuracy.FAIR]
...
class GeofenceEvent:
    ...
    def severity(self) -> str:
        ...
        if self.event_type == GeofenceEventType.BREACH:
            if self.duration_outside and self.duration_outside.total_seconds() > 1800:  # 30 min
                return "high"
            return "medium"
        elif self.event_type == GeofenceEventType.EXITED and self.zone.zone_type == "safe_zone":
            return "medium"
        return "low"
...
class GPSGeofenceManager:
    ...
    def __init__(self, hass: HomeAssistant) -> None:
        ...
        self._zone_status: dict[str, dict[str, bool]] = {}  # dog_id -> zone_name -> inside
    ...
    async def async_add_gps_point(
        ...
        try:
            ...
            if route:
                ...
                if (config and route.gps_points and 
                    config.min_distance_for_point > 0):
    ...
    async def _update_location_from_device_tracker(self, dog_id: str) -> None:
        ...
        try:
            ...
            for entity in entity_registry.entities.values():
                if (entity.platform == "device_tracker" and 
                    dog_id.lower() in (entity.name or "").lower()):
            ...
            _LOGGER.debug("Failed to update location from device tracker for %s: %s", dog_id, err)
    ...
    async def _check_geofence_zones(self, dog_id: str, gps_point: GPSPoint) -> None:
        ...
        for zone in zones:
            ...
            if is_inside != was_inside:
                event_type = GeofenceEventType.ENTERED if is_inside else GeofenceEventType.EXITED
    ...
    async def _send_geofence_notification(self, event: GeofenceEvent) -> None:
        ...
        try:
            ...
            if event.event_type == GeofenceEventType.ENTERED:
                ...
                duration_min = event.duration_outside.total_seconds() / 60 if event.duration_outside else 0
    ...
    async def _update_route_with_new_point(self, route: WalkRoute, new_point: GPSPoint) -> None:
    ...
    async def _export_routes_gpx(self, dog_id: str, routes: list[WalkRoute]) -> dict[str, Any]:
        ...
        for route in routes:
            gpx_content += f'  <trk>\n'
            gpx_content += f'    <name>Walk {route.start_time.strftime("%Y-%m-%d %H:%M")}</name>\n'
    ...
    async def _export_routes_json(self, dog_id: str, routes: list[WalkRoute]) -> dict[str, Any]:
    ...
    async def _export_routes_csv(self, dog_id: str, routes: list[WalkRoute]) -> dict[str, Any]:
