    ...
    if isinstance(guard_metrics, Mapping):
        ...
        if isinstance(reasons_raw, Mapping):
            for reason_key, count in reasons_raw.items():
                ...
                classified_errors[classification] = classified_errors.get(classification, 0) + coerced
        ...
        if isinstance(services, Mapping):
            for service_name, service_payload in services.items():
                ...
                if reason_text is not None:
                    notification_reasons[reason_text] = notification_reasons.get(reason_text, 0) + failures
                classification = classify_error_reason(reason_text, error=error_text)
                classified_errors[classification] = classified_errors.get(classification, 0) + failures
