...
class PawControlDiscovery:
    ...
    async def async_discover_devices(
        ...
        _LOGGER.info("Starting %s device discovery for categories: %s", 
                    "quick" if quick_scan else "thorough", categories)
    ...
    def get_discovered_devices(
        ...
        if category:
            return [
                device for device in self._discovered_devices.values()
