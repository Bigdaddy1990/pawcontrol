...
class PawControlNotificationManager:
    ...
    if notification.notification_services:
      ...
      for service_name in notification.notification_services:
        try:
          ...
          guard_result = await async_call_hass_service_if_available(
            ...
            service_name,
            service_data,
            description=(f"mobile notification {notification.id} for {service_name}"),
            logger=_LOGGER,
          )
      ...
      if not self._notify_service_available(mobile_service):
        ...
        guard_result = await async_call_hass_service_if_available(
          ...
          description=(f"fallback notification {notification.id} for {mobile_service}"),
          logger=_LOGGER,
        )
        if guard_result.executed:
  ...
  async def _send_media_player_notification(
    ...
    await async_call_hass_service_if_available(
      ...
      description=(f"media player announcement {notification.id} on {media_player}"),
      logger=_LOGGER,
    )
    ...
    async with self._lock:
      ...
      active_notifications = len(
        [
          ...
          if not n.acknowledged and (not n.expires_at or n.expires_at > dt_util.now())
        ],
      )
