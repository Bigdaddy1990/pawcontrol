...
if TYPE_CHECKING:
    from .feeding_manager import FeedingComplianceResult
...
class PawControlNotificationManager:
    ...
    async def _send_webhook_notification(self, notification: NotificationEvent) -> None:
        ...
        if secret:
            ...
            signature, signed_timestamp = authenticator.generate_signature(
                payload_bytes,
                timestamp=timestamp,
            )
            headers[f"{header_prefix}-Signature"] = signature
            headers[f"{header_prefix}-Timestamp"] = str(signed_timestamp)
