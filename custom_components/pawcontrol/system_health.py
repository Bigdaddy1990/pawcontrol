...
async def async_system_health_info(hass: HomeAssistant) -> dict[str, Any]:
    ...
    for entry in entries:
        ...
        coordinator_health = "healthy" if coordinator.last_update_success else "unhealthy"
        last_update = coordinator.last_update_time.isoformat() if hasattr(coordinator, "last_update_time") else "never"
        ...
        entry_info = {
            f"entry_{entry.entry_id[:8]}": {
                "dogs": dogs_count,
                "active_modules": ", ".join(sorted(active_modules)) if active_modules else "none",
                ...
                "visitor_mode": coordinator.visitor_mode if hasattr(coordinator, "visitor_mode") else False,
                "emergency_mode": coordinator.emergency_mode if hasattr(coordinator, "emergency_mode") else False,
            }
        }
    ...
    if not entries:
        ...
        all_healthy = all(
            getattr(getattr(entry, "runtime_data", None), "coordinator", None) and
            getattr(getattr(entry, "runtime_data", None), "coordinator", None).last_update_success
            for entry in entries
        )
    ...
    if recommendations:
        health_info["recommendations"] = " | ".join(recommendations[:3])  # Limit to 3 recommendations
