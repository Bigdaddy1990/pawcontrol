    ...
    async def async_step_push_settings(
        ...
        if user_input is not None:
            ...
            mutable[CONF_MQTT_ENABLED] = bool(
                user_input.get(
                    CONF_MQTT_ENABLED,
                    current.get(CONF_MQTT_ENABLED, DEFAULT_MQTT_ENABLED),
                ),
            )
            ...
            if isinstance(topic, str) and topic.strip():
                ...
                mutable[CONF_MQTT_TOPIC] = current.get(
        ...
        schema = vol.Schema({
            ...
            vol.Optional(
                ...
                default=int(
                    current.get(
                        CONF_PUSH_PAYLOAD_MAX_BYTES, DEFAULT_PUSH_PAYLOAD_MAX_BYTES
                    )
                ),
            ...
            vol.Optional(
                ...
                default=int(
                    current.get(
                        CONF_PUSH_NONCE_TTL_SECONDS, DEFAULT_PUSH_NONCE_TTL_SECONDS
                    )
                ),
            ): selector.NumberSelector(
                selector.NumberSelectorConfig(
