...
class ConfigMigrator:
    ...
    async def migrate_config_entry(
        ...
        try:
            ...
            for from_version, to_version in migration_path:
                ...
                if migration_func:
                    try:
                        migration_result = await migration_func(
                        ...
                        result.errors.append(
                            f"Migration {from_version} -> {to_version} failed: {err}"
                        )
        ...
        if CONF_DOGS in data:
            for dog in data[CONF_DOGS]:
                if "modules" not in dog:
                    ...
                    changes.append(
                        f"Added default modules for dog {dog.get(CONF_DOG_NAME, 'unknown')}"
                    )
        ...
        if CONF_DOGS in data:
            for dog in data[CONF_DOGS]:
                ...
                if "modules" not in dog:
                    ...
                    changes.append(
                        f"Added complete module configuration for {dog[CONF_DOG_NAME]}"
                    )
                else:
                    for module, default_value in default_modules.items():
                        if module not in dog["modules"]:
                            ...
                            changes.append(
                                f"Added missing module '{module}' for {dog[CONF_DOG_NAME]}"
                            )
    ...
    async def _restore_from_backup(
        self, config_entry: ConfigEntry, backup_data: Dict[str, Any]
    ) -> None:
    ...
    async def _store_migration_record(
        self, entry_id: str, result: MigrationResult
    ) -> None:
...
class PerformanceMonitor:
    ...
    async def _check_performance_issues(self, metrics: PerformanceMetrics) -> None:
        ...
        if issues:
            _LOGGER.warning(
                "Performance issues detected for %s: %s",
                metrics.config_entry_id,
                "; ".join(issues),
            )
    ...
    async def generate_performance_report(self, entry_id: str) -> Dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                "statistics": {
                    "avg_update_duration_ms": sum(update_durations)
                    / len(update_durations),
                    ...
                    "avg_gps_points": sum(m.gps_points_processed for m in history)
                    / len(history),
    ...
    def _generate_recommendations(
        self, history: List[PerformanceMetrics], current: Optional[PerformanceMetrics]
    ) -> List[str]:
...
class HealthChecker:
    ...
    async def _check_coordinator_status(
        self, config_entry: ConfigEntry
    ) -> Dict[str, Any]:
        """Check coordinator status."""
        check_result = {
        ...
        try:
            ...
            if hasattr(coordinator, "last_update"):
                ...
                if last_update:
                    ...
                    check_result["details"]["time_since_update_seconds"] = (
                    ...
                    if time_since_update.total_seconds() > 3600:  # 1 hour
                        check_result["status"] = "warning"
                        check_result["issues"].append(
                            "Coordinator hasn't updated in over an hour"
                        )
    ...
    async def _check_entity_status(self, config_entry: ConfigEntry) -> Dict[str, Any]:
        ...
        try:
            ...
            if len(disabled_entities) > 0:
                check_result["status"] = "warning"
                check_result["issues"].append(
                    f"{len(disabled_entities)} entities are disabled"
                )
            ...
            if unavailable_count > 0:
                check_result["status"] = "warning"
                check_result["issues"].append(
                    f"{unavailable_count} entities are unavailable"
                )
    ...
    async def _check_gps_functionality(
        self, config_entry: ConfigEntry
    ) -> Dict[str, Any]:
        """Check GPS functionality."""
        check_result = {
        ...
        try:
            ...
            check_result["details"]["route_recording"] = gps_options.get(
                "route_recording", True
            )
        ...
        try:
            ...
            for entity_id in person_entities:
                ...
                if not state:
                    check_result["status"] = "warning"
                    check_result["issues"].append(
                        f"Person entity not found: {entity_id}"
                    )
            ...
            for entity_id in device_trackers:
                ...
                if not state:
                    check_result["status"] = "warning"
                    check_result["issues"].append(
                        f"Device tracker not found: {entity_id}"
                    )
            ...
            if door_sensor:
                ...
                if not state:
                    check_result["status"] = "warning"
                    check_result["issues"].append(
                        f"Door sensor not found: {door_sensor}"
                    )
                else:
