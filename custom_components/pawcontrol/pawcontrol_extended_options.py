...
class OptionsFlowHandler(config_entries.OptionsFlow):
    ...
    async def async_step_edit_dog(
        ...
        schema = vol.Schema({
            vol.Required(CONF_DOG_NAME, default=dog_to_edit.get(CONF_DOG_NAME, "")): str,
            vol.Optional(CONF_DOG_BREED, default=dog_to_edit.get(CONF_DOG_BREED, "")): str,
            vol.Optional(CONF_DOG_AGE, default=dog_to_edit.get(CONF_DOG_AGE, 1)): vol.All(
                vol.Coerce(int), vol.Range(min=MIN_DOG_AGE_YEARS, max=MAX_DOG_AGE_YEARS)
            ),
            vol.Optional(CONF_DOG_WEIGHT, default=dog_to_edit.get(CONF_DOG_WEIGHT, 20.0)): vol.All(
                vol.Coerce(float), vol.Range(min=MIN_DOG_WEIGHT_KG, max=MAX_DOG_WEIGHT_KG)
            ),
            vol.Optional(CONF_DOG_SIZE, default=dog_to_edit.get(CONF_DOG_SIZE, SIZE_MEDIUM)): vol.In([
                SIZE_SMALL, SIZE_MEDIUM, SIZE_LARGE, SIZE_XLARGE
            ]),
    ...
    async def _async_backup_configuration(self) -> FlowResult:
        ...
        try:
            ...
            backup_path = self.hass.config.path(f"pawcontrol_backup_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json")
