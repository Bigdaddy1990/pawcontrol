...
class HealthCalculator:
    ...
    def calculate_life_stage(age_months: int, breed_size: str = "medium") -> LifeStage:
        ...
        size_adjustments = {
            ...
            "medium": {"adult": 15, "senior": 84},    # 15 months to adult, 7 years to senior
            "large": {"adult": 18, "senior": 72},     # 18 months to adult, 6 years to senior
    ...
    def calculate_daily_calories(
        ...
        base_multiplier = HealthCalculator.BASE_CALORIE_REQUIREMENTS.get(
            life_stage, 95
        ) / 95  # Normalize to adult baseline
    ...
    def validate_portion_safety(
        ...
        safety_result = {
            ...
            "warnings": [],
            "recommendations": [],
            "portion_per_kg": round(calculated_portion / dog_weight, 1) if dog_weight > 0 else 0
        }
    ...
    def get_diet_interaction_effects(
        ...
        interactions["risk_level"] = (
            "high" if interactions["conflicting"] else
            "medium" if interactions["caution"] else
            "low"
        )
    ...
    def analyze_feeding_history(
        ...
        # Calculate averages
        avg_daily_calories = sum(day["calories"] for day in recent_days.values()) / len(recent_days)
        avg_daily_meals = sum(day["meals"] for day in recent_days.values()) / len(recent_days)
        ...
        if calorie_variance > 20:
            ...
            recommendations.append("Feeding is well balanced, maintain current portions")
    ...
    def generate_health_report(health_metrics: HealthMetrics) -> Dict[str, Any]:
        ...
        if health_metrics.body_condition_score:
            ...
            if bcs in [BodyConditionScore.EMACIATED, BodyConditionScore.VERY_THIN]:
                ...
                report["recommendations"].append("Immediate veterinary consultation needed")
        ...
        serious_conditions = ["diabetes", "heart_disease", "kidney_disease", "liver_disease", "cancer"]
        for condition in health_metrics.health_conditions:
            if condition.lower() in serious_conditions:
                ...
                report["recommendations"].append(f"Continue monitoring {condition} as prescribed")
                report["health_score"] -= 15
        ...
        if health_metrics.age_months:
            ...
            if age_years < 1:
                report["recommendations"].append("Ensure puppy vaccination schedule is complete")
                report["recommendations"].append("Monitor growth rate and adjust portions accordingly")
            elif age_years > 7:
        ...
        if health_metrics.activity_level == ActivityLevel.VERY_LOW:
            report["recommendations"].append("Gradually increase physical activity if health permits")
