...
ENTITY_PROFILES = {
    "basic": {
        ...
        "modules": {MODULE_FEEDING: True, MODULE_WALK: True, MODULE_HEALTH: False, MODULE_GPS: False},
    },
    "standard": {
        "max_entities": 12,
        "description": "Balanced monitoring with GPS",
        "modules": {MODULE_FEEDING: True, MODULE_WALK: True, MODULE_HEALTH: True, MODULE_GPS: True},
    },
    "advanced": {
        "max_entities": 18,
    ...
    "gps_focus": {
        ...
        "modules": {MODULE_FEEDING: False, MODULE_WALK: True, MODULE_HEALTH: False, MODULE_GPS: True},
    },
    "health_focus": {
        "max_entities": 10,
        "description": "Health monitoring focused",
        "modules": {MODULE_FEEDING: True, MODULE_WALK: False, MODULE_HEALTH: True, MODULE_GPS: False},
    },
}
...
class EntityFactory:
    ...
    def create_entities_for_dog(
        ...
        _LOGGER.info(
            ...
            dog_name, dog_id, profile, max_entities
        )
        ...
        if modules.get(MODULE_FEEDING, False):
            entity_candidates.extend(self._create_feeding_entities(dog_id, dog_name, profile))
        
        if modules.get(MODULE_WALK, False):
            entity_candidates.extend(self._create_walk_entities(dog_id, dog_name, profile))
        
        if modules.get(MODULE_GPS, False):
            entity_candidates.extend(self._create_gps_entities(dog_id, dog_name, profile))
        
        if modules.get(MODULE_HEALTH, False):
            entity_candidates.extend(self._create_health_entities(dog_id, dog_name, profile))
        ...
        _LOGGER.info(
            ...
            len(entities), len(entity_candidates), dog_name,
            ", ".join(selected_types)
        )
    ...
    def _create_core_entities(self, dog_id: str, dog_name: str) -> list[dict[str, Any]]:
        ...
        return [
            ...
            {
                "entity": PawControlLastActionSensor(self.coordinator, dog_id, dog_name),
                "type": "last_action",
                "priority": ENTITY_PRIORITIES["last_action"],
            },
            {
                "entity": PawControlActivityScoreSensor(self.coordinator, dog_id, dog_name),
                "type": "activity_score",
    ...
    def _create_feeding_entities(
        ...
        entities.extend([
            {
                "entity": PawControlLastFeedingSensor(self.coordinator, dog_id, dog_name),
                "type": "last_feeding",
                "priority": ENTITY_PRIORITIES["last_feeding"],
            },
            {
                "entity": PawControlFeedingScheduleAdherenceSensor(self.coordinator, dog_id, dog_name),
                "type": "feeding_schedule_adherence",
                "priority": ENTITY_PRIORITIES["feeding_schedule_adherence"],
            },
            {
                "entity": PawControlHealthAwarePortionSensor(self.coordinator, dog_id, dog_name),
        ...
        if profile in ["standard", "advanced", "health_focus"]:
            entities.extend([
                {
                    "entity": PawControlTotalFeedingsTodaySensor(self.coordinator, dog_id, dog_name),
                    "type": "total_feedings_today",
                    "priority": ENTITY_PRIORITIES["total_feedings_today"],
                },
                {
                    "entity": PawControlDailyCaloriesSensor(self.coordinator, dog_id, dog_name),
                    "type": "daily_calories",
                    "priority": ENTITY_PRIORITIES["daily_calories"],
                },
                {
                    "entity": PawControlFeedingRecommendationSensor(self.coordinator, dog_id, dog_name),
        ...
        if profile == "advanced":
            entities.extend([
                {
                    "entity": PawControlDietValidationStatusSensor(self.coordinator, dog_id, dog_name),
                    "type": "diet_validation_status",
                    "priority": ENTITY_PRIORITIES["diet_validation_status"],
                },
                # Add detailed meal sensors for advanced users
                {
                    "entity": PawControlFeedingCountTodaySensor(self.coordinator, dog_id, dog_name, "breakfast"),
                    "type": "feeding_count_today_breakfast",
                    "priority": ENTITY_PRIORITIES["feeding_count_today_breakfast"],
                },
                {
                    "entity": PawControlFeedingCountTodaySensor(self.coordinator, dog_id, dog_name, "dinner"),
                    "type": "feeding_count_today_dinner",
                    "priority": ENTITY_PRIORITIES["feeding_count_today_dinner"],
                },
                {
                    "entity": PawControlMealPortionSensor(self.coordinator, dog_id, dog_name, "breakfast"),
                    "type": "breakfast_portion",
                    "priority": ENTITY_PRIORITIES["breakfast_portion"],
                },
                {
                    "entity": PawControlMealPortionSensor(self.coordinator, dog_id, dog_name, "dinner"),
                    "type": "dinner_portion",
    ...
    def _create_walk_entities(
        ...
        entities.extend([
            {
                "entity": PawControlLastWalkSensor(self.coordinator, dog_id, dog_name),
                "type": "last_walk",
                "priority": ENTITY_PRIORITIES["last_walk"],
            },
            {
                "entity": PawControlWalkCountTodaySensor(self.coordinator, dog_id, dog_name),
        ...
        if profile in ["standard", "advanced", "gps_focus"]:
            entities.extend([
                {
                    "entity": PawControlLastWalkDurationSensor(self.coordinator, dog_id, dog_name),
                    "type": "last_walk_duration",
                    "priority": ENTITY_PRIORITIES["last_walk_duration"],
                },
                {
                    "entity": PawControlTotalWalkTimeTodaySensor(self.coordinator, dog_id, dog_name),
        ...
        if profile == "advanced":
            entities.extend([
                {
                    "entity": PawControlWeeklyWalkCountSensor(self.coordinator, dog_id, dog_name),
                    "type": "weekly_walk_count",
                    "priority": ENTITY_PRIORITIES["weekly_walk_count"],
                },
                {
                    "entity": PawControlAverageWalkDurationSensor(self.coordinator, dog_id, dog_name),
    ...
    def _create_gps_entities(
        ...
        entities.extend([
            {
                "entity": PawControlCurrentZoneSensor(self.coordinator, dog_id, dog_name),
                "type": "current_zone",
                "priority": ENTITY_PRIORITIES["current_zone"],
            },
            {
                "entity": PawControlDistanceFromHomeSensor(self.coordinator, dog_id, dog_name),
        ...
        if profile in ["standard", "advanced", "gps_focus"]:
            entities.extend([
                {
                    "entity": PawControlCurrentSpeedSensor(self.coordinator, dog_id, dog_name),
                    "type": "current_speed",
                    "priority": ENTITY_PRIORITIES["current_speed"],
                },
                {
                    "entity": PawControlGPSAccuracySensor(self.coordinator, dog_id, dog_name),
                    "type": "gps_accuracy",
        ...
        if profile in ["advanced", "gps_focus"]:
            entities.extend([
                {
                    "entity": PawControlTotalDistanceTodaySensor(self.coordinator, dog_id, dog_name),
                    "type": "total_distance_today",
                    "priority": ENTITY_PRIORITIES["total_distance_today"],
                },
                {
                    "entity": PawControlGPSBatteryLevelSensor(self.coordinator, dog_id, dog_name),
    ...
    def _create_health_entities(
        ...
        entities.extend([
            {
                "entity": PawControlHealthStatusSensor(self.coordinator, dog_id, dog_name),
                "type": "health_status",
                "priority": ENTITY_PRIORITIES["health_status"],
            },
            {
                "entity": PawControlWeightSensor(self.coordinator, dog_id, dog_name),
                "type": "weight",
        ...
        if profile in ["standard", "advanced", "health_focus"]:
            entities.extend([
                {
                    "entity": PawControlBodyConditionScoreSensor(self.coordinator, dog_id, dog_name),
                    "type": "body_condition_score",
                    "priority": ENTITY_PRIORITIES["body_condition_score"],
                },
                {
                    "entity": PawControlWeightTrendSensor(self.coordinator, dog_id, dog_name),
                    "type": "weight_trend",
        ...
        if profile in ["advanced", "health_focus"]:
            entities.extend([
                {
                    "entity": PawControlLastVetVisitSensor(self.coordinator, dog_id, dog_name),
                    "type": "last_vet_visit",
