...
class WalkManager:
    ...
    async def async_update_gps_data(
        ...
        if not self._validate_coordinates(latitude, longitude):
            _LOGGER.warning("Invalid GPS coordinates for %s: %f, %f", dog_id, latitude, longitude)
            return False
        ...
        async with self._data_lock:
            ...
            if (self._gps_data[dog_id]["latitude"] is not None and 
                self._gps_data[dog_id]["longitude"] is not None):
                old_location = (
            ...
            if old_location and self._walk_detection_enabled:
                await self._process_walk_detection(dog_id, old_location, (latitude, longitude))
            
            _LOGGER.debug("Updated GPS data for %s: %f, %f (accuracy: %s)", 
                         dog_id, latitude, longitude, accuracy)
            return True

    async def async_start_walk(self, dog_id: str, walk_type: str = "manual") -> Optional[str]:
    ...
    async def async_end_walk(self, dog_id: str) -> Optional[Dict[str, Any]]:
        ...
        async with self._data_lock:
            ...
            if walk_data["path"]:
                walk_data["distance"] = self._calculate_total_distance(walk_data["path"])
                ...
                walk_data["calories_burned"] = self._estimate_calories_burned(dog_id, walk_data)
            ...
            _LOGGER.info("Completed walk for %s: %.1fm in %.0fs", 
                        dog_id, walk_data["distance"], duration)
            return walk_data
    ...
    async def async_get_current_walk(self, dog_id: str) -> Optional[Dict[str, Any]]:
        ...
        """
        async with self._data_lock:
            return self._current_walks.get(dog_id, {}).copy() if dog_id in self._current_walks else None
    ...
    async def _process_walk_detection(
        ...
        if (distance > 10 and  # Moved more than 10 meters
            dog_id not in self._current_walks and
            self._gps_data[dog_id].get("speed", 0) > 1):  # Speed > 1 km/h
    ...
    async def _update_daily_walk_stats(self, dog_id: str, walk_data: Dict[str, Any]) -> None:
        ...
        self._walk_data[dog_id]["weekly_distance"] = sum(w.get("distance", 0) for w in week_walks)
    ...
    def _calculate_max_speed(self, path: List[Dict[str, Any]]) -> Optional[float]:
        ...
        """
        speeds = [point.get("speed") for point in path if point.get("speed") is not None]
        return max(speeds) if speeds else None

    def _estimate_calories_burned(self, dog_id: str, walk_data: Dict[str, Any]) -> Optional[float]:
    ...
    async def async_get_statistics(self) -> Dict[str, Any]:
        ...
        async with self._data_lock:
            ...
            dogs_with_gps = sum(1 for gps in self._gps_data.values() if gps["available"])
            active_walks = len(self._current_walks)
            
            total_walks_today = sum(data["walks_today"] for data in self._walk_data.values())
            total_distance_today = sum(data["total_distance_today"] for data in self._walk_data.values())
