...
class PawControlDataStorage:
    ...
    async def async_load_all_data(self) -> Dict[str, Any]:
        ...
        try:
            ...
            load_tasks = [
                self._load_store_data(store_key)
            ...
            for store_key, result in zip(self._stores.keys(), results):
                if isinstance(result, Exception):
                    _LOGGER.error(
                        "Failed to load %s data: %s", store_key, result
                    )
                    data[store_key] = {}
                else:
    ...
    async def async_cleanup_old_data(self, retention_days: int = 90) -> None:
        ...
        for store_key in self._stores.keys():
            try:
                ...
                _LOGGER.error(
                    "Failed to cleanup %s data: %s", store_key, err
                )
...
class PawControlData:
    ...
    async def async_load_data(self) -> None:
        ...
        try:
            ...
            _LOGGER.debug("Data manager initialized with %d data types", len(self._data))
        except Exception as err:
    ...
    async def async_log_feeding(
        ...
        try:
            ...
            # Fire event
            self.hass.bus.async_fire(EVENT_FEEDING_LOGGED, {
                "dog_id": dog_id,
                **feeding_data,
            })
    ...
    async def async_start_walk(
        ...
        try:
            ...
            # Fire event
            self.hass.bus.async_fire(EVENT_WALK_STARTED, {
                "dog_id": dog_id,
                **walk_data,
            })
    ...
    async def async_end_walk(self, dog_id: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # Fire event
            self.hass.bus.async_fire(EVENT_WALK_ENDED, {
                "dog_id": dog_id,
                **completed_walk,
            })
    ...
    async def async_log_health(
        ...
        try:
            ...
            # Fire event
            self.hass.bus.async_fire(EVENT_HEALTH_LOGGED, {
                "dog_id": dog_id,
                **health_data,
            })
        ...
        try:
            ...
            for dog_id in [dog[CONF_DOG_ID] for dog in self._dogs]:
                ...
                if "daily" in stats_data[dog_id]:
                    ...
                    stats_data[dog_id]["history"][yesterday] = stats_data[dog_id]["daily"]
