...
class GPSLogic:
    ...
    async def _setup_person_tracking(self, person_entity: str) -> None:
        ...
        def person_changed(entity_id: str, old_state: State, new_state: State):
            ...
            if old_state and old_state.state != new_state.state:
                ...
                elif old_state.state == STATE_NOT_HOME and new_state.state == STATE_HOME:
    ...
    def _update_location(self, entity_id: str, latitude: float, longitude: float) -> None:
        ...
        for dog_id in dogs:
            if dog_id in self._walk_sessions:
                ...
                if session.get("confirmed"):
                    session["total_distance"] = session.get("total_distance", 0) + distance
