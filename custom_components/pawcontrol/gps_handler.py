...
def _inc_state(hass, entity_id: str, inc: float, attrs: dict | None = None):
    ...
    try:
        val = float(st.state) if st and st.state not in ("unknown","unavailable") else 0.0
    except Exception:
        val = 0.0
...
class PawControlGPSHandler:
    ...
    def _get(self, dog: str) -> Dict[str, Any]:
        st = self._routes.setdefault(dog, {"active": False, "start": None, "points": [], "distance_m": 0.0})
        self._metrics.setdefault(dog, {"points_total": 0, "points_dropped": 0, "acc_sum": 0.0})
        self._safe_status.setdefault(dog, {"inside": None, "last_ts": None, "enters": 0, "leaves": 0, "time_today": 0.0})
        return st
    ...
    async def async_start_walk(self, walk_type: str | None = None, dog_id: str | None = None) -> None:
        ...
        self.hass.bus.async_fire("pawcontrol_walk_started", {"dog_id": dog, "walk_type": walk_type or "normal"})
        self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_walk_started", _now().isoformat())

    async def async_end_walk(self, rating: int | None = None, notes: str | None = None, dog_id: str | None = None) -> None:
        dog = self._dog_id(dog_id)
        st = self._get(dog)
        end = _now()
        duration_s = (end - (st.get("start") or end)).total_seconds() if st.get("start") else 0.0
        dist_m = float(st.get("distance_m") or 0.0)
        avg_kmh = (dist_m/1000.0) / (duration_s/3600.0) if duration_s > 0 else 0.0
        self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_walk_distance_last", round(dist_m, 1), {"unit_of_measurement": "m"})
        self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_walk_duration_last", int(duration_s), {"unit_of_measurement": "s"})
        self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_walk_avg_speed_last", round(avg_kmh, 2), {"unit_of_measurement": "km/h"})
        self.hass.bus.async_fire("pawcontrol_walk_finished", {"dog_id": dog, "distance_m": dist_m, "duration_s": duration_s, "avg_speed_kmh": avg_kmh, "rating": rating, "notes": notes})
        self.hass.bus.async_fire("pawcontrol_route_summary", {"dog_id": dog, "distance_m": dist_m, "duration_s": duration_s, "points": len(st.get('points') or [])})
        st["active"] = False
        # Persist route summary
        try:
            from .route_store import RouteHistoryStore
            store = RouteHistoryStore(self.hass, getattr(self, "entry_id", "default"), DOMAIN)
            await store.async_add_walk(self.hass, getattr(self, "entry_id", "default"), DOMAIN, dog,
                                       st.get("start").isoformat() if st.get("start") else None,
                                       end.isoformat(), dist_m, duration_s, len(st.get("points") or []),
                                       limit=int((((self.options or {}).get("advanced") or {}).get("route_history_limit") or 500)))
        except Exception:
            pass

    async def async_update_location(self, latitude: float, longitude: float, accuracy: float | None = None, source: str | None = None, dog_id: str | None = None) -> None:
        dog = self._dog_id(dog_id)
        st = self._get(dog)
        ...
        if st.get("active"):
            self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_walk_distance_current", round(st["distance_m"], 1), {"unit_of_measurement": "m"})
        # daily counters
        if st.get("paused"):
            # Still dispatch to tracker & events, but don't accumulate stats
            async_dispatcher_send(self.hass, f"{DOMAIN}_gps_update_{dog}", latitude, longitude, accuracy)
            self.hass.bus.async_fire("pawcontrol_route_point", {"dog_id": dog, "lat": latitude, "lon": longitude, "acc": accuracy, "ts": now_iso})
            return

        # continue with accumulation
        if inc > 0:
            _inc_state(self.hass, f"sensor.{DOMAIN}_{dog}_walk_distance_today", inc, {"unit_of_measurement": "m"})
        # duration accumulation
        try:
            if len(pts) >= 1:
                from datetime import datetime as _dt
                t_prev = _dt.fromisoformat(pts[-1][2].replace("Z", "+00:00")) if isinstance(pts[-1][2], str) else None
                t_now = _now()
                if t_prev:
                    ds = (t_now - t_prev).total_seconds()
                    if st.get("active") and ds > 0:
                        _inc_state(self.hass, f"sensor.{DOMAIN}_{dog}_walk_time_today", ds, {"unit_of_measurement": "s"})
        except Exception:
            pass
        ...
        # GPS metrics
        try:
            mt = self._metrics.get(dog) or {"points_total": 0, "points_dropped": 0, "acc_sum": 0.0}
            ...
            if accuracy is not None:
                ...
                self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_gps_accuracy_avg", round(avg, 1))
            self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_gps_points_total", int(mt["points_total"]))
        ...
        try:
            ...
            if z:
                ...
                inside = dist <= radius
                now_dt = _now()
                stz = self._safe_status.setdefault(dog, {"inside": None, "last_ts": now_dt, "enters": 0, "leaves": 0, "time_today": 0.0})
                ...
                if stz.get("last_ts"):
                    ...
                    if prev_inside is True and delta > 0:
                        stz["time_today"] = float(stz.get("time_today", 0.0)) + float(delta)
                # transition counters
                ...
                stz["last_ts"] = now_dt
                # publish metrics
                self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_time_in_safe_zone_today", int(stz.get("time_today", 0.0)), {"unit_of_measurement": "s"})
                self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_safe_zone_enters_today", int(stz.get("enters", 0)))
                self.hass.states.async_set(f"sensor.{DOMAIN}_{dog}_safe_zone_leaves_today", int(stz.get("leaves", 0)))
                # dispatch to binary_sensor
                async_dispatcher_send(self.hass, f"pawcontrol_safe_zone_update_{dog}", inside, dist, radius)
...
async def async_resume_tracking(self, dog_id: str | None = None) -> None:
    ...
    async def async_export_last_route(self, dog_id: str | None = None, fmt: str = "geojson", to_media: bool = False) -> str | None:
        ...
        dog = self._dog_id(dog_id)
        st = self._get(dog)
        base = self.hass.config.path("media/pawcontrol_routes") if to_media else self.hass.config.path("pawcontrol_routes")
        os.makedirs(base, exist_ok=True)
        if fmt == "geojson":
            feat = {"type": "Feature", "properties": {"dog_id": dog}, "geometry": {"type": "LineString", "coordinates": [[p[1], p[0]] for p in (st.get("points") or [])]}}
    ...
    async def async_generate_diagnostics(self, dog_id: str | None = None) -> str | None:
        ...
        os.makedirs(base, exist_ok=True)
        st = self._get(dog)
        out = {"dog_id": dog, "active": st.get("active"), "distance_m": st.get("distance_m"), "points": st.get("points")[-500:], "options": self.options}
