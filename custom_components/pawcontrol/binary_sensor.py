...
def _create_system_binary_sensors(
    coordinator: PawControlCoordinator, entry: ConfigEntry
...
class WalkInProgressBinarySensor(PawControlBinarySensorEntity, BinarySensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        try:
                ...
                if start_time_str := walk_data.get("walk_start_time"):
                    try:
                        ...
                        if start_time:
                            ...
                            attributes["current_duration_min"] = round(
                                duration_minutes, 1
                            )
                    except (ValueError, TypeError):
                        pass
            ...
            _LOGGER.debug(
                "Error getting walk progress attributes for %s: %s", self.dog_id, err
            )
            return super().extra_state_attributes
...
class IsHungryBinarySensor(PawControlBinarySensorEntity, BinarySensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            _LOGGER.debug(
                "Error getting hunger attributes for %s: %s", self.dog_id, err
            )
            return super().extra_state_attributes
        ...
        try:
            ...
            try:
                ...
                if last_feeding_time:
                    hours_since = (
                        dt_util.utcnow() - last_feeding_time
                    ).total_seconds() / 3600
...
class NeedsMedicationBinarySensor(PawControlBinarySensorEntity, BinarySensorEntity):
    ...
    def is_on(self) -> bool:
        ...
        try:
            ...
            _LOGGER.debug(
                "Error checking medication needs for %s: %s", self.dog_id, err
            )
            return False
...
class NeedsGroomingBinarySensor(PawControlBinarySensorEntity, BinarySensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            _LOGGER.debug(
                "Error getting grooming attributes for %s: %s", self.dog_id, err
            )
            return super().extra_state_attributes
...
class GroomingOverdueBinarySensor(PawControlBinarySensorEntity, BinarySensorEntity):
    ...
    def is_on(self) -> bool:
        ...
        try:
            grooming_data = self.dog_data.get("grooming", {})
            last_grooming = grooming_data.get("last_grooming")
            interval_days = grooming_data.get(
                "grooming_interval_days", GROOMING_OVERDUE_DAYS
            )
            ...
            _LOGGER.debug(
                "Error checking grooming overdue for %s: %s", self.dog_id, err
            )
            return False
...
class IsHomeBinarySensor(PawControlBinarySensorEntity, BinarySensorEntity):
    ...
    def extra_state_attributes(self) -> dict[str, Any] | None:
        ...
        try:
            ...
            attributes.update(
                {
                    ...
                    "time_inside_today_min": location_data.get(
            ...
            _LOGGER.debug(
                "Error getting location attributes for %s: %s", self.dog_id, err
            )
            return super().extra_state_attributes
            ...
            if last_gps_update:
                try:
                    ...
                    if last_update_time:
                        ...
                        minutes_since = (
                            dt_util.utcnow() - last_update_time
                        ).total_seconds() / 60
...
class GeofenceAlertsActiveBinarySensor(
    PawControlBinarySensorEntity, BinarySensorEntity
):
...
class TrainingInProgressBinarySensor(PawControlBinarySensorEntity, BinarySensorEntity):
    ...
    def is_on(self) -> bool:
        ...
        try:
            ...
            _LOGGER.debug(
                "Error checking training progress for %s: %s", self.dog_id, err
            )
            return False
    ...
    def is_on(self) -> bool:
        ...
        try:
            ...
            return (
                ...
                and not self.coordinator.emergency_mode
            )
        except Exception as err:
