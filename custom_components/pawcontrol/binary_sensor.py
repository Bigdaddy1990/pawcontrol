...
async def async_setup_entry(
    ...
    for dog in dogs:
        ...
        if modules.get(MODULE_FEEDING, False):
            entities.extend(_create_feeding_binary_sensors(coordinator, dog_id, dog_name))
        ...
        if modules.get(MODULE_HEALTH, False):
            entities.extend(_create_health_binary_sensors(coordinator, dog_id, dog_name))
    
    # Add all entities at once for better performance
    async_add_entities(entities, update_before_add=True)
    
    _LOGGER.info(
        "Created %d binary sensor entities for %d dogs",
        len(entities),
        len(dogs)
    )
...
class PawControlBinarySensorBase(CoordinatorEntity[PawControlCoordinator], BinarySensorEntity):
    ...
    def extra_state_attributes(self) -> AttributeDict:
        ...
        if dog_data:
            attrs.update({
                ...
                "modified_notifications": dog_data.get("visitor_mode_settings", {}).get("modified_notifications", True),
                "reduced_alerts": dog_data.get("visitor_mode_settings", {}).get("reduced_alerts", True),
            })
