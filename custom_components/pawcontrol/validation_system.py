...
class ConfigValidator:
    ...
    def validate_dog_id(dog_id: str) -> str:
        ...
        if not normalized:
            raise ValidationError("Dog ID contains no valid characters", "dog_id", dog_id)
        
        if len(normalized) < 2:
            raise ValidationError("Dog ID must be at least 2 characters long", "dog_id", normalized)
        
        if len(normalized) > 32:
            raise ValidationError("Dog ID must be at most 32 characters long", "dog_id", normalized)
        
        if not ConfigValidator.DOG_ID_PATTERN.match(normalized):
            raise ValidationError("Dog ID can only contain letters, numbers, and underscores", "dog_id", normalized)
        
        # Must not start with underscore or number
        if normalized[0] in '_0123456789':
            raise ValidationError("Dog ID must start with a letter", "dog_id", normalized)
    ...
    def validate_dog_name(name: str) -> str:
        ...
        if not normalized:
            raise ValidationError("Dog name cannot be empty after trimming", "dog_name", name)
        
        if len(normalized) < 1:
            raise ValidationError("Dog name must be at least 1 character long", "dog_name", normalized)
        
        if len(normalized) > 50:
            raise ValidationError("Dog name must be at most 50 characters long", "dog_name", normalized)
        
        # Check for invalid characters (allow letters, numbers, spaces, hyphens, apostrophes)
        if not re.match(r"^[a-zA-Z0-9\s\-']+$", normalized):
            raise ValidationError("Dog name can only contain letters, numbers, spaces, hyphens, and apostrophes", "dog_name", normalized)
    ...
    def validate_dog_age(age: Union[int, float, str]) -> int:
        ...
        if age_int < MIN_DOG_AGE_YEARS:
            raise ValidationError(f"Age cannot be less than {MIN_DOG_AGE_YEARS} years", "dog_age", age_int)
        
        if age_int > MAX_DOG_AGE_YEARS:
            raise ValidationError(f"Age cannot be more than {MAX_DOG_AGE_YEARS} years", "dog_age", age_int)
    ...
    def validate_dog_weight(weight: Union[int, float, str]) -> float:
        ...
        if weight_float < MIN_DOG_WEIGHT_KG:
            raise ValidationError(f"Weight cannot be less than {MIN_DOG_WEIGHT_KG} kg", "dog_weight", weight_float)
        
        if weight_float > MAX_DOG_WEIGHT_KG:
            raise ValidationError(f"Weight cannot be more than {MAX_DOG_WEIGHT_KG} kg", "dog_weight", weight_float)
    ...
    def validate_dog_size(size: str) -> str:
        ...
        if size not in valid_sizes:
            raise ValidationError(f"Size must be one of: {', '.join(valid_sizes)}", "dog_size", size)
    ...
    def validate_time_string(time_str: str) -> str:
        ...
        if not ConfigValidator.TIME_PATTERN.match(time_str):
            raise ValidationError("Time must be in HH:MM or HH:MM:SS format", "time", time_str)
    ...
    @staticmethod
    def validate_coordinates(lat: Union[float, str], lon: Union[float, str]) -> tuple[float, float]:
        ...
        try:
            ...
            raise ValidationError("Coordinates must be valid numbers", "coordinates", (lat, lon))
        
        if not (-90 <= lat_float <= 90):
            raise ValidationError("Latitude must be between -90 and 90", "latitude", lat_float)
        
        if not (-180 <= lon_float <= 180):
            raise ValidationError("Longitude must be between -180 and 180", "longitude", lon_float)
    ...
    def validate_radius(radius: Union[int, float, str]) -> int:
        ...
        if radius_int < MIN_SAFE_ZONE_RADIUS:
            raise ValidationError(f"Radius cannot be less than {MIN_SAFE_ZONE_RADIUS} meters", "radius", radius_int)
        
        if radius_int > MAX_SAFE_ZONE_RADIUS:
            raise ValidationError(f"Radius cannot be more than {MAX_SAFE_ZONE_RADIUS} meters", "radius", radius_int)
    ...
    def validate_notification_channels(channels: List[str]) -> List[str]:
        ...
        if not isinstance(channels, list):
            raise ValidationError("Notification channels must be a list", "notification_channels", channels)
        
        if not channels:
            raise ValidationError("At least one notification channel must be selected", "notification_channels", channels)
        
        invalid_channels = [ch for ch in channels if ch not in valid_channels]
        if invalid_channels:
            raise ValidationError(f"Invalid notification channels: {', '.join(invalid_channels)}", "notification_channels", invalid_channels)
    ...
    def validate_entity_id(entity_id: str) -> str:
        ...
        if not entity_pattern.match(entity_id):
            raise ValidationError("Entity ID must be in format 'domain.object_id'", "entity_id", entity_id)
...
class SchemaBuilder:
    ...
    def dog_config_schema(existing_dog: Dict[str, Any] | None = None) -> vol.Schema:
        ...
        return vol.Schema({
            vol.Required("dog_id", default=defaults.get("dog_id", "")): ConfigValidator.validate_dog_id,
            vol.Required("dog_name", default=defaults.get("dog_name", "")): ConfigValidator.validate_dog_name,
            vol.Optional("dog_breed", default=defaults.get("dog_breed", "")): cv.string,
            vol.Optional("dog_age", default=defaults.get("dog_age", 1)): ConfigValidator.validate_dog_age,
            vol.Optional("dog_weight", default=defaults.get("dog_weight", 20.0)): ConfigValidator.validate_dog_weight,
            vol.Optional("dog_size", default=defaults.get("dog_size", SIZE_MEDIUM)): ConfigValidator.validate_dog_size,
            # Module configurations
            vol.Optional("module_walk", default=modules.get("walk", True)): cv.boolean,
            vol.Optional("module_feeding", default=modules.get("feeding", True)): cv.boolean,
            vol.Optional("module_health", default=modules.get("health", True)): cv.boolean,
            vol.Optional("module_gps", default=modules.get("gps", True)): cv.boolean,
            vol.Optional("module_notifications", default=modules.get("notifications", True)): cv.boolean,
            vol.Optional("module_dashboard", default=modules.get("dashboard", True)): cv.boolean,
            vol.Optional("module_grooming", default=modules.get("grooming", True)): cv.boolean,
            vol.Optional("module_medication", default=modules.get("medication", True)): cv.boolean,
            vol.Optional("module_training", default=modules.get("training", True)): cv.boolean,
        })
    ...
    def gps_config_schema(existing_config: Dict[str, Any] | None = None) -> vol.Schema:
        ...
        return vol.Schema({
            vol.Optional("gps_enabled", default=defaults.get("enabled", True)): cv.boolean,
            vol.Optional("gps_accuracy_filter", default=defaults.get("accuracy_filter", GPS_MIN_ACCURACY)): vol.All(
                vol.Coerce(int), vol.Range(min=1, max=1000)
            ),
            vol.Optional("gps_distance_filter", default=defaults.get("distance_filter", GPS_POINT_FILTER_DISTANCE)): vol.All(
                vol.Coerce(int), vol.Range(min=1, max=100)
            ),
            vol.Optional("gps_update_interval", default=defaults.get("update_interval", 30)): vol.All(
                vol.Coerce(int), vol.Range(min=10, max=300)
            ),
            vol.Optional("auto_start_walk", default=defaults.get("auto_start_walk", False)): cv.boolean,
            vol.Optional("auto_end_walk", default=defaults.get("auto_end_walk", True)): cv.boolean,
            vol.Optional("route_recording", default=defaults.get("route_recording", True)): cv.boolean,
            vol.Optional("route_history_days", default=defaults.get("route_history_days", 90)): vol.All(
                vol.Coerce(int), vol.Range(min=1, max=365)
            ),
        })
    
    @staticmethod
    def geofence_config_schema(existing_config: Dict[str, Any] | None = None, home_lat: float = 0.0, home_lon: float = 0.0) -> vol.Schema:
        ...
        return vol.Schema({
            vol.Optional("geofencing_enabled", default=defaults.get("geofencing_enabled", True)): cv.boolean,
            vol.Optional("geofence_lat", default=defaults.get("geofence_lat", home_lat)): vol.Coerce(float),
            vol.Optional("geofence_lon", default=defaults.get("geofence_lon", home_lon)): vol.Coerce(float),
            vol.Optional("geofence_radius_m", default=defaults.get("geofence_radius_m", 150)): ConfigValidator.validate_radius,
            vol.Optional("geofence_alerts_enabled", default=defaults.get("geofence_alerts_enabled", True)): cv.boolean,
            vol.Optional("use_home_location", default=defaults.get("use_home_location", False)): cv.boolean,
            vol.Optional("multiple_zones", default=defaults.get("multiple_zones", False)): cv.boolean,
            vol.Optional("zone_detection_mode", default=defaults.get("zone_detection_mode", "home_assistant")): vol.In([
                "home_assistant", "custom", "both"
            ]),
        })
    
    @staticmethod
    def notification_config_schema(existing_config: Dict[str, Any] | None = None) -> vol.Schema:
        """Build schema for notification configuration."""
        defaults = existing_config or {}
        
        return vol.Schema({
            vol.Optional("notifications_enabled", default=defaults.get("enabled", True)): cv.boolean,
            vol.Optional("quiet_hours_enabled", default=defaults.get("quiet_hours_enabled", False)): cv.boolean,
            vol.Optional("quiet_start", default=defaults.get("quiet_start", "22:00")): ConfigValidator.validate_time_string,
            vol.Optional("quiet_end", default=defaults.get("quiet_end", "07:00")): ConfigValidator.validate_time_string,
            vol.Optional("reminder_repeat_min", default=defaults.get("reminder_repeat_min", DEFAULT_REMINDER_REPEAT)): vol.All(
                vol.Coerce(int), vol.Range(min=5, max=120)
            ),
            vol.Optional("snooze_min", default=defaults.get("snooze_min", DEFAULT_SNOOZE_MIN)): vol.All(
                vol.Coerce(int), vol.Range(min=5, max=60)
            ),
            vol.Optional("priority_notifications", default=defaults.get("priority_notifications", True)): cv.boolean,
            vol.Optional("summary_notifications", default=defaults.get("summary_notifications", True)): cv.boolean,
            vol.Optional("notification_channels", default=defaults.get("notification_channels", ["mobile", "persistent"])): ConfigValidator.validate_notification_channels,
        })
...
class ErrorHandler:
    ...
    @classmethod
    def validate_and_handle_errors(cls, data: Dict[str, Any], schema: vol.Schema) -> tuple[Dict[str, Any], Dict[str, str]]:
        ...
        try:
            ...
            for error in ex.errors:
                field_path = '.'.join(str(p) for p in error.path) if error.path else 'base'
...
class DataSanitizer:
    ...
    def sanitize_dog_config(data: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "dog_name" in data:
            try:
                sanitized["dog_name"] = ConfigValidator.validate_dog_name(data["dog_name"])
            except ValidationError:
                pass
...
def create_comprehensive_validator() -> Callable[[Dict[str, Any], str], tuple[Dict[str, Any], Dict[str, str]]]:
    ...
    """
    def validate_config_section(data: Dict[str, Any], section_type: str) -> tuple[Dict[str, Any], Dict[str, str]]:
