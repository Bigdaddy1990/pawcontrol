...
class PawControlConfigFlow(ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_dog_modules(
        ...
        if user_input is not None:
            ...
            if self._dogs:
                self._dogs[-1]["modules"] = {
                    "feeding": user_input.get("feeding", True),
                    "walk": user_input.get("walk", True),
                    "health": user_input.get("health", True),
                    "gps": user_input.get("gps", False),
                    "notifications": user_input.get("notifications", True),
                }

            return await self.async_step_add_another()

        current_dog = self._dogs[-1] if self._dogs else None
        dog_name = current_dog[CONF_DOG_NAME] if current_dog else "Unknown"
