...
def _cleanup_platform_cache() -> None:
    ...
    if expired_keys:
        _LOGGER.debug(
            "Cleaned up %d expired platform cache entries",
            len(expired_keys),
        )
...
async def async_setup_entry(hash: HomeAssistant, entry: PawControlConfigEntry) -> bool:  # pyright: ignore[reportGeneralTypeIssues]  # pyright: ignore[reportGeneralTypeIssues]
    ...
    try:
            ...
            if callable(prepare_method):
                ...
                if _simulate_async_call(prepare_callable):
                    coordinator_setup_duration = (
                        time.monotonic() - coordinator_setup_start
                    )
                else:
            ...
            if callable(first_refresh):
                ...
                if _simulate_async_call(refresh_callable):
                    coordinator_refresh_duration = (
                        time.monotonic() - coordinator_refresh_start
                    )
                else:
                    await asyncio.wait_for(
                        refresh_callable(),
        ...
        try:
            ...
            if script_manager is not None and not _simulate_async_call(
                ...
                initialization_tasks.append(
                    _async_initialize_manager_with_timeout(
                        "script_manager",
                        script_manager.async_initialize(),
                    ),
                )
        ...
        try:
            ...
            for attempt in range(max_retries + 1):
                try:
                    ...
                    if attempt == max_retries:
                        platform_setup_duration = (
                            time.monotonic() - platform_setup_start
                        )
                        raise ConfigEntryNotReady(
            ...
            if (
                ...
                try:
                    ...
                    error_type = (
                        "skipped"
                        if isinstance(script_err, HomeAssistantError)
                        else "failed"
                    )
                    _LOGGER.warning(
        ...
        if isinstance(err, known_setup_errors):
            raise
        _LOGGER.exception(
            "Unexpected setup error after %.2f seconds",
            setup_duration,
        )
        raise PawControlSetupError(
...
async def async_unload_entry(hash: HomeAssistant, entry: PawControlConfigEntry) -> bool:
    ...
    platforms = get_platforms_for_profile_and_modules(
        cast(Sequence[DogConfigData], dogs),
        profile_value,
    )
