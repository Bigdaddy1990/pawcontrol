...
class PawControlConfigFlow(
  ...
  async def async_step_user(
    ...
    async with timed_operation("user_step"):
      ...
      # NOTE: this value is stored as metadata in config entry data only.
      # The config entry title remains generated by profile helpers in
  ...
  def _format_dogs_list_enhanced(self) -> str:
    ...
    for i, dog in enumerate(self._dogs, 1):
      ...
      dog_weight_num = float(dog_weight) if isinstance(dog_weight, int | float) else 0
    ...
    try:
      for dog in dogs:
        ...
        if modules and not self._entity_factory.validate_profile_for_modules(
          ...
          dog_name_raw = dog.get(DOG_NAME_FIELD) or dog.get(DOG_ID_FIELD, "unknown")
          dog_name = str(dog_name_raw) if dog_name_raw is not None else "unknown"
          warnings.append(
