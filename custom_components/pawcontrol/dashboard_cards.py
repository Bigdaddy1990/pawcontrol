...
class DogCardGenerator(BaseCardGenerator):
    ...
    async def generate_dog_overview_cards(
        ...
        # Action buttons
        action_buttons = await self.templates.get_action_buttons_template(dog_id, modules)
        if action_buttons:
...
class StatisticsCardGenerator(BaseCardGenerator):
    ...
    def _generate_summary_card(self, dogs_config: list[dict[str, Any]]) -> dict[str, Any]:
