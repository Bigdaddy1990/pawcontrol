...
async def async_setup_entry(
    ...
    if entities:
        ...
        _LOGGER.info(
            "Created %d device tracker entities for GPS-enabled dogs",
            len(entities)
        )
    else:
...
class PawControlDeviceTracker(
    ...
    def _is_currently_moving(self, gps_data: Dict[str, Any]) -> bool:
        ...
        if (current_lat is not None and current_lon is not None and 
            self._last_known_location is not None):
    ...
    def _get_tracking_status(self, gps_data: Dict[str, Any]) -> str:
        ...
        """
        has_location = (
            gps_data.get("latitude") is not None and 
        ...
        if signal_quality in ["excellent", "good"] and data_freshness in ["current", "recent"]:
    ...
    async def async_update_location(
        ...
        _LOGGER.info(
            "Manual location update for %s: %f, %f",
            self._dog_name,
            latitude,
            longitude
        )
    ...
    def get_location_history(self, hours: int = 24) -> List[Dict[str, Any]]:
        ...
        return [
            entry for entry in self._location_history
    ...
    def calculate_distance_traveled(self, hours: int = 24) -> float:
        ...
        for i in range(1, len(history)):
            ...
            dist = distance(
                prev_entry["latitude"], prev_entry["longitude"],
