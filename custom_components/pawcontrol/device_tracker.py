    ...
    try:
        ...
        for dog_config in dogs:
            ...
            if not modules.get(MODULE_GPS, False):
                _LOGGER.debug(
                    "GPS module disabled for dog %s, skipping tracker", dog_id
                )
                continue
...
class PawDeviceTracker(PawControlDeviceTrackerEntity, TrackerEntity):
    ...
    def location_accuracy(self) -> int | None:
        ...
        try:
            ...
            _LOGGER.debug(
                "Error getting location accuracy for %s: %s", self.dog_id, err
            )
            return None
            ...
            attributes.update(
                {
                    ...
                    "safe_zone_radius": location_data.get(
                        "safe_zone_radius", DEFAULT_SAFE_ZONE_RADIUS
                    ),
                }
            )
            ...
            attributes.update(
                {
                    ...
                    "time_inside_today_min": daily_stats.get(
                        "time_inside_today_min", 0
                    ),
                    "time_outside_today_min": daily_stats.get(
                        "time_outside_today_min", 0
                    ),
                    "total_distance_today_m": daily_stats.get(
                        "total_distance_today_m", 0
                    ),
                }
            )
