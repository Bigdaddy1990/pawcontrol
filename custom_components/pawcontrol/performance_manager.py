...
class PerformanceMonitor:
    ...
    def __init__(self, alert_threshold: float = PERFORMANCE_ALERT_THRESHOLD) -> None:
        ...
        _LOGGER.debug("PerformanceMonitor initialized with threshold=%.1fs", alert_threshold)
    ...
    def get_recent_performance(self, minutes: int = 5) -> dict[str, Any]:
        ...
        recent_times = list(self._update_times)[-recent_count:] if recent_count > 0 else []
        recent_errors = list(self._error_counts)[-recent_count:] if recent_count > 0 else []
    ...
    def update_alert_threshold(self, new_threshold: float) -> dict[str, Any]:
        ...
        if self._update_times:
            ...
            return {
                ...
                "impact_assessment": self._assess_threshold_impact(old_alerts, new_alerts),
            }
        else:
            return {
                "old_threshold": old_threshold,
    ...
    def _assess_threshold_impact(self, old_alerts: int, new_alerts: int) -> str:
        ...
        if new_alerts == old_alerts:
            ...
            return f"Will increase alerts by {increase} ({increase/old_alerts*100:.1f}% more)" if old_alerts > 0 else f"Will generate {new_alerts} new alerts"
        else:
            decrease = old_alerts - new_alerts
            return f"Will reduce alerts by {decrease} ({decrease/old_alerts*100:.1f}% fewer)" if old_alerts > 0 else "Will eliminate all alerts"
    ...
    def analyze_performance_trend(self) -> dict[str, Any]:
        ...
        return {
            ...
            "analysis_confidence": "high" if len(times) >= 50 else "medium" if len(times) >= 20 else "low",
        }
    ...
    def get_performance_health_score(self) -> dict[str, Any]:
        ...
        if stats["average_update_time"] > self._alert_threshold:
            speed_score = max(0, 100 - (stats["average_update_time"] / self._alert_threshold - 1) * 50)
        
        reliability_score = max(0, 100 - stats["error_rate"] * 2)  # 2% penalty per 1% error rate
        ...
        return {
            ...
            "recommendations": self._get_health_recommendations(speed_score, reliability_score, consistency_score),
        }

    def _get_health_recommendations(self, speed: float, reliability: float, consistency: float) -> list[str]:
        ...
        if speed < 75:
            recommendations.append("Consider optimizing update algorithms or reducing data processing load")
        
        if reliability < 75:
            recommendations.append("Investigate and fix error sources to improve reliability")
        
        if consistency < 75:
            recommendations.append("Reduce performance variance through better resource management")
        
        if speed > 90 and reliability > 90 and consistency > 90:
            recommendations.append("Performance is excellent - consider monitoring trends for early degradation detection")
