...
class PawControlCoordinator(DataUpdateCoordinator[dict[str, Any]]):
    ...
    async def _update_dog_calculated_fields(self, dog_id: str, data: DogData) -> None:
        ...
        try:
            ...
            data["feeding"]["is_hungry"] = self.feeding_manager.calculate_is_hungry(
                dog_id
            )

            # Health calculations
            health_results = await self.health_calculator.calculate_health_metrics(
                dog_id, data
            )
            ...
            _LOGGER.error(
                "Failed to update calculated fields for dog %s: %s", dog_id, err
            )
